...
import { milestones, patchMeta } from './milestones-v666-v670.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('presentability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('PresentabilityModule')) {
...
if (!tests.includes("describe('presentability rollout integration'")) {
...
if (!app.includes('PresentabilityRolloutResponse')) {
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.165\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.166-v5.170 rollout milestones.')
