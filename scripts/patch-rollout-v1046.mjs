...
import { milestones, patchMeta } from './milestones-v1046-v1050.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('./registryvaultizability-rollout.js')) {
  index = index.replace(
...
if (!appModule.includes("from './registryvaultizability/registryvaultizability.module.js'")) {
...
if (!tests.includes("describe('registryvaultizability rollout integration'")) {
...
if (!app.includes("from './registryvaultizability-ui'")) {
  ...
  app = app.replace(
    '  IntegrityjournalizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  IntegrityjournalizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
...
console.log('Patched shared files for v5.546-v5.550 rollout milestones.')
