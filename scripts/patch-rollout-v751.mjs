...
import { milestones, patchMeta } from './milestones-v751-v755.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('abstractizability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('AbstractizabilityModule')) {
...
if (!tests.includes("describe('abstractizability rollout integration'")) {
...
if (!app.includes('AbstractizabilityRolloutResponse')) {
  ...
  app = app.replace(
    '  CanonicalizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  CanonicalizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.250\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.251-v5.255 rollout milestones.')
