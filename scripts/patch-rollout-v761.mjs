...
import { milestones, patchMeta } from './milestones-v761-v765.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('inductizability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('InductizabilityModule')) {
...
if (!tests.includes("describe('inductizability rollout integration'")) {
...
if (!app.includes('InductizabilityRolloutResponse')) {
  ...
  app = app.replace(
    '  ExtrapolizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  ExtrapolizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.260\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.261-v5.265 rollout milestones.')
