...
import { milestones, patchMeta } from './milestones-v781-v785.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('scalabilizability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('ScalabilizabilityModule')) {
...
if (!tests.includes("describe('scalabilizability rollout integration'")) {
...
if (!app.includes('ScalabilizabilityRolloutResponse')) {
  ...
  app = app.replace(
    '  AdaptizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  AdaptizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.280\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.281-v5.285 rollout milestones.')
