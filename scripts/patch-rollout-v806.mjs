...
import { milestones, patchMeta } from './milestones-v806-v810.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('rollbackabilizability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('RollbackabilizabilityModule')) {
...
if (!tests.includes("describe('rollbackabilizability rollout integration'")) {
...
if (!app.includes('RollbackabilizabilityRolloutResponse')) {
  ...
  app = app.replace(
    '  TroubleshootizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  TroubleshootizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.225\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.306-v5.310 rollout milestones.')
