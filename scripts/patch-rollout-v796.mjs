...
import { milestones, patchMeta } from './milestones-v796-v800.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('restorabilizability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('RestorabilizabilityModule')) {
...
if (!tests.includes("describe('restorabilizability rollout integration'")) {
...
if (!app.includes('RestorabilizabilityRolloutResponse')) {
  ...
  app = app.replace(
    '  ObservabilizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  ObservabilizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
...
console.log('Patched shared files for v5.296-v5.300 rollout milestones.')
