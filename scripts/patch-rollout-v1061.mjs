...
import { milestones, patchMeta } from './milestones-v1061-v1065.mjs'
...
const propsAnchor = {
  defensibilityvaultizability: 'defensibility',
  explainabilityvaultizability: 'explainability',
  demonstrabilityvaultizability: 'demonstrability',
  justifiabilityvaultizability: 'justifiability',
  reviewabilityvaultizability: 'reviewability',
}
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('./defensibilityvaultizability-rollout.js')) {
  index = index.replace(
...
if (!appModule.includes("from './defensibilityvaultizability/defensibilityvaultizability.module.js'")) {
...
if (!tests.includes("describe('defensibilityvaultizability rollout integration'")) {
...
if (!app.includes('DefensibilityvaultizabilityRolloutResponse')) {
...
for (const m of patchMeta) {
  ...
  if (anchor) {
    ...
    app = app.replace(
      `            handle${anchor.charAt(0).toUpperCase() + anchor.slice(1)}AdminAction,`,
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.560\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.561-v5.565 rollout milestones.')
