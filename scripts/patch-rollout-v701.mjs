...
import { milestones, patchMeta } from './milestones-v701-v705.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('epistemizability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('EpistemizabilityModule')) {
...
if (!tests.includes("describe('epistemizability rollout integration'")) {
...
if (!app.includes('EpistemizabilityRolloutResponse')) {
  ...
  app = app.replace(
    '  HermeticizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  HermeticizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.200\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.201-v5.205 rollout milestones.')
