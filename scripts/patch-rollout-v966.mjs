...
import { milestones, patchMeta } from './milestones-v966-v970.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('./certificationizability-rollout.js')) {
  index = index.replace(
...
if (!appModule.includes("from './certificationizability/certificationizability.module.js'")) {
...
if (!tests.includes("describe('certificationizability rollout integration'")) {
...
if (!app.includes("from './certificationizability-ui'")) {
  ...
  app = app.replace(
    '  AttestationizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  AttestationizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
...
console.log('Patched shared files for v5.466-v5.470 rollout milestones.')
