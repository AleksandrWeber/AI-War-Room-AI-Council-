...
import { milestones, patchMeta } from './milestones-v811-v815.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('scriptabilizability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('ScriptabilizabilityModule')) {
...
if (!tests.includes("describe('scriptabilizability rollout integration'")) {
...
if (!app.includes('ScriptabilizabilityRolloutResponse')) {
  ...
  app = app.replace(
    '  FeatureflagizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  FeatureflagizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.310\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.311-v5.315 rollout milestones.')
