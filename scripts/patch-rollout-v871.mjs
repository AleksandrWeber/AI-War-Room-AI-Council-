...
import { milestones, patchMeta } from './milestones-v871-v875.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('ackizability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('AckizabilityModule')) {
...
if (!tests.includes("describe('ackizability rollout integration'")) {
...
if (!app.includes('AckizabilityRolloutResponse')) {
  ...
  app = app.replace(
    '  TimeoutizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  TimeoutizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
...
console.log('Patched shared files for v5.371-v5.375 rollout milestones.')
