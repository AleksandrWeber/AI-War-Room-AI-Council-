...
import { milestones, patchMeta } from './milestones-v1101-v1105.mjs'
...
const propsAnchor = {
  orchestrabilityvaultizability: 'orchestrability',
  schedulabilityvaultizability: 'schedulability',
  automatabilityvaultizability: 'automatability',
  monitorabilityvaultizability: 'monitorability',
  predictabilityvaultizability: 'predictability',
}

const bulkImportAnchor = 'integrabilityvaultizability-ui'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('./orchestrabilityvaultizability-rollout.js')) {
  index = index.replace(
...
if (!appModule.includes("from './orchestrabilityvaultizability/orchestrabilityvaultizability.module.js'")) {
...
if (!tests.includes("describe('orchestrabilityvaultizability rollout integration'")) {
...
if (!app.includes('OrchestrabilityvaultizabilityRolloutResponse')) {
  ...
  app = app.replace(
    '  IntegrabilityvaultizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  IntegrabilityvaultizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
}
...
for (const m of patchMeta) {
  ...
  if (!domainBulk.includes(`from '../../${m.name}-ui'`)) {
    domainBulk = domainBulk.replace(
      `  formatIntegrabilityvaultizabilityDomain,\n} from '../../${bulkImportAnchor}'`,
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      'Current `v5.600` behavior:',
      `${block}Current \`v5.600\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.601-v5.605 rollout milestones.')
