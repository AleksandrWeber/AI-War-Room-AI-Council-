...
import { milestones, patchMeta } from './milestones-v766-v770.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('interpolizability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('InterpolizabilityModule')) {
...
if (!tests.includes("describe('interpolizability rollout integration'")) {
...
if (!app.includes('InterpolizabilityRolloutResponse')) {
  ...
  app = app.replace(
    '  FalsifiizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  FalsifiizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
...
console.log('Patched shared files for v5.266-v5.270 rollout milestones.')
