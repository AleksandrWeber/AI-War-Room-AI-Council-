...
import { milestones, patchMeta } from './milestones-v721-v725.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('catalogizability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('CatalogizabilityModule')) {
...
if (!tests.includes("describe('catalogizability rollout integration'")) {
...
if (!app.includes('CatalogizabilityRolloutResponse')) {
  ...
  app = app.replace(
    '  NomenclatizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  NomenclatizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.220\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.221-v5.225 rollout milestones.')
