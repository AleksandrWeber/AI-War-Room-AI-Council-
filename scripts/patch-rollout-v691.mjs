...
import { milestones, patchMeta } from './milestones-v691-v695.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('semiotizability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('SemiotizabilityModule')) {
...
if (!tests.includes("describe('semiotizability rollout integration'")) {
...
if (!app.includes('SemiotizabilityRolloutResponse')) {
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.190\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.191-v5.195 rollout milestones.')
