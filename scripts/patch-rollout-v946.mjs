...
import { milestones, patchMeta } from './milestones-v946-v950.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('clusteringizability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('ClusteringizabilityModule')) {
...
if (!tests.includes("describe('clusteringizability rollout integration'")) {
...
if (!app.includes('ClusteringizabilityRolloutResponse')) {
  ...
  app = app.replace(
    '  BalancingizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  BalancingizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
...
console.log('Patched shared files for v5.446-v5.450 rollout milestones.')
