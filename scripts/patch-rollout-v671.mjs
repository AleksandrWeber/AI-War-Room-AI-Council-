...
import { milestones, patchMeta } from './milestones-v671-v675.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('symbolizability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('SymbolizabilityModule')) {
...
if (!tests.includes("describe('symbolizability rollout integration'")) {
...
if (!app.includes('SymbolizabilityRolloutResponse')) {
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.170\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.171-v5.175 rollout milestones.')
