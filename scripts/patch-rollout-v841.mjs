...
import { milestones, patchMeta } from './milestones-v841-v845.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('topologizability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('TopologizabilityModule')) {
...
if (!tests.includes("describe('topologizability rollout integration'")) {
...
if (!app.includes('TopologizabilityRolloutResponse')) {
  ...
  app = app.replace(
    '  MeshabilizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  MeshabilizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
...
console.log('Patched shared files for v5.341-v5.345 rollout milestones.')
