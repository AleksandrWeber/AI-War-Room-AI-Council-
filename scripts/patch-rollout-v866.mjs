...
import { milestones, patchMeta } from './milestones-v866-v870.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('bufferizability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('BufferizabilityModule')) {
...
if (!tests.includes("describe('bufferizability rollout integration'")) {
...
if (!app.includes('BufferizabilityRolloutResponse')) {
  ...
  app = app.replace(
    '  DebouncizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  DebouncizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
...
console.log('Patched shared files for v5.366-v5.370 rollout milestones.')
