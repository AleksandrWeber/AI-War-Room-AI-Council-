...
import { milestones, patchMeta } from './milestones-v836-v840.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('virtualizability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('VirtualizabilityModule')) {
...
if (!tests.includes("describe('virtualizability rollout integration'")) {
...
if (!app.includes('VirtualizabilityRolloutResponse')) {
  ...
  app = app.replace(
    '  BoundarizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  BoundarizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
...
console.log('Patched shared files for v5.336-v5.340 rollout milestones.')
