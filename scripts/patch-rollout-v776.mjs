...
import { milestones, patchMeta } from './milestones-v776-v780.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('approximatizability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('ApproximatizabilityModule')) {
...
if (!tests.includes("describe('approximatizability rollout integration'")) {
...
if (!app.includes('ApproximatizabilityRolloutResponse')) {
  ...
  app = app.replace(
    '  TolerizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  TolerizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
...
console.log('Patched shared files for v5.276-v5.280 rollout milestones.')
