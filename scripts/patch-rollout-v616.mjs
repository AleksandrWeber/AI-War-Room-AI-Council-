...
import { milestones, patchMeta } from './milestones-v616-v620.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('repeatability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('RepeatabilityModule')) {
...
if (!tests.includes("describe('repeatability rollout integration'")) {
...
if (!app.includes('repeatabilityRollout')) {
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.115\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.116-v5.120 rollout milestones.')
