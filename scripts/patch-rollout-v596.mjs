...
import { milestones, patchMeta } from './milestones-v596-v600.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('compatibility-rollout')) {
  index = index.replace(
...
if (!appModule.includes('CompatibilityModule')) {
...
if (!tests.includes("describe('compatibility rollout integration'")) {
...
if (!app.includes('compatibilityRollout')) {
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.95\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.96-v5.100 rollout milestones.')
