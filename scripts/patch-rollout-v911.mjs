...
import { milestones, patchMeta } from './milestones-v911-v915.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('restoreizability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('RestoreizabilityModule')) {
...
if (!tests.includes("describe('restoreizability rollout integration'")) {
...
if (!app.includes('RestoreizabilityRolloutResponse')) {
  ...
  app = app.replace(
    '  ArchiveizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  ArchiveizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
...
console.log('Patched shared files for v5.411-v5.415 rollout milestones.')
