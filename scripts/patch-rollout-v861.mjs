...
import { milestones, patchMeta } from './milestones-v861-v865.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('unicastizability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('UnicastizabilityModule')) {
...
if (!tests.includes("describe('unicastizability rollout integration'")) {
...
if (!app.includes('UnicastizabilityRolloutResponse')) {
  ...
  app = app.replace(
    '  MulticastizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  MulticastizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
...
console.log('Patched shared files for v5.361-v5.365 rollout milestones.')
