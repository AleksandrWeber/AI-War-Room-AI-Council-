...
import { milestones, patchMeta } from './milestones-v926-v930.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('groupizability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('GroupizabilityModule')) {
...
if (!tests.includes("describe('groupizability rollout integration'")) {
...
if (!app.includes('GroupizabilityRolloutResponse')) {
  ...
  app = app.replace(
    '  PivotizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  PivotizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
...
console.log('Patched shared files for v5.426-v5.430 rollout milestones.')
