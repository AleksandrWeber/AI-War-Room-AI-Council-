...
import { milestones, patchMeta } from './milestones-v1086-v1090.mjs'
...
const propsAnchor = {
  compatibilityvaultizability: 'compatibility',
  adaptabilityvaultizability: 'adaptability',
  flexibilityvaultizability: 'flexibility',
  extensibilityvaultizability: 'extensibility',
  modifiabilityvaultizability: 'modifiability',
}

const bulkImportAnchor = 'portabilityvaultizability-ui'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('./compatibilityvaultizability-rollout.js')) {
  index = index.replace(
...
if (!appModule.includes("from './compatibilityvaultizability/compatibilityvaultizability.module.js'")) {
...
if (!tests.includes("describe('compatibilityvaultizability rollout integration'")) {
...
if (!app.includes('CompatibilityvaultizabilityRolloutResponse')) {
...
for (const m of patchMeta) {
  ...
  if (!domainBulk.includes(`from '../../${m.name}-ui'`)) {
    domainBulk = domainBulk.replace(
      `  formatPortabilityvaultizabilityDomain,\n} from '../../${bulkImportAnchor}'`,
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      'Current `v5.585` behavior:',
      `${block}Current \`v5.585\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.586-v5.590 rollout milestones.')
