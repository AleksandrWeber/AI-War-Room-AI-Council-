...
import { milestones, patchMeta } from './milestones-v731-v735.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('bibliographizability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('BibliographizabilityModule')) {
...
if (!tests.includes("describe('bibliographizability rollout integration'")) {
...
if (!app.includes('BibliographizabilityRolloutResponse')) {
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.230\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.231-v5.235 rollout milestones.')
