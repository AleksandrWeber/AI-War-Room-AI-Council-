...
import { milestones, patchMeta } from './milestones-v886-v890.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('quorumizability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('QuorumizabilityModule')) {
...
if (!tests.includes("describe('quorumizability rollout integration'")) {
...
if (!app.includes('QuorumizabilityRolloutResponse')) {
  ...
  app = app.replace(
    '  ConsensusizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  ConsensusizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
...
console.log('Patched shared files for v5.386-v5.390 rollout milestones.')
