...
import { milestones, patchMeta } from './milestones-v791-v795.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('availabilizability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('AvailabilizabilityModule')) {
...
if (!tests.includes("describe('availabilizability rollout integration'")) {
...
if (!app.includes('AvailabilizabilityRolloutResponse')) {
  ...
  app = app.replace(
    '  SustainizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  SustainizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.290\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.291-v5.295 rollout milestones.')
