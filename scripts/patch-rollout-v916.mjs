...
import { milestones, patchMeta } from './milestones-v916-v920.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('searchizability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('SearchizabilityModule')) {
...
if (!tests.includes("describe('searchizability rollout integration'")) {
...
if (!app.includes('SearchizabilityRolloutResponse')) {
  ...
  app = app.replace(
    '  IndexingizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  IndexingizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
...
console.log('Patched shared files for v5.416-v5.420 rollout milestones.')
