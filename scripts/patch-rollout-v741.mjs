...
import { milestones, patchMeta } from './milestones-v741-v745.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('glossarizability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('GlossarizabilityModule')) {
...
if (!tests.includes("describe('glossarizability rollout integration'")) {
...
if (!app.includes('GlossarizabilityRolloutResponse')) {
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.240\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.241-v5.245 rollout milestones.')
