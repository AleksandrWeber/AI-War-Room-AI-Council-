...
import { milestones, patchMeta } from './milestones-v621-v625.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('usability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('UsabilityModule')) {
...
if (!tests.includes("describe('usability rollout integration'")) {
...
if (!app.includes('usabilityRollout')) {
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.120\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.121-v5.125 rollout milestones.')
