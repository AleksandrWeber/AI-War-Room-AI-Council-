...
import { milestones, patchMeta } from './milestones-v586-v590.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('referencability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('ReferencabilityModule')) {
...
if (!tests.includes("describe('referencability rollout integration'")) {
...
if (!app.includes('referencabilityRollout')) {
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.85\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.86-v5.90 rollout milestones.')
