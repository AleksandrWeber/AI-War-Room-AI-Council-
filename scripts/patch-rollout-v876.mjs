...
import { milestones, patchMeta } from './milestones-v876-v880.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('partitionizability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('PartitionizabilityModule')) {
...
if (!tests.includes("describe('partitionizability rollout integration'")) {
...
if (!app.includes('PartitionizabilityRolloutResponse')) {
  ...
  app = app.replace(
    '  SequencizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  SequencizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
...
console.log('Patched shared files for v5.376-v5.380 rollout milestones.')
