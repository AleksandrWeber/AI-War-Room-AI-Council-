...
import { milestones, patchMeta } from './milestones-v996-v1000.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('./identityproofizability-rollout.js')) {
  index = index.replace(
...
if (!appModule.includes("from './identityproofizability/identityproofizability.module.js'")) {
...
if (!tests.includes("describe('identityproofizability rollout integration'")) {
...
if (!app.includes("from './identityproofizability-ui'")) {
  ...
  app = app.replace(
    '  ZerotrustizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  ZerotrustizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
...
console.log('Patched shared files for v5.496-v5.500 rollout milestones.')
