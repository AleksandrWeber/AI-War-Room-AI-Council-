...
import { milestones, patchMeta } from './milestones-v576-v580.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('assessability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('AssessabilityModule')) {
...
if (!tests.includes("describe('assessability rollout integration'")) {
...
if (!app.includes('assessabilityRollout')) {
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.75\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.76-v5.80 rollout milestones.')
