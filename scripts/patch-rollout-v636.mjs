...
import { milestones, patchMeta } from './milestones-v636-v640.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('learnability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('LearnabilityModule')) {
...
if (!tests.includes("describe('learnability rollout integration'")) {
...
if (!app.includes('learnabilityRollout')) {
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.135\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.136-v5.140 rollout milestones.')
