...
import { milestones, patchMeta } from './milestones-v746-v750.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('contextualizability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('ContextualizabilityModule')) {
...
if (!tests.includes("describe('contextualizability rollout integration'")) {
...
if (!app.includes('ContextualizabilityRolloutResponse')) {
  ...
  app = app.replace(
    '  FootnotizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  FootnotizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
...
console.log('Patched shared files for v5.246-v5.250 rollout milestones.')
