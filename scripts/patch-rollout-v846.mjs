...
import { milestones, patchMeta } from './milestones-v846-v850.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('routizability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('RoutizabilityModule')) {
...
if (!tests.includes("describe('routizability rollout integration'")) {
...
if (!app.includes('RoutizabilityRolloutResponse')) {
  ...
  app = app.replace(
    '  RelayizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  RelayizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
...
console.log('Patched shared files for v5.346-v5.350 rollout milestones.')
