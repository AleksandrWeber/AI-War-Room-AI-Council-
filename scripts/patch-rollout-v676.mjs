...
import { milestones, patchMeta } from './milestones-v676-v680.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('typifiability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('TypifiabilityModule')) {
...
if (!tests.includes("describe('typifiability rollout integration'")) {
...
if (!app.includes('TypifiabilityRolloutResponse')) {
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.175\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.176-v5.180 rollout milestones.')
