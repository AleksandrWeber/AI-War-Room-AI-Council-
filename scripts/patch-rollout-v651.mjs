...
import { milestones, patchMeta } from './milestones-v651-v655.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('recognizability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('RecognizabilityModule')) {
...
if (!tests.includes("describe('recognizability rollout integration'")) {
...
if (!app.includes('recognizabilityRollout')) {
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.150\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.151-v5.155 rollout milestones.')
