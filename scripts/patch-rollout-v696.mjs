...
import { milestones, patchMeta } from './milestones-v696-v700.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('syntacticizability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('SyntacticizabilityModule')) {
...
if (!tests.includes("describe('syntacticizability rollout integration'")) {
...
if (!app.includes('SyntacticizabilityRolloutResponse')) {
  ...
  app = app.replace(
    '  PragmatizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  PragmatizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.195\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.196-v5.200 rollout milestones.')
