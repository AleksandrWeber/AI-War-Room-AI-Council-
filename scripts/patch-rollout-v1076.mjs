...
import { milestones, patchMeta } from './milestones-v1076-v1080.mjs'
...
const propsAnchor = {
  referencabilityvaultizability: 'referencability',
  locatabilityvaultizability: 'locatability',
  retrievabilityvaultizability: 'retrievability',
  discoverabilityvaultizability: 'discoverability',
  navigabilityvaultizability: 'navigability',
}

const bulkImportAnchor = 'assignabilityvaultizability-ui'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('./referencabilityvaultizability-rollout.js')) {
  index = index.replace(
...
if (!appModule.includes("from './referencabilityvaultizability/referencabilityvaultizability.module.js'")) {
...
if (!tests.includes("describe('referencabilityvaultizability rollout integration'")) {
...
if (!app.includes('ReferencabilityvaultizabilityRolloutResponse')) {
  ...
  app = app.replace(
    '  AssignabilityvaultizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  AssignabilityvaultizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
}
...
for (const m of patchMeta) {
  ...
  if (!domainBulk.includes(`from '../../${m.name}-ui'`)) {
    domainBulk = domainBulk.replace(
      `  formatAssignabilityvaultizabilityDomain,\n} from '../../${bulkImportAnchor}'`,
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.575\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.576-v5.580 rollout milestones.')
