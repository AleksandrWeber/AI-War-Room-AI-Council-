...
import { milestones, patchMeta } from './milestones-v716-v720.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('systematizability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('SystematizabilityModule')) {
...
if (!tests.includes("describe('systematizability rollout integration'")) {
...
if (!app.includes('SystematizabilityRolloutResponse')) {
  ...
  app = app.replace(
    '  OrdinarizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  OrdinarizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.215\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.216-v5.220 rollout milestones.')
