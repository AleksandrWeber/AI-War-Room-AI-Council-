...
import { milestones, patchMeta } from './milestones-v726-v730.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('archivizability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('ArchivizabilityModule')) {
...
if (!tests.includes("describe('archivizability rollout integration'")) {
...
if (!app.includes('ArchivizabilityRolloutResponse')) {
  ...
  app = app.replace(
    '  RegistryizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  RegistryizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
...
console.log('Patched shared files for v5.226-v5.230 rollout milestones.')
