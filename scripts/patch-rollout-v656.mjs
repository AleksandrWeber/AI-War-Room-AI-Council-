...
import { milestones, patchMeta } from './milestones-v656-v660.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('discernibility-rollout')) {
  index = index.replace(
...
if (!appModule.includes('DiscernibilityModule')) {
...
if (!tests.includes("describe('discernibility rollout integration'")) {
...
if (!app.includes('discernibilityRollout')) {
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.155\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.156-v5.160 rollout milestones.')
