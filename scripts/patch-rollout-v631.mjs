...
import { milestones, patchMeta } from './milestones-v631-v635.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('affordability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('AffordabilityModule')) {
...
if (!tests.includes("describe('affordability rollout integration'")) {
...
if (!app.includes('affordabilityRollout')) {
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.130\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.131-v5.135 rollout milestones.')
