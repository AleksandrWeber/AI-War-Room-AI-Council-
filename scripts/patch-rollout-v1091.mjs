...
import { milestones, patchMeta } from './milestones-v1091-v1095.mjs'
...
const propsAnchor = {
  configurabilityvaultizability: 'configurability',
  customizabilityvaultizability: 'customizability',
  operabilityvaultizability: 'operability',
  tunabilityvaultizability: 'tunability',
  adjustabilityvaultizability: 'adjustability',
}

const bulkImportAnchor = 'modifiabilityvaultizability-ui'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('./configurabilityvaultizability-rollout.js')) {
  index = index.replace(
...
if (!appModule.includes("from './configurabilityvaultizability/configurabilityvaultizability.module.js'")) {
...
if (!tests.includes("describe('configurabilityvaultizability rollout integration'")) {
...
if (!app.includes('ConfigurabilityvaultizabilityRolloutResponse')) {
...
for (const m of patchMeta) {
  ...
  if (!domainBulk.includes(`from '../../${m.name}-ui'`)) {
    domainBulk = domainBulk.replace(
      `  formatModifiabilityvaultizabilityDomain,\n} from '../../${bulkImportAnchor}'`,
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      'Current `v5.590` behavior:',
      `${block}Current \`v5.590\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.591-v5.595 rollout milestones.')
