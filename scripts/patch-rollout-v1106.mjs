...
import { milestones, patchMeta } from './milestones-v1106-v1110.mjs'
...
const propsAnchor = {
  repeatabilityvaultizability: 'repeatability',
  responsivenessvaultizability: 'responsiveness',
  dependabilityvaultizability: 'dependability',
  composabilityvaultizability: 'composability',
  trustworthinessvaultizability: 'trustworthiness',
}

const bulkImportAnchor = 'predictabilityvaultizability-ui'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('./repeatabilityvaultizability-rollout.js')) {
  index = index.replace(
...
if (!appModule.includes("from './repeatabilityvaultizability/repeatabilityvaultizability.module.js'")) {
...
if (!tests.includes("describe('repeatabilityvaultizability rollout integration'")) {
...
if (!app.includes('RepeatabilityvaultizabilityRolloutResponse')) {
  ...
  app = app.replace(
    '  PredictabilityvaultizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  PredictabilityvaultizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
}
...
for (const m of patchMeta) {
  ...
  if (!domainBulk.includes(`from '../../${m.name}-ui'`)) {
    domainBulk = domainBulk.replace(
      `  formatPredictabilityvaultizabilityDomain,\n} from '../../${bulkImportAnchor}'`,
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      'Current `v5.605` behavior:',
      `${block}Current \`v5.605\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.606-v5.610 rollout milestones.')
