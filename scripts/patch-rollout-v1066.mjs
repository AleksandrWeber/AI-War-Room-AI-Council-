...
import { milestones, patchMeta } from './milestones-v1066-v1070.mjs'
...
const propsAnchor = {
  assessabilityvaultizability: 'assessability',
  measurabilityvaultizability: 'measurability',
  certifiabilityvaultizability: 'certifiability',
  substantiabilityvaultizability: 'substantiability',
  warrantabilityvaultizability: 'warrantability',
}

const bulkImportAnchor = 'reviewabilityvaultizability-ui'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('./assessabilityvaultizability-rollout.js')) {
  index = index.replace(
...
if (!appModule.includes("from './assessabilityvaultizability/assessabilityvaultizability.module.js'")) {
...
if (!tests.includes("describe('assessabilityvaultizability rollout integration'")) {
...
if (!app.includes('AssessabilityvaultizabilityRolloutResponse')) {
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.565\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.566-v5.570 rollout milestones.')
