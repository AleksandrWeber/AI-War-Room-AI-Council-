...
import { milestones, patchMeta } from './milestones-v786-v790.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('recoverizability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('RecoverizabilityModule')) {
...
if (!tests.includes("describe('recoverizability rollout integration'")) {
...
if (!app.includes('RecoverizabilityRolloutResponse')) {
  ...
  app = app.replace(
    '  DependableizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  DependableizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
...
console.log('Patched shared files for v5.286-v5.290 rollout milestones.')
