...
import { milestones, patchMeta } from './milestones-v821-v825.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('integrabilizability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('IntegrabilizabilityModule')) {
...
if (!tests.includes("describe('integrabilizability rollout integration'")) {
...
if (!app.includes('IntegrabilizabilityRolloutResponse')) {
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.320\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.321-v5.325 rollout milestones.')
