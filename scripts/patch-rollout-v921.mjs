...
import { milestones, patchMeta } from './milestones-v921-v925.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('queryizability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('QueryizabilityModule')) {
...
if (!tests.includes("describe('queryizability rollout integration'")) {
...
if (!app.includes('QueryizabilityRolloutResponse')) {
  ...
  app = app.replace(
    '  RetentionizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  RetentionizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
...
console.log('Patched shared files for v5.421-v5.425 rollout milestones.')
