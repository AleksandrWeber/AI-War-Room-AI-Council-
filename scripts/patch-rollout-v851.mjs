...
import { milestones, patchMeta } from './milestones-v851-v855.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('subscribizability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('SubscribizabilityModule')) {
...
if (!tests.includes("describe('subscribizability rollout integration'")) {
...
if (!app.includes('SubscribizabilityRolloutResponse')) {
  ...
  app = app.replace(
    '  NotifizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  NotifizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
...
console.log('Patched shared files for v5.351-v5.355 rollout milestones.')
