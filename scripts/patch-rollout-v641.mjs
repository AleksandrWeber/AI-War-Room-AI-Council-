...
import { milestones, patchMeta } from './milestones-v641-v645.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('readability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('ReadabilityModule')) {
...
if (!tests.includes("describe('readability rollout integration'")) {
...
if (!app.includes('readabilityRollout')) {
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.140\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.141-v5.145 rollout milestones.')
