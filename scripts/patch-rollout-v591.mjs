...
import { milestones, patchMeta } from './milestones-v591-v595.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('connectability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('ConnectabilityModule')) {
...
if (!tests.includes("describe('connectability rollout integration'")) {
...
if (!app.includes('connectabilityRollout')) {
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.90\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.91-v5.95 rollout milestones.')
