...
import { milestones, patchMeta } from './milestones-v661-v665.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('expressiveness-rollout')) {
  index = index.replace(
...
if (!appModule.includes('ExpressivenessModule')) {
...
if (!tests.includes("describe('expressiveness rollout integration'")) {
...
if (!app.includes('expressivenessRollout')) {
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.160\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.161-v5.165 rollout milestones.')
