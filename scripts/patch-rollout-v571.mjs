...
import { milestones, patchMeta } from './milestones-v571-v575.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('inspectability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('InspectabilityModule')) {
...
if (!tests.includes("describe('inspectability rollout integration'")) {
...
if (!app.includes('inspectabilityRollout')) {
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.70\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.71-v5.75 rollout milestones.')
