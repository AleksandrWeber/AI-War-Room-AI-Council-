...
import { milestones, patchMeta } from './milestones-v711-v715.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('taxonomizability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('TaxonomizabilityModule')) {
...
if (!tests.includes("describe('taxonomizability rollout integration'")) {
...
if (!app.includes('TaxonomizabilityRolloutResponse')) {
  ...
  app = app.replace(
    '  CategorizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  CategorizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.210\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.211-v5.215 rollout milestones.')
