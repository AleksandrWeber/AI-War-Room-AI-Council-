...
import { milestones, patchMeta } from './milestones-v626-v630.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('viability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('ViabilityModule')) {
...
if (!tests.includes("describe('viability rollout integration'")) {
...
if (!app.includes('viabilityRollout')) {
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.125\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.126-v5.130 rollout milestones.')
