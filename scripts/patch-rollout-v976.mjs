...
import { milestones, patchMeta } from './milestones-v976-v980.mjs'

const root = join(import.meta.dirname, '..')
const BASE = {
  name: 'controlizability',
  Name: 'Controlizability',
}
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('./entitlementizability-rollout.js')) {
  index = index.replace(
...
if (!appModule.includes("from './entitlementizability/entitlementizability.module.js'")) {
...
if (!tests.includes("describe('entitlementizability rollout integration'")) {
...
if (!app.includes("from './entitlementizability-ui'")) {
  ...
  app = app.replace(
    '  ControlizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  ControlizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
...
console.log('Patched shared files for v5.476-v5.480 rollout milestones.')
