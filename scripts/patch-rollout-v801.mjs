...
import { milestones, patchMeta } from './milestones-v801-v805.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('configurabilizability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('ConfigurabilizabilityModule')) {
...
if (!tests.includes("describe('configurabilizability rollout integration'")) {
...
if (!app.includes('ConfigurabilizabilityRolloutResponse')) {
  ...
  app = app.replace(
    '  DeployabilizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  DeployabilizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.300\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.301-v5.305 rollout milestones.')
