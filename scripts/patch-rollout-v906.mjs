...
import { milestones, patchMeta } from './milestones-v906-v910.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('memorizability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('MemorizabilityModule')) {
...
if (!tests.includes("describe('memorizability rollout integration'")) {
...
if (!app.includes('MemorizabilityRolloutResponse')) {
  ...
  app = app.replace(
    '  CacheizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  CacheizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
...
console.log('Patched shared files for v5.406-v5.410 rollout milestones.')
