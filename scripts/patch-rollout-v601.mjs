...
import { milestones, patchMeta } from './milestones-v601-v605.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('configurability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('ConfigurabilityModule')) {
...
if (!tests.includes("describe('configurability rollout integration'")) {
...
if (!app.includes('configurabilityRollout')) {
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.100\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.101-v5.105 rollout milestones.')
