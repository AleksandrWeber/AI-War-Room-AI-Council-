...
import { milestones, patchMeta } from './milestones-v816-v820.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('releasizability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('ReleasizabilityModule')) {
...
if (!tests.includes("describe('releasizability rollout integration'")) {
...
if (!app.includes('ReleasizabilityRolloutResponse')) {
  ...
  app = app.replace(
    '  TriggerizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  TriggerizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
...
console.log('Patched shared files for v5.316-v5.320 rollout milestones.')
