...
import { milestones, patchMeta } from './milestones-v771-v775.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('calibratizability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('CalibratizabilityModule')) {
...
if (!tests.includes("describe('calibratizability rollout integration'")) {
...
if (!app.includes('CalibratizabilityRolloutResponse')) {
  ...
  app = app.replace(
    '  OptimizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  OptimizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.270\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.271-v5.275 rollout milestones.')
