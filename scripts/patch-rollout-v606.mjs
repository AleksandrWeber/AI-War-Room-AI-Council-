...
import { milestones, patchMeta } from './milestones-v606-v610.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('programmability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('ProgrammabilityModule')) {
...
if (!tests.includes("describe('programmability rollout integration'")) {
...
if (!app.includes('programmabilityRollout')) {
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.105\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.106-v5.110 rollout milestones.')
