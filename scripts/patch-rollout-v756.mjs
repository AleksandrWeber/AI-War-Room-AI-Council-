...
import { milestones, patchMeta } from './milestones-v756-v760.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('probabilizability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('ProbabilizabilityModule')) {
...
if (!tests.includes("describe('probabilizability rollout integration'")) {
...
if (!app.includes('ProbabilizabilityRolloutResponse')) {
  ...
  app = app.replace(
    '  DeducizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  DeducizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
...
console.log('Patched shared files for v5.256-v5.260 rollout milestones.')
