...
import { milestones, patchMeta } from './milestones-v956-v960.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('./scalingizability-rollout.js')) {
  index = index.replace(
...
if (!appModule.includes("from './scalingizability/scalingizability.module.js'")) {
...
if (!tests.includes("describe('scalingizability rollout integration'")) {
...
if (!app.includes("from './scalingizability-ui'")) {
  ...
  app = app.replace(
    '  DeallocationizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  DeallocationizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
...
console.log('Patched shared files for v5.456-v5.460 rollout milestones.')
