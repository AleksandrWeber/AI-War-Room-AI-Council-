...
import { milestones, patchMeta } from './milestones-v686-v690.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('analogizability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('AnalogizabilityModule')) {
...
if (!tests.includes("describe('analogizability rollout integration'")) {
...
if (!app.includes('AnalogizabilityRolloutResponse')) {
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.185\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.186-v5.190 rollout milestones.')
