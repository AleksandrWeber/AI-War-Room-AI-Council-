...
import { milestones, patchMeta } from './milestones-v566-v570.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('validity-rollout')) {
  index = index.replace(
...
if (!appModule.includes('ValidityModule')) {
...
if (!tests.includes("describe('validity rollout integration'")) {
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.65\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.66-v5.70 rollout milestones.')
