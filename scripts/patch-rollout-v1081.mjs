...
import { milestones, patchMeta } from './milestones-v1081-v1085.mjs'
...
const propsAnchor = {
  connectabilityvaultizability: 'connectability',
  linkabilityvaultizability: 'linkability',
  interchangeabilityvaultizability: 'interchangeability',
  transferabilityvaultizability: 'transferability',
  portabilityvaultizability: 'portability',
}

const bulkImportAnchor = 'navigabilityvaultizability-ui'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('./connectabilityvaultizability-rollout.js')) {
  index = index.replace(
...
if (!appModule.includes("from './connectabilityvaultizability/connectabilityvaultizability.module.js'")) {
...
if (!tests.includes("describe('connectabilityvaultizability rollout integration'")) {
...
if (!app.includes('ConnectabilityvaultizabilityRolloutResponse')) {
...
for (const m of patchMeta) {
  ...
  if (!domainBulk.includes(`from '../../${m.name}-ui'`)) {
    domainBulk = domainBulk.replace(
      `  formatNavigabilityvaultizabilityDomain,\n} from '../../${bulkImportAnchor}'`,
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.580\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.581-v5.585 rollout milestones.')
