...
import { milestones, patchMeta } from './milestones-v971-v975.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('./auditingizability-rollout.js')) {
  index = index.replace(
...
if (!appModule.includes("from './auditingizability/auditingizability.module.js'")) {
...
if (!tests.includes("describe('auditingizability rollout integration'")) {
...
if (!app.includes("from './auditingizability-ui'")) {
  ...
  app = app.replace(
    '  TelemetryizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  TelemetryizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
...
console.log('Patched shared files for v5.471-v5.475 rollout milestones.')
