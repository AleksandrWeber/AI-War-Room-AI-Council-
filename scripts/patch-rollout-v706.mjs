...
import { milestones, patchMeta } from './milestones-v706-v710.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('teleologizability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('TeleologizabilityModule')) {
...
if (!tests.includes("describe('teleologizability rollout integration'")) {
...
if (!app.includes('TeleologizabilityRolloutResponse')) {
  ...
  app = app.replace(
    '  AxiologizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  AxiologizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.205\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.206-v5.210 rollout milestones.')
