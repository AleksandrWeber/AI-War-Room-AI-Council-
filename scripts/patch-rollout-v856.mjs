...
import { milestones, patchMeta } from './milestones-v856-v860.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('handoffizability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('HandoffizabilityModule')) {
...
if (!tests.includes("describe('handoffizability rollout integration'")) {
...
if (!app.includes('HandoffizabilityRolloutResponse')) {
  ...
  app = app.replace(
    '  DispatchizabilityAdminSummaryResponse,\n  RunCapabilitiesResponse,',
    `  DispatchizabilityAdminSummaryResponse,\n${typeBlock}\n  RunCapabilitiesResponse,`,
  )
...
console.log('Patched shared files for v5.356-v5.360 rollout milestones.')
