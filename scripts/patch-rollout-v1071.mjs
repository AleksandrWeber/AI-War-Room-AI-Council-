...
import { milestones, patchMeta } from './milestones-v1071-v1075.mjs'
...
const propsAnchor = {
  attributabilityvaultizability: 'attributability',
  identifiabilityvaultizability: 'identifiability',
  comparabilityvaultizability: 'comparability',
  distinguishabilityvaultizability: 'distinguishability',
  assignabilityvaultizability: 'assignability',
}

const bulkImportAnchor = 'warrantabilityvaultizability-ui'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('./attributabilityvaultizability-rollout.js')) {
  index = index.replace(
...
if (!appModule.includes("from './attributabilityvaultizability/attributabilityvaultizability.module.js'")) {
...
if (!tests.includes("describe('attributabilityvaultizability rollout integration'")) {
...
if (!app.includes('AttributabilityvaultizabilityRolloutResponse')) {
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.570\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.571-v5.575 rollout milestones.')
