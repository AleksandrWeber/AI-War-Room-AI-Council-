...
import { milestones, patchMeta } from './milestones-v646-v650.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('intelligibility-rollout')) {
  index = index.replace(
...
if (!appModule.includes('IntelligibilityModule')) {
...
if (!tests.includes("describe('intelligibility rollout integration'")) {
...
if (!app.includes('intelligibilityRollout')) {
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.145\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.146-v5.150 rollout milestones.')
