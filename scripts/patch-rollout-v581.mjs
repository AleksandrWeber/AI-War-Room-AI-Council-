...
import { milestones, patchMeta } from './milestones-v581-v585.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('attributability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('AttributabilityModule')) {
...
if (!tests.includes("describe('attributability rollout integration'")) {
...
if (!app.includes('attributabilityRollout')) {
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.80\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.81-v5.85 rollout milestones.')
