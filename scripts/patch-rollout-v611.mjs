...
import { milestones, patchMeta } from './milestones-v611-v615.mjs'
...
const exportBlock = milestones
  ...
  .join('\n')
if (!index.includes('orchestrability-rollout')) {
  index = index.replace(
...
if (!appModule.includes('OrchestrabilityModule')) {
...
if (!tests.includes("describe('orchestrability rollout integration'")) {
...
if (!app.includes('orchestrabilityRollout')) {
...
`
  if (!readme.includes(`Current \`${m.version}\``)) {
    readme = readme.replace(
      ...
      `${block}Current \`v5.110\` behavior:`,
    )
  }
}
writeFileSync(readmePath, readme)

console.log('Patched shared files for v5.111-v5.115 rollout milestones.')
