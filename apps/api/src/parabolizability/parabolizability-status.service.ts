...
import type { ParabolizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_PARABOLIZABILITY_TABLES } from './parabolizability-rollout.helpers.js'

const WORKSPACE_PARABOLIZABILITY_DOMAINS: Array<{
  domain: ParabolizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ParabolizabilityStatusService {
  ...
  async getParabolizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_PARABOLIZABILITY_TABLES,
    )

    return {
      existingParabolizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceParabolizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_PARABOLIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_PARABOLIZABILITY_DOMAINS.map(async (entry) => {
