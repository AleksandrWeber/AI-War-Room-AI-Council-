...
import type { CoordinationizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_COORDINATIONIZABILITY_TABLES } from './coordinationizability-rollout.helpers.js'

const WORKSPACE_COORDINATIONIZABILITY_DOMAINS: Array<{
  domain: CoordinationizabilityAdminDomain
  tableName: string
...
@Injectable()
export class CoordinationizabilityStatusService {
  ...
  async getCoordinationizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_COORDINATIONIZABILITY_TABLES,
    )

    return {
      existingCoordinationizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceCoordinationizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_COORDINATIONIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_COORDINATIONIZABILITY_DOMAINS.map(async (entry) => {
