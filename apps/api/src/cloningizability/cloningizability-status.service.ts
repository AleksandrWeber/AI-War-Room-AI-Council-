...
import type { CloningizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CLONINGIZABILITY_TABLES } from './cloningizability-rollout.helpers.js'

const WORKSPACE_CLONINGIZABILITY_DOMAINS: Array<{
  domain: CloningizabilityAdminDomain
  tableName: string
...
@Injectable()
export class CloningizabilityStatusService {
  ...
  async getCloningizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CLONINGIZABILITY_TABLES,
    )

    return {
      existingCloningizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceCloningizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CLONINGIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CLONINGIZABILITY_DOMAINS.map(async (entry) => {
