...
import type { ConnectabilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CONNECTABILITYVAULTIZABILITY_TABLES } from './connectabilityvaultizability-rollout.helpers.js'

const WORKSPACE_CONNECTABILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: ConnectabilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ConnectabilityvaultizabilityStatusService {
  ...
  async getConnectabilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CONNECTABILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingConnectabilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceConnectabilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CONNECTABILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CONNECTABILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
