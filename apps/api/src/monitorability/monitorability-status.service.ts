...
import type { MonitorabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_MONITORABILITY_TABLES } from './monitorability-rollout.helpers.js'

const WORKSPACE_MONITORABILITY_DOMAINS: Array<{
  domain: MonitorabilityAdminDomain
  tableName: string
...
@Injectable()
export class MonitorabilityStatusService {
  ...
  async getMonitorabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_MONITORABILITY_TABLES,
    )

    return {
      existingMonitorabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceMonitorabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_MONITORABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_MONITORABILITY_DOMAINS.map(async (entry) => {
