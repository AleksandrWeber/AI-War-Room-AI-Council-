...
import type { UnicastizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_UNICASTIZABILITY_TABLES } from './unicastizability-rollout.helpers.js'

const WORKSPACE_UNICASTIZABILITY_DOMAINS: Array<{
  domain: UnicastizabilityAdminDomain
  tableName: string
...
@Injectable()
export class UnicastizabilityStatusService {
  ...
  async getUnicastizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_UNICASTIZABILITY_TABLES,
    )

    return {
      existingUnicastizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceUnicastizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_UNICASTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_UNICASTIZABILITY_DOMAINS.map(async (entry) => {
