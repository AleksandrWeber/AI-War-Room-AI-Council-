...
import type { MaterializabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_MATERIALIZABILITY_TABLES } from './materializability-rollout.helpers.js'

const WORKSPACE_MATERIALIZABILITY_DOMAINS: Array<{
  domain: MaterializabilityAdminDomain
  tableName: string
...
@Injectable()
export class MaterializabilityStatusService {
  ...
  async getMaterializabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_MATERIALIZABILITY_TABLES,
    )

    return {
      existingMaterializabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceMaterializabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_MATERIALIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_MATERIALIZABILITY_DOMAINS.map(async (entry) => {
