...
import type { DistinctivenessAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_DISTINCTIVENESS_TABLES } from './distinctiveness-rollout.helpers.js'

const WORKSPACE_DISTINCTIVENESS_DOMAINS: Array<{
  domain: DistinctivenessAdminDomain
  tableName: string
...
@Injectable()
export class DistinctivenessStatusService {
  ...
  async getDistinctivenessTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_DISTINCTIVENESS_TABLES,
    )

    return {
      existingDistinctivenessTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceDistinctivenessInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_DISTINCTIVENESS_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_DISTINCTIVENESS_DOMAINS.map(async (entry) => {
