...
import type { AuditregistryizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_AUDITREGISTRYIZABILITY_TABLES } from './auditregistryizability-rollout.helpers.js'

const WORKSPACE_AUDITREGISTRYIZABILITY_DOMAINS: Array<{
  domain: AuditregistryizabilityAdminDomain
  tableName: string
...
@Injectable()
export class AuditregistryizabilityStatusService {
  ...
  async getAuditregistryizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_AUDITREGISTRYIZABILITY_TABLES,
    )

    return {
      existingAuditregistryizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAuditregistryizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_AUDITREGISTRYIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_AUDITREGISTRYIZABILITY_DOMAINS.map(async (entry) => {
