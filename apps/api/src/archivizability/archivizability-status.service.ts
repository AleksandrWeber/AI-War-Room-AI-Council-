...
import type { ArchivizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ARCHIVIZABILITY_TABLES } from './archivizability-rollout.helpers.js'

const WORKSPACE_ARCHIVIZABILITY_DOMAINS: Array<{
  domain: ArchivizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ArchivizabilityStatusService {
  ...
  async getArchivizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ARCHIVIZABILITY_TABLES,
    )

    return {
      existingArchivizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceArchivizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ARCHIVIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ARCHIVIZABILITY_DOMAINS.map(async (entry) => {
