...
import type { ScalingizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_SCALINGIZABILITY_TABLES } from './scalingizability-rollout.helpers.js'

const WORKSPACE_SCALINGIZABILITY_DOMAINS: Array<{
  domain: ScalingizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ScalingizabilityStatusService {
  ...
  async getScalingizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_SCALINGIZABILITY_TABLES,
    )

    return {
      existingScalingizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceScalingizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_SCALINGIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_SCALINGIZABILITY_DOMAINS.map(async (entry) => {
