...
import type { TelemetryizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_TELEMETRYIZABILITY_TABLES } from './telemetryizability-rollout.helpers.js'

const WORKSPACE_TELEMETRYIZABILITY_DOMAINS: Array<{
  domain: TelemetryizabilityAdminDomain
  tableName: string
...
@Injectable()
export class TelemetryizabilityStatusService {
  ...
  async getTelemetryizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_TELEMETRYIZABILITY_TABLES,
    )

    return {
      existingTelemetryizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceTelemetryizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_TELEMETRYIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_TELEMETRYIZABILITY_DOMAINS.map(async (entry) => {
