...
import type { InventoryizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_INVENTORYIZABILITY_TABLES } from './inventoryizability-rollout.helpers.js'

const WORKSPACE_INVENTORYIZABILITY_DOMAINS: Array<{
  domain: InventoryizabilityAdminDomain
  tableName: string
...
@Injectable()
export class InventoryizabilityStatusService {
  ...
  async getInventoryizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_INVENTORYIZABILITY_TABLES,
    )

    return {
      existingInventoryizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceInventoryizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_INVENTORYIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_INVENTORYIZABILITY_DOMAINS.map(async (entry) => {
