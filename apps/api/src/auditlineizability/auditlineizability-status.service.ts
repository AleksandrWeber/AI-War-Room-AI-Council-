...
import type { AuditlineizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_AUDITLINEIZABILITY_TABLES } from './auditlineizability-rollout.helpers.js'

const WORKSPACE_AUDITLINEIZABILITY_DOMAINS: Array<{
  domain: AuditlineizabilityAdminDomain
  tableName: string
...
@Injectable()
export class AuditlineizabilityStatusService {
  ...
  async getAuditlineizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_AUDITLINEIZABILITY_TABLES,
    )

    return {
      existingAuditlineizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAuditlineizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_AUDITLINEIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_AUDITLINEIZABILITY_DOMAINS.map(async (entry) => {
