...
import type { TamperproofizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_TAMPERPROOFIZABILITY_TABLES } from './tamperproofizability-rollout.helpers.js'

const WORKSPACE_TAMPERPROOFIZABILITY_DOMAINS: Array<{
  domain: TamperproofizabilityAdminDomain
  tableName: string
...
@Injectable()
export class TamperproofizabilityStatusService {
  ...
  async getTamperproofizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_TAMPERPROOFIZABILITY_TABLES,
    )

    return {
      existingTamperproofizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceTamperproofizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_TAMPERPROOFIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_TAMPERPROOFIZABILITY_DOMAINS.map(async (entry) => {
