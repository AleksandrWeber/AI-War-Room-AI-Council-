...
import type { AuditingizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_AUDITINGIZABILITY_TABLES } from './auditingizability-rollout.helpers.js'

const WORKSPACE_AUDITINGIZABILITY_DOMAINS: Array<{
  domain: AuditingizabilityAdminDomain
  tableName: string
...
@Injectable()
export class AuditingizabilityStatusService {
  ...
  async getAuditingizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_AUDITINGIZABILITY_TABLES,
    )

    return {
      existingAuditingizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAuditingizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_AUDITINGIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_AUDITINGIZABILITY_DOMAINS.map(async (entry) => {
