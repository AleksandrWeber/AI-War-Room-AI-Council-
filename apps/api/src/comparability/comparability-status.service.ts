...
import type { ComparabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_COMPARABILITY_TABLES } from './comparability-rollout.helpers.js'

const WORKSPACE_COMPARABILITY_DOMAINS: Array<{
  domain: ComparabilityAdminDomain
  tableName: string
...
@Injectable()
export class ComparabilityStatusService {
  ...
  async getComparabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_COMPARABILITY_TABLES,
    )

    return {
      existingComparabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceComparabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_COMPARABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_COMPARABILITY_DOMAINS.map(async (entry) => {
