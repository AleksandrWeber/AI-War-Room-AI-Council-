...
import type { AudittrailizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_AUDITTRAILIZABILITY_TABLES } from './audittrailizability-rollout.helpers.js'

const WORKSPACE_AUDITTRAILIZABILITY_DOMAINS: Array<{
  domain: AudittrailizabilityAdminDomain
  tableName: string
...
@Injectable()
export class AudittrailizabilityStatusService {
  ...
  async getAudittrailizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_AUDITTRAILIZABILITY_TABLES,
    )

    return {
      existingAudittrailizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAudittrailizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_AUDITTRAILIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_AUDITTRAILIZABILITY_DOMAINS.map(async (entry) => {
