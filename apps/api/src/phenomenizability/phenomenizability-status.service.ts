...
import type { PhenomenizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_PHENOMENIZABILITY_TABLES } from './phenomenizability-rollout.helpers.js'

const WORKSPACE_PHENOMENIZABILITY_DOMAINS: Array<{
  domain: PhenomenizabilityAdminDomain
  tableName: string
...
@Injectable()
export class PhenomenizabilityStatusService {
  ...
  async getPhenomenizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_PHENOMENIZABILITY_TABLES,
    )

    return {
      existingPhenomenizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspacePhenomenizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_PHENOMENIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_PHENOMENIZABILITY_DOMAINS.map(async (entry) => {
