...
import type { CanaryizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CANARYIZABILITY_TABLES } from './canaryizability-rollout.helpers.js'

const WORKSPACE_CANARYIZABILITY_DOMAINS: Array<{
  domain: CanaryizabilityAdminDomain
  tableName: string
...
@Injectable()
export class CanaryizabilityStatusService {
  ...
  async getCanaryizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CANARYIZABILITY_TABLES,
    )

    return {
      existingCanaryizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceCanaryizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CANARYIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CANARYIZABILITY_DOMAINS.map(async (entry) => {
