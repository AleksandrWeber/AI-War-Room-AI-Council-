...
import type { TypifiabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_TYPIFIABILITY_TABLES } from './typifiability-rollout.helpers.js'

const WORKSPACE_TYPIFIABILITY_DOMAINS: Array<{
  domain: TypifiabilityAdminDomain
  tableName: string
...
@Injectable()
export class TypifiabilityStatusService {
  ...
  async getTypifiabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_TYPIFIABILITY_TABLES,
    )

    return {
      existingTypifiabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceTypifiabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_TYPIFIABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_TYPIFIABILITY_DOMAINS.map(async (entry) => {
