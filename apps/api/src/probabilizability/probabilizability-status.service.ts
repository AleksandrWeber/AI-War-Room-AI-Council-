...
import type { ProbabilizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_PROBABILIZABILITY_TABLES } from './probabilizability-rollout.helpers.js'

const WORKSPACE_PROBABILIZABILITY_DOMAINS: Array<{
  domain: ProbabilizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ProbabilizabilityStatusService {
  ...
  async getProbabilizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_PROBABILIZABILITY_TABLES,
    )

    return {
      existingProbabilizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceProbabilizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_PROBABILIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_PROBABILIZABILITY_DOMAINS.map(async (entry) => {
