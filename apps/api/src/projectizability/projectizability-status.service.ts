...
import type { ProjectizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_PROJECTIZABILITY_TABLES } from './projectizability-rollout.helpers.js'

const WORKSPACE_PROJECTIZABILITY_DOMAINS: Array<{
  domain: ProjectizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ProjectizabilityStatusService {
  ...
  async getProjectizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_PROJECTIZABILITY_TABLES,
    )

    return {
      existingProjectizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceProjectizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_PROJECTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_PROJECTIZABILITY_DOMAINS.map(async (entry) => {
