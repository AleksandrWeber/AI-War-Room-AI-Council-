...
import type { InferencizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_INFERENCIZABILITY_TABLES } from './inferencizability-rollout.helpers.js'

const WORKSPACE_INFERENCIZABILITY_DOMAINS: Array<{
  domain: InferencizabilityAdminDomain
  tableName: string
...
@Injectable()
export class InferencizabilityStatusService {
  ...
  async getInferencizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_INFERENCIZABILITY_TABLES,
    )

    return {
      existingInferencizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceInferencizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_INFERENCIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_INFERENCIZABILITY_DOMAINS.map(async (entry) => {
