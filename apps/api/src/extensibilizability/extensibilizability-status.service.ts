...
import type { ExtensibilizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_EXTENSIBILIZABILITY_TABLES } from './extensibilizability-rollout.helpers.js'

const WORKSPACE_EXTENSIBILIZABILITY_DOMAINS: Array<{
  domain: ExtensibilizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ExtensibilizabilityStatusService {
  ...
  async getExtensibilizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_EXTENSIBILIZABILITY_TABLES,
    )

    return {
      existingExtensibilizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceExtensibilizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_EXTENSIBILIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_EXTENSIBILIZABILITY_DOMAINS.map(async (entry) => {
