...
import type { ScriptabilizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_SCRIPTABILIZABILITY_TABLES } from './scriptabilizability-rollout.helpers.js'

const WORKSPACE_SCRIPTABILIZABILITY_DOMAINS: Array<{
  domain: ScriptabilizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ScriptabilizabilityStatusService {
  ...
  async getScriptabilizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_SCRIPTABILIZABILITY_TABLES,
    )

    return {
      existingScriptabilizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceScriptabilizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_SCRIPTABILIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_SCRIPTABILIZABILITY_DOMAINS.map(async (entry) => {
