...
import type { SplitizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_SPLITIZABILITY_TABLES } from './splitizability-rollout.helpers.js'

const WORKSPACE_SPLITIZABILITY_DOMAINS: Array<{
  domain: SplitizabilityAdminDomain
  tableName: string
...
@Injectable()
export class SplitizabilityStatusService {
  ...
  async getSplitizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_SPLITIZABILITY_TABLES,
    )

    return {
      existingSplitizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceSplitizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_SPLITIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_SPLITIZABILITY_DOMAINS.map(async (entry) => {
