...
import type { LineageizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_LINEAGEIZABILITY_TABLES } from './lineageizability-rollout.helpers.js'

const WORKSPACE_LINEAGEIZABILITY_DOMAINS: Array<{
  domain: LineageizabilityAdminDomain
  tableName: string
...
@Injectable()
export class LineageizabilityStatusService {
  ...
  async getLineageizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_LINEAGEIZABILITY_TABLES,
    )

    return {
      existingLineageizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceLineageizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_LINEAGEIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_LINEAGEIZABILITY_DOMAINS.map(async (entry) => {
