...
import type { StratifiabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_STRATIFIABILITY_TABLES } from './stratifiability-rollout.helpers.js'

const WORKSPACE_STRATIFIABILITY_DOMAINS: Array<{
  domain: StratifiabilityAdminDomain
  tableName: string
...
@Injectable()
export class StratifiabilityStatusService {
  ...
  async getStratifiabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_STRATIFIABILITY_TABLES,
    )

    return {
      existingStratifiabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceStratifiabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_STRATIFIABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_STRATIFIABILITY_DOMAINS.map(async (entry) => {
