...
import type { IconizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ICONIZABILITY_TABLES } from './iconizability-rollout.helpers.js'

const WORKSPACE_ICONIZABILITY_DOMAINS: Array<{
  domain: IconizabilityAdminDomain
  tableName: string
...
@Injectable()
export class IconizabilityStatusService {
  ...
  async getIconizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ICONIZABILITY_TABLES,
    )

    return {
      existingIconizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceIconizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ICONIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ICONIZABILITY_DOMAINS.map(async (entry) => {
