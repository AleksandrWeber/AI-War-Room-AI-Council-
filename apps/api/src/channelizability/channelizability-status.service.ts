...
import type { ChannelizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CHANNELIZABILITY_TABLES } from './channelizability-rollout.helpers.js'

const WORKSPACE_CHANNELIZABILITY_DOMAINS: Array<{
  domain: ChannelizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ChannelizabilityStatusService {
  ...
  async getChannelizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CHANNELIZABILITY_TABLES,
    )

    return {
      existingChannelizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceChannelizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CHANNELIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CHANNELIZABILITY_DOMAINS.map(async (entry) => {
