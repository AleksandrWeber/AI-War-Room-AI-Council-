...
import type { OperabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_OPERABILITY_TABLES } from './operability-rollout.helpers.js'

const WORKSPACE_OPERABILITY_DOMAINS: Array<{
  domain: OperabilityAdminDomain
  tableName: string
...
@Injectable()
export class OperabilityStatusService {
  ...
  async getOperabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_OPERABILITY_TABLES,
    )

    return {
      existingOperabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceOperabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_OPERABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_OPERABILITY_DOMAINS.map(async (entry) => {
