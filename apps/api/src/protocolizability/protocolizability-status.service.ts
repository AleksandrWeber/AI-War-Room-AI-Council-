...
import type { ProtocolizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_PROTOCOLIZABILITY_TABLES } from './protocolizability-rollout.helpers.js'

const WORKSPACE_PROTOCOLIZABILITY_DOMAINS: Array<{
  domain: ProtocolizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ProtocolizabilityStatusService {
  ...
  async getProtocolizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_PROTOCOLIZABILITY_TABLES,
    )

    return {
      existingProtocolizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceProtocolizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_PROTOCOLIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_PROTOCOLIZABILITY_DOMAINS.map(async (entry) => {
