...
import type { DramatizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_DRAMATIZABILITY_TABLES } from './dramatizability-rollout.helpers.js'

const WORKSPACE_DRAMATIZABILITY_DOMAINS: Array<{
  domain: DramatizabilityAdminDomain
  tableName: string
...
@Injectable()
export class DramatizabilityStatusService {
  ...
  async getDramatizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_DRAMATIZABILITY_TABLES,
    )

    return {
      existingDramatizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceDramatizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_DRAMATIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_DRAMATIZABILITY_DOMAINS.map(async (entry) => {
