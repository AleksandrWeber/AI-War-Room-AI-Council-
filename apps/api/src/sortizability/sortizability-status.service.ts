...
import type { SortizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_SORTIZABILITY_TABLES } from './sortizability-rollout.helpers.js'

const WORKSPACE_SORTIZABILITY_DOMAINS: Array<{
  domain: SortizabilityAdminDomain
  tableName: string
...
@Injectable()
export class SortizabilityStatusService {
  ...
  async getSortizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_SORTIZABILITY_TABLES,
    )

    return {
      existingSortizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceSortizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_SORTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_SORTIZABILITY_DOMAINS.map(async (entry) => {
