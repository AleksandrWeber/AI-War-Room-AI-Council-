...
import type { JournalizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_JOURNALIZABILITY_TABLES } from './journalizability-rollout.helpers.js'

const WORKSPACE_JOURNALIZABILITY_DOMAINS: Array<{
  domain: JournalizabilityAdminDomain
  tableName: string
...
@Injectable()
export class JournalizabilityStatusService {
  ...
  async getJournalizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_JOURNALIZABILITY_TABLES,
    )

    return {
      existingJournalizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceJournalizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_JOURNALIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_JOURNALIZABILITY_DOMAINS.map(async (entry) => {
