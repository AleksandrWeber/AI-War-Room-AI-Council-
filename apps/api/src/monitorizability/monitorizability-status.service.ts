...
import type { MonitorizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_MONITORIZABILITY_TABLES } from './monitorizability-rollout.helpers.js'

const WORKSPACE_MONITORIZABILITY_DOMAINS: Array<{
  domain: MonitorizabilityAdminDomain
  tableName: string
...
@Injectable()
export class MonitorizabilityStatusService {
  ...
  async getMonitorizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_MONITORIZABILITY_TABLES,
    )

    return {
      existingMonitorizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceMonitorizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_MONITORIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_MONITORIZABILITY_DOMAINS.map(async (entry) => {
