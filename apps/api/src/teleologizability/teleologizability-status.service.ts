...
import type { TeleologizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_TELEOLOGIZABILITY_TABLES } from './teleologizability-rollout.helpers.js'

const WORKSPACE_TELEOLOGIZABILITY_DOMAINS: Array<{
  domain: TeleologizabilityAdminDomain
  tableName: string
...
@Injectable()
export class TeleologizabilityStatusService {
  ...
  async getTeleologizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_TELEOLOGIZABILITY_TABLES,
    )

    return {
      existingTeleologizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceTeleologizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_TELEOLOGIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_TELEOLOGIZABILITY_DOMAINS.map(async (entry) => {
