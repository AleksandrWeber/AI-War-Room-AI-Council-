...
import type { TunabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_TUNABILITY_TABLES } from './tunability-rollout.helpers.js'

const WORKSPACE_TUNABILITY_DOMAINS: Array<{
  domain: TunabilityAdminDomain
  tableName: string
...
@Injectable()
export class TunabilityStatusService {
  ...
  async getTunabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_TUNABILITY_TABLES,
    )

    return {
      existingTunabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceTunabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_TUNABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_TUNABILITY_DOMAINS.map(async (entry) => {
