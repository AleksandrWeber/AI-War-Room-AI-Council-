...
import type { TeachabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_TEACHABILITY_TABLES } from './teachability-rollout.helpers.js'

const WORKSPACE_TEACHABILITY_DOMAINS: Array<{
  domain: TeachabilityAdminDomain
  tableName: string
...
@Injectable()
export class TeachabilityStatusService {
  ...
  async getTeachabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_TEACHABILITY_TABLES,
    )

    return {
      existingTeachabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceTeachabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_TEACHABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_TEACHABILITY_DOMAINS.map(async (entry) => {
