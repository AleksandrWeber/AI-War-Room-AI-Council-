...
import type { FalsifiizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_FALSIFIIZABILITY_TABLES } from './falsifiizability-rollout.helpers.js'

const WORKSPACE_FALSIFIIZABILITY_DOMAINS: Array<{
  domain: FalsifiizabilityAdminDomain
  tableName: string
...
@Injectable()
export class FalsifiizabilityStatusService {
  ...
  async getFalsifiizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_FALSIFIIZABILITY_TABLES,
    )

    return {
      existingFalsifiizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceFalsifiizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_FALSIFIIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_FALSIFIIZABILITY_DOMAINS.map(async (entry) => {
