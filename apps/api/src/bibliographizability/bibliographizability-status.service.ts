...
import type { BibliographizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_BIBLIOGRAPHIZABILITY_TABLES } from './bibliographizability-rollout.helpers.js'

const WORKSPACE_BIBLIOGRAPHIZABILITY_DOMAINS: Array<{
  domain: BibliographizabilityAdminDomain
  tableName: string
...
@Injectable()
export class BibliographizabilityStatusService {
  ...
  async getBibliographizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_BIBLIOGRAPHIZABILITY_TABLES,
    )

    return {
      existingBibliographizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceBibliographizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_BIBLIOGRAPHIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_BIBLIOGRAPHIZABILITY_DOMAINS.map(async (entry) => {
