...
import type { EvidencevaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_EVIDENCEVAULTIZABILITY_TABLES } from './evidencevaultizability-rollout.helpers.js'

const WORKSPACE_EVIDENCEVAULTIZABILITY_DOMAINS: Array<{
  domain: EvidencevaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class EvidencevaultizabilityStatusService {
  ...
  async getEvidencevaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_EVIDENCEVAULTIZABILITY_TABLES,
    )

    return {
      existingEvidencevaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceEvidencevaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_EVIDENCEVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_EVIDENCEVAULTIZABILITY_DOMAINS.map(async (entry) => {
