...
import type { OrchestrizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ORCHESTRIZABILITY_TABLES } from './orchestrizability-rollout.helpers.js'

const WORKSPACE_ORCHESTRIZABILITY_DOMAINS: Array<{
  domain: OrchestrizabilityAdminDomain
  tableName: string
...
@Injectable()
export class OrchestrizabilityStatusService {
  ...
  async getOrchestrizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ORCHESTRIZABILITY_TABLES,
    )

    return {
      existingOrchestrizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceOrchestrizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ORCHESTRIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ORCHESTRIZABILITY_DOMAINS.map(async (entry) => {
