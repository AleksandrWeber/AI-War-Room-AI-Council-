...
import type { WalizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_WALIZABILITY_TABLES } from './walizability-rollout.helpers.js'

const WORKSPACE_WALIZABILITY_DOMAINS: Array<{
  domain: WalizabilityAdminDomain
  tableName: string
...
@Injectable()
export class WalizabilityStatusService {
  ...
  async getWalizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_WALIZABILITY_TABLES,
    )

    return {
      existingWalizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceWalizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_WALIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_WALIZABILITY_DOMAINS.map(async (entry) => {
