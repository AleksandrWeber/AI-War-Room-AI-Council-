...
import type { MonitorabilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_MONITORABILITYVAULTIZABILITY_TABLES } from './monitorabilityvaultizability-rollout.helpers.js'

const WORKSPACE_MONITORABILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: MonitorabilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class MonitorabilityvaultizabilityStatusService {
  ...
  async getMonitorabilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_MONITORABILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingMonitorabilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceMonitorabilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_MONITORABILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_MONITORABILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
