...
import type { AutomatizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_AUTOMATIZABILITY_TABLES } from './automatizability-rollout.helpers.js'

const WORKSPACE_AUTOMATIZABILITY_DOMAINS: Array<{
  domain: AutomatizabilityAdminDomain
  tableName: string
...
@Injectable()
export class AutomatizabilityStatusService {
  ...
  async getAutomatizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_AUTOMATIZABILITY_TABLES,
    )

    return {
      existingAutomatizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAutomatizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_AUTOMATIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_AUTOMATIZABILITY_DOMAINS.map(async (entry) => {
