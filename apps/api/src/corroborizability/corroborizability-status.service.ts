...
import type { CorroborizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CORROBORIZABILITY_TABLES } from './corroborizability-rollout.helpers.js'

const WORKSPACE_CORROBORIZABILITY_DOMAINS: Array<{
  domain: CorroborizabilityAdminDomain
  tableName: string
...
@Injectable()
export class CorroborizabilityStatusService {
  ...
  async getCorroborizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CORROBORIZABILITY_TABLES,
    )

    return {
      existingCorroborizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceCorroborizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CORROBORIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CORROBORIZABILITY_DOMAINS.map(async (entry) => {
