...
import type { TraceabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_TRACEABILITY_TABLES } from './traceability-rollout.helpers.js'

const WORKSPACE_TRACEABILITY_DOMAINS: Array<{
  domain: TraceabilityAdminDomain
  tableName: string
...
@Injectable()
export class TraceabilityStatusService {
  ...
  async getTraceabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_TRACEABILITY_TABLES,
    )

    return {
      existingTraceabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceTraceabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_TRACEABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_TRACEABILITY_DOMAINS.map(async (entry) => {
