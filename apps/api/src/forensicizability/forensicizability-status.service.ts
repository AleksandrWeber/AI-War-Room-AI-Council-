...
import type { ForensicizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_FORENSICIZABILITY_TABLES } from './forensicizability-rollout.helpers.js'

const WORKSPACE_FORENSICIZABILITY_DOMAINS: Array<{
  domain: ForensicizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ForensicizabilityStatusService {
  ...
  async getForensicizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_FORENSICIZABILITY_TABLES,
    )

    return {
      existingForensicizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceForensicizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_FORENSICIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_FORENSICIZABILITY_DOMAINS.map(async (entry) => {
