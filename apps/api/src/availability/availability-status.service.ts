...
import type { AvailabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_AVAILABILITY_TABLES } from './availability-rollout.helpers.js'

const WORKSPACE_AVAILABILITY_DOMAINS: Array<{
  domain: AvailabilityAdminDomain
  tableName: string
...
export class AvailabilityStatusService {
  ...
  async getAvailabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_AVAILABILITY_TABLES,
    )

    return {
      existingAvailabilityTableCount: existingTables.size,
      existingTables,
    }
  }

  async getWorkspaceAvailabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_AVAILABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_AVAILABILITY_DOMAINS.map(async (entry) => {
