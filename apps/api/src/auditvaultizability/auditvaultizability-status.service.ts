...
import type { AuditvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_AUDITVAULTIZABILITY_TABLES } from './auditvaultizability-rollout.helpers.js'

const WORKSPACE_AUDITVAULTIZABILITY_DOMAINS: Array<{
  domain: AuditvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class AuditvaultizabilityStatusService {
  ...
  async getAuditvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_AUDITVAULTIZABILITY_TABLES,
    )

    return {
      existingAuditvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAuditvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_AUDITVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_AUDITVAULTIZABILITY_DOMAINS.map(async (entry) => {
