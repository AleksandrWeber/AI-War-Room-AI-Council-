...
import type { AttestledgerizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ATTESTLEDGERIZABILITY_TABLES } from './attestledgerizability-rollout.helpers.js'

const WORKSPACE_ATTESTLEDGERIZABILITY_DOMAINS: Array<{
  domain: AttestledgerizabilityAdminDomain
  tableName: string
...
@Injectable()
export class AttestledgerizabilityStatusService {
  ...
  async getAttestledgerizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ATTESTLEDGERIZABILITY_TABLES,
    )

    return {
      existingAttestledgerizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAttestledgerizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ATTESTLEDGERIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ATTESTLEDGERIZABILITY_DOMAINS.map(async (entry) => {
