...
import type { IntegrityizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_INTEGRITYIZABILITY_TABLES } from './integrityizability-rollout.helpers.js'

const WORKSPACE_INTEGRITYIZABILITY_DOMAINS: Array<{
  domain: IntegrityizabilityAdminDomain
  tableName: string
...
@Injectable()
export class IntegrityizabilityStatusService {
  ...
  async getIntegrityizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_INTEGRITYIZABILITY_TABLES,
    )

    return {
      existingIntegrityizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceIntegrityizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_INTEGRITYIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_INTEGRITYIZABILITY_DOMAINS.map(async (entry) => {
