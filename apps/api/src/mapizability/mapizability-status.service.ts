...
import type { MapizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_MAPIZABILITY_TABLES } from './mapizability-rollout.helpers.js'

const WORKSPACE_MAPIZABILITY_DOMAINS: Array<{
  domain: MapizabilityAdminDomain
  tableName: string
...
@Injectable()
export class MapizabilityStatusService {
  ...
  async getMapizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_MAPIZABILITY_TABLES,
    )

    return {
      existingMapizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceMapizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_MAPIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_MAPIZABILITY_DOMAINS.map(async (entry) => {
