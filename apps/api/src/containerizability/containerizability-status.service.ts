...
import type { ContainerizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CONTAINERIZABILITY_TABLES } from './containerizability-rollout.helpers.js'

const WORKSPACE_CONTAINERIZABILITY_DOMAINS: Array<{
  domain: ContainerizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ContainerizabilityStatusService {
  ...
  async getContainerizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CONTAINERIZABILITY_TABLES,
    )

    return {
      existingContainerizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceContainerizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CONTAINERIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CONTAINERIZABILITY_DOMAINS.map(async (entry) => {
