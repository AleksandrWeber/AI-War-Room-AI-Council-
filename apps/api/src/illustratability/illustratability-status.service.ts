...
import type { IllustratabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ILLUSTRATABILITY_TABLES } from './illustratability-rollout.helpers.js'

const WORKSPACE_ILLUSTRATABILITY_DOMAINS: Array<{
  domain: IllustratabilityAdminDomain
  tableName: string
...
@Injectable()
export class IllustratabilityStatusService {
  ...
  async getIllustratabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ILLUSTRATABILITY_TABLES,
    )

    return {
      existingIllustratabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceIllustratabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ILLUSTRATABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ILLUSTRATABILITY_DOMAINS.map(async (entry) => {
