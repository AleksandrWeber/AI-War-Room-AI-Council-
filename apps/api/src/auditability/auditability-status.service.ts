...
import type { AuditabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_AUDITABILITY_TABLES } from './auditability-rollout.helpers.js'

const WORKSPACE_AUDITABILITY_DOMAINS: Array<{
  domain: AuditabilityAdminDomain
  tableName: string
...
@Injectable()
export class AuditabilityStatusService {
  ...
  async getAuditabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_AUDITABILITY_TABLES,
    )

    return {
      existingAuditabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAuditabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_AUDITABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_AUDITABILITY_DOMAINS.map(async (entry) => {
