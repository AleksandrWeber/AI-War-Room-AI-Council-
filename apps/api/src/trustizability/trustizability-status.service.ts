...
import type { TrustizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_TRUSTIZABILITY_TABLES } from './trustizability-rollout.helpers.js'

const WORKSPACE_TRUSTIZABILITY_DOMAINS: Array<{
  domain: TrustizabilityAdminDomain
  tableName: string
...
@Injectable()
export class TrustizabilityStatusService {
  ...
  async getTrustizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_TRUSTIZABILITY_TABLES,
    )

    return {
      existingTrustizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceTrustizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_TRUSTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_TRUSTIZABILITY_DOMAINS.map(async (entry) => {
