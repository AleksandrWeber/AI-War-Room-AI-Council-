...
import type { RepeatabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_REPEATABILITY_TABLES } from './repeatability-rollout.helpers.js'

const WORKSPACE_REPEATABILITY_DOMAINS: Array<{
  domain: RepeatabilityAdminDomain
  tableName: string
...
@Injectable()
export class RepeatabilityStatusService {
  ...
  async getRepeatabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_REPEATABILITY_TABLES,
    )

    return {
      existingRepeatabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceRepeatabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_REPEATABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_REPEATABILITY_DOMAINS.map(async (entry) => {
