...
import type { HealingizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_HEALINGIZABILITY_TABLES } from './healingizability-rollout.helpers.js'

const WORKSPACE_HEALINGIZABILITY_DOMAINS: Array<{
  domain: HealingizabilityAdminDomain
  tableName: string
...
@Injectable()
export class HealingizabilityStatusService {
  ...
  async getHealingizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_HEALINGIZABILITY_TABLES,
    )

    return {
      existingHealingizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceHealingizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_HEALINGIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_HEALINGIZABILITY_DOMAINS.map(async (entry) => {
