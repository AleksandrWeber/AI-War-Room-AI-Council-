...
import type { AuthenticityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_AUTHENTICITYVAULTIZABILITY_TABLES } from './authenticityvaultizability-rollout.helpers.js'

const WORKSPACE_AUTHENTICITYVAULTIZABILITY_DOMAINS: Array<{
  domain: AuthenticityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class AuthenticityvaultizabilityStatusService {
  ...
  async getAuthenticityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_AUTHENTICITYVAULTIZABILITY_TABLES,
    )

    return {
      existingAuthenticityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAuthenticityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_AUTHENTICITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_AUTHENTICITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
