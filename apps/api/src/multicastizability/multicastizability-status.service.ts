...
import type { MulticastizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_MULTICASTIZABILITY_TABLES } from './multicastizability-rollout.helpers.js'

const WORKSPACE_MULTICASTIZABILITY_DOMAINS: Array<{
  domain: MulticastizabilityAdminDomain
  tableName: string
...
@Injectable()
export class MulticastizabilityStatusService {
  ...
  async getMulticastizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_MULTICASTIZABILITY_TABLES,
    )

    return {
      existingMulticastizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceMulticastizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_MULTICASTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_MULTICASTIZABILITY_DOMAINS.map(async (entry) => {
