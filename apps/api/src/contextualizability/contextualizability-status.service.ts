...
import type { ContextualizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CONTEXTUALIZABILITY_TABLES } from './contextualizability-rollout.helpers.js'

const WORKSPACE_CONTEXTUALIZABILITY_DOMAINS: Array<{
  domain: ContextualizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ContextualizabilityStatusService {
  ...
  async getContextualizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CONTEXTUALIZABILITY_TABLES,
    )

    return {
      existingContextualizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceContextualizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CONTEXTUALIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CONTEXTUALIZABILITY_DOMAINS.map(async (entry) => {
