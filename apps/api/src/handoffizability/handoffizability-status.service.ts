...
import type { HandoffizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_HANDOFFIZABILITY_TABLES } from './handoffizability-rollout.helpers.js'

const WORKSPACE_HANDOFFIZABILITY_DOMAINS: Array<{
  domain: HandoffizabilityAdminDomain
  tableName: string
...
@Injectable()
export class HandoffizabilityStatusService {
  ...
  async getHandoffizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_HANDOFFIZABILITY_TABLES,
    )

    return {
      existingHandoffizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceHandoffizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_HANDOFFIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_HANDOFFIZABILITY_DOMAINS.map(async (entry) => {
