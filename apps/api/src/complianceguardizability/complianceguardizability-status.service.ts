...
import type { ComplianceguardizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_COMPLIANCEGUARDIZABILITY_TABLES } from './complianceguardizability-rollout.helpers.js'

const WORKSPACE_COMPLIANCEGUARDIZABILITY_DOMAINS: Array<{
  domain: ComplianceguardizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ComplianceguardizabilityStatusService {
  ...
  async getComplianceguardizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_COMPLIANCEGUARDIZABILITY_TABLES,
    )

    return {
      existingComplianceguardizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceComplianceguardizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_COMPLIANCEGUARDIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_COMPLIANCEGUARDIZABILITY_DOMAINS.map(async (entry) => {
