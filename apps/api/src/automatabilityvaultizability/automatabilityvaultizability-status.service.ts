...
import type { AutomatabilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_AUTOMATABILITYVAULTIZABILITY_TABLES } from './automatabilityvaultizability-rollout.helpers.js'

const WORKSPACE_AUTOMATABILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: AutomatabilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class AutomatabilityvaultizabilityStatusService {
  ...
  async getAutomatabilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_AUTOMATABILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingAutomatabilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAutomatabilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_AUTOMATABILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_AUTOMATABILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
