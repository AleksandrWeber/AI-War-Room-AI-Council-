...
import type { SecurityizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_SECURITYIZABILITY_TABLES } from './securityizability-rollout.helpers.js'

const WORKSPACE_SECURITYIZABILITY_DOMAINS: Array<{
  domain: SecurityizabilityAdminDomain
  tableName: string
...
@Injectable()
export class SecurityizabilityStatusService {
  ...
  async getSecurityizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_SECURITYIZABILITY_TABLES,
    )

    return {
      existingSecurityizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceSecurityizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_SECURITYIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_SECURITYIZABILITY_DOMAINS.map(async (entry) => {
