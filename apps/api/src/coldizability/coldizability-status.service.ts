...
import type { ColdizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_COLDIZABILITY_TABLES } from './coldizability-rollout.helpers.js'

const WORKSPACE_COLDIZABILITY_DOMAINS: Array<{
  domain: ColdizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ColdizabilityStatusService {
  ...
  async getColdizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_COLDIZABILITY_TABLES,
    )

    return {
      existingColdizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceColdizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_COLDIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_COLDIZABILITY_DOMAINS.map(async (entry) => {
