...
import type { ConnotabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CONNOTABILITY_TABLES } from './connotability-rollout.helpers.js'

const WORKSPACE_CONNOTABILITY_DOMAINS: Array<{
  domain: ConnotabilityAdminDomain
  tableName: string
...
@Injectable()
export class ConnotabilityStatusService {
  ...
  async getConnotabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CONNOTABILITY_TABLES,
    )

    return {
      existingConnotabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceConnotabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CONNOTABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CONNOTABILITY_DOMAINS.map(async (entry) => {
