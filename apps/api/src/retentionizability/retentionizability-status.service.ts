...
import type { RetentionizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_RETENTIONIZABILITY_TABLES } from './retentionizability-rollout.helpers.js'

const WORKSPACE_RETENTIONIZABILITY_DOMAINS: Array<{
  domain: RetentionizabilityAdminDomain
  tableName: string
...
@Injectable()
export class RetentionizabilityStatusService {
  ...
  async getRetentionizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_RETENTIONIZABILITY_TABLES,
    )

    return {
      existingRetentionizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceRetentionizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_RETENTIONIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_RETENTIONIZABILITY_DOMAINS.map(async (entry) => {
