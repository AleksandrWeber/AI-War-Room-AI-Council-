...
import type { MemorabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_MEMORABILITY_TABLES } from './memorability-rollout.helpers.js'

const WORKSPACE_MEMORABILITY_DOMAINS: Array<{
  domain: MemorabilityAdminDomain
  tableName: string
...
@Injectable()
export class MemorabilityStatusService {
  ...
  async getMemorabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_MEMORABILITY_TABLES,
    )

    return {
      existingMemorabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceMemorabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_MEMORABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_MEMORABILITY_DOMAINS.map(async (entry) => {
