...
import type { RegistryjournalizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_REGISTRYJOURNALIZABILITY_TABLES } from './registryjournalizability-rollout.helpers.js'

const WORKSPACE_REGISTRYJOURNALIZABILITY_DOMAINS: Array<{
  domain: RegistryjournalizabilityAdminDomain
  tableName: string
...
@Injectable()
export class RegistryjournalizabilityStatusService {
  ...
  async getRegistryjournalizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_REGISTRYJOURNALIZABILITY_TABLES,
    )

    return {
      existingRegistryjournalizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceRegistryjournalizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_REGISTRYJOURNALIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_REGISTRYJOURNALIZABILITY_DOMAINS.map(async (entry) => {
