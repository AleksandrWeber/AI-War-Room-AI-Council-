...
import type { ImportizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_IMPORTIZABILITY_TABLES } from './importizability-rollout.helpers.js'

const WORKSPACE_IMPORTIZABILITY_DOMAINS: Array<{
  domain: ImportizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ImportizabilityStatusService {
  ...
  async getImportizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_IMPORTIZABILITY_TABLES,
    )

    return {
      existingImportizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceImportizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_IMPORTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_IMPORTIZABILITY_DOMAINS.map(async (entry) => {
