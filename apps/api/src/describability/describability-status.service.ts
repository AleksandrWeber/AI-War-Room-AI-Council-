...
import type { DescribabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_DESCRIBABILITY_TABLES } from './describability-rollout.helpers.js'

const WORKSPACE_DESCRIBABILITY_DOMAINS: Array<{
  domain: DescribabilityAdminDomain
  tableName: string
...
@Injectable()
export class DescribabilityStatusService {
  ...
  async getDescribabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_DESCRIBABILITY_TABLES,
    )

    return {
      existingDescribabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceDescribabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_DESCRIBABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_DESCRIBABILITY_DOMAINS.map(async (entry) => {
