...
import type { ReconciliationizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_RECONCILIATIONIZABILITY_TABLES } from './reconciliationizability-rollout.helpers.js'

const WORKSPACE_RECONCILIATIONIZABILITY_DOMAINS: Array<{
  domain: ReconciliationizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ReconciliationizabilityStatusService {
  ...
  async getReconciliationizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_RECONCILIATIONIZABILITY_TABLES,
    )

    return {
      existingReconciliationizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceReconciliationizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_RECONCILIATIONIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_RECONCILIATIONIZABILITY_DOMAINS.map(async (entry) => {
