...
import type { CertificationizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CERTIFICATIONIZABILITY_TABLES } from './certificationizability-rollout.helpers.js'

const WORKSPACE_CERTIFICATIONIZABILITY_DOMAINS: Array<{
  domain: CertificationizabilityAdminDomain
  tableName: string
...
@Injectable()
export class CertificationizabilityStatusService {
  ...
  async getCertificationizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CERTIFICATIONIZABILITY_TABLES,
    )

    return {
      existingCertificationizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceCertificationizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CERTIFICATIONIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CERTIFICATIONIZABILITY_DOMAINS.map(async (entry) => {
