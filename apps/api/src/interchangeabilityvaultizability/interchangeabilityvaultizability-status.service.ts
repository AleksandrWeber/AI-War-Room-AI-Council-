...
import type { InterchangeabilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_INTERCHANGEABILITYVAULTIZABILITY_TABLES } from './interchangeabilityvaultizability-rollout.helpers.js'

const WORKSPACE_INTERCHANGEABILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: InterchangeabilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class InterchangeabilityvaultizabilityStatusService {
  ...
  async getInterchangeabilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_INTERCHANGEABILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingInterchangeabilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceInterchangeabilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_INTERCHANGEABILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_INTERCHANGEABILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
