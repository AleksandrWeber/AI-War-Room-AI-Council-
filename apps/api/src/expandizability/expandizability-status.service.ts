...
import type { ExpandizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_DECOMPACTIZABILITY_TABLES } from './expandizability-rollout.helpers.js'

const WORKSPACE_DECOMPACTIZABILITY_DOMAINS: Array<{
  domain: ExpandizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ExpandizabilityStatusService {
  ...
  async getExpandizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_DECOMPACTIZABILITY_TABLES,
    )

    return {
      existingExpandizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceExpandizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_DECOMPACTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_DECOMPACTIZABILITY_DOMAINS.map(async (entry) => {
