...
import type { TrustworthinessvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_TRUSTWORTHINESSVAULTIZABILITY_TABLES } from './trustworthinessvaultizability-rollout.helpers.js'

const WORKSPACE_TRUSTWORTHINESSVAULTIZABILITY_DOMAINS: Array<{
  domain: TrustworthinessvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class TrustworthinessvaultizabilityStatusService {
  ...
  async getTrustworthinessvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_TRUSTWORTHINESSVAULTIZABILITY_TABLES,
    )

    return {
      existingTrustworthinessvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceTrustworthinessvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_TRUSTWORTHINESSVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_TRUSTWORTHINESSVAULTIZABILITY_DOMAINS.map(async (entry) => {
