...
import type { WitnessproofizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_WITNESSPROOFIZABILITY_TABLES } from './witnessproofizability-rollout.helpers.js'

const WORKSPACE_WITNESSPROOFIZABILITY_DOMAINS: Array<{
  domain: WitnessproofizabilityAdminDomain
  tableName: string
...
@Injectable()
export class WitnessproofizabilityStatusService {
  ...
  async getWitnessproofizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_WITNESSPROOFIZABILITY_TABLES,
    )

    return {
      existingWitnessproofizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceWitnessproofizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_WITNESSPROOFIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_WITNESSPROOFIZABILITY_DOMAINS.map(async (entry) => {
