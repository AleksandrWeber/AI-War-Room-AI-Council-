...
import type { RegistrationizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_REGISTRATIONIZABILITY_TABLES } from './registrationizability-rollout.helpers.js'

const WORKSPACE_REGISTRATIONIZABILITY_DOMAINS: Array<{
  domain: RegistrationizabilityAdminDomain
  tableName: string
...
@Injectable()
export class RegistrationizabilityStatusService {
  ...
  async getRegistrationizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_REGISTRATIONIZABILITY_TABLES,
    )

    return {
      existingRegistrationizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceRegistrationizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_REGISTRATIONIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_REGISTRATIONIZABILITY_DOMAINS.map(async (entry) => {
