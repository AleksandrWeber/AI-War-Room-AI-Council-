...
import type { MitigationizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_MITIGATIONIZABILITY_TABLES } from './mitigationizability-rollout.helpers.js'

const WORKSPACE_MITIGATIONIZABILITY_DOMAINS: Array<{
  domain: MitigationizabilityAdminDomain
  tableName: string
...
@Injectable()
export class MitigationizabilityStatusService {
  ...
  async getMitigationizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_MITIGATIONIZABILITY_TABLES,
    )

    return {
      existingMitigationizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceMitigationizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_MITIGATIONIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_MITIGATIONIZABILITY_DOMAINS.map(async (entry) => {
