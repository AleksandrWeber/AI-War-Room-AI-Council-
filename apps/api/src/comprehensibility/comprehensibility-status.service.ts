...
import type { ComprehensibilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_COMPREHENSIBILITY_TABLES } from './comprehensibility-rollout.helpers.js'

const WORKSPACE_COMPREHENSIBILITY_DOMAINS: Array<{
  domain: ComprehensibilityAdminDomain
  tableName: string
...
@Injectable()
export class ComprehensibilityStatusService {
  ...
  async getComprehensibilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_COMPREHENSIBILITY_TABLES,
    )

    return {
      existingComprehensibilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceComprehensibilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_COMPREHENSIBILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_COMPREHENSIBILITY_DOMAINS.map(async (entry) => {
