...
import type { SegregationizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_SEGREGATIONIZABILITY_TABLES } from './segregationizability-rollout.helpers.js'

const WORKSPACE_SEGREGATIONIZABILITY_DOMAINS: Array<{
  domain: SegregationizabilityAdminDomain
  tableName: string
...
@Injectable()
export class SegregationizabilityStatusService {
  ...
  async getSegregationizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_SEGREGATIONIZABILITY_TABLES,
    )

    return {
      existingSegregationizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceSegregationizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_SEGREGATIONIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_SEGREGATIONIZABILITY_DOMAINS.map(async (entry) => {
