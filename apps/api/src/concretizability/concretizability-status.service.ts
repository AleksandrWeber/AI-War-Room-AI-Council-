...
import type { ConcretizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CONCRETIZABILITY_TABLES } from './concretizability-rollout.helpers.js'

const WORKSPACE_CONCRETIZABILITY_DOMAINS: Array<{
  domain: ConcretizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ConcretizabilityStatusService {
  ...
  async getConcretizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CONCRETIZABILITY_TABLES,
    )

    return {
      existingConcretizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceConcretizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CONCRETIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CONCRETIZABILITY_DOMAINS.map(async (entry) => {
