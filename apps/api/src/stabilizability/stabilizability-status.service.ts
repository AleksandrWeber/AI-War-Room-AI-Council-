...
import type { StabilizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_STABILIZABILITY_TABLES } from './stabilizability-rollout.helpers.js'

const WORKSPACE_STABILIZABILITY_DOMAINS: Array<{
  domain: StabilizabilityAdminDomain
  tableName: string
...
@Injectable()
export class StabilizabilityStatusService {
  ...
  async getStabilizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_STABILIZABILITY_TABLES,
    )

    return {
      existingStabilizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceStabilizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_STABILIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_STABILIZABILITY_DOMAINS.map(async (entry) => {
