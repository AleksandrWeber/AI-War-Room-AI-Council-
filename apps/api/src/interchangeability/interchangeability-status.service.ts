...
import type { InterchangeabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_INTERCHANGEABILITY_TABLES } from './interchangeability-rollout.helpers.js'

const WORKSPACE_INTERCHANGEABILITY_DOMAINS: Array<{
  domain: InterchangeabilityAdminDomain
  tableName: string
...
@Injectable()
export class InterchangeabilityStatusService {
  ...
  async getInterchangeabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_INTERCHANGEABILITY_TABLES,
    )

    return {
      existingInterchangeabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceInterchangeabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_INTERCHANGEABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_INTERCHANGEABILITY_DOMAINS.map(async (entry) => {
