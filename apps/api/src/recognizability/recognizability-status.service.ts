...
import type { RecognizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_RECOGNIZABILITY_TABLES } from './recognizability-rollout.helpers.js'

const WORKSPACE_RECOGNIZABILITY_DOMAINS: Array<{
  domain: RecognizabilityAdminDomain
  tableName: string
...
@Injectable()
export class RecognizabilityStatusService {
  ...
  async getRecognizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_RECOGNIZABILITY_TABLES,
    )

    return {
      existingRecognizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceRecognizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_RECOGNIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_RECOGNIZABILITY_DOMAINS.map(async (entry) => {
