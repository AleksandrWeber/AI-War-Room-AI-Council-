...
import type { RetrodictizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_RETRODICTIZABILITY_TABLES } from './retrodictizability-rollout.helpers.js'

const WORKSPACE_RETRODICTIZABILITY_DOMAINS: Array<{
  domain: RetrodictizabilityAdminDomain
  tableName: string
...
@Injectable()
export class RetrodictizabilityStatusService {
  ...
  async getRetrodictizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_RETRODICTIZABILITY_TABLES,
    )

    return {
      existingRetrodictizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceRetrodictizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_RETRODICTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_RETRODICTIZABILITY_DOMAINS.map(async (entry) => {
