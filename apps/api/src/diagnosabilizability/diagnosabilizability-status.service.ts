...
import type { DiagnosabilizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_DIAGNOSABILIZABILITY_TABLES } from './diagnosabilizability-rollout.helpers.js'

const WORKSPACE_DIAGNOSABILIZABILITY_DOMAINS: Array<{
  domain: DiagnosabilizabilityAdminDomain
  tableName: string
...
@Injectable()
export class DiagnosabilizabilityStatusService {
  ...
  async getDiagnosabilizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_DIAGNOSABILIZABILITY_TABLES,
    )

    return {
      existingDiagnosabilizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceDiagnosabilizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_DIAGNOSABILIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_DIAGNOSABILIZABILITY_DOMAINS.map(async (entry) => {
