...
import type { AnalogizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ANALOGIZABILITY_TABLES } from './analogizability-rollout.helpers.js'

const WORKSPACE_ANALOGIZABILITY_DOMAINS: Array<{
  domain: AnalogizabilityAdminDomain
  tableName: string
...
@Injectable()
export class AnalogizabilityStatusService {
  ...
  async getAnalogizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ANALOGIZABILITY_TABLES,
    )

    return {
      existingAnalogizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAnalogizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ANALOGIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ANALOGIZABILITY_DOMAINS.map(async (entry) => {
