...
import type { ObservabilizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_OBSERVABILIZABILITY_TABLES } from './observabilizability-rollout.helpers.js'

const WORKSPACE_OBSERVABILIZABILITY_DOMAINS: Array<{
  domain: ObservabilizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ObservabilizabilityStatusService {
  ...
  async getObservabilizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_OBSERVABILIZABILITY_TABLES,
    )

    return {
      existingObservabilizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceObservabilizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_OBSERVABILIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_OBSERVABILIZABILITY_DOMAINS.map(async (entry) => {
