...
import type { ComplianceproofizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_COMPLIANCEPROOFIZABILITY_TABLES } from './complianceproofizability-rollout.helpers.js'

const WORKSPACE_COMPLIANCEPROOFIZABILITY_DOMAINS: Array<{
  domain: ComplianceproofizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ComplianceproofizabilityStatusService {
  ...
  async getComplianceproofizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_COMPLIANCEPROOFIZABILITY_TABLES,
    )

    return {
      existingComplianceproofizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceComplianceproofizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_COMPLIANCEPROOFIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_COMPLIANCEPROOFIZABILITY_DOMAINS.map(async (entry) => {
