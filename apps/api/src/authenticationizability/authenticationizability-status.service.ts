...
import type { AuthenticationizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_AUTHENTICATIONIZABILITY_TABLES } from './authenticationizability-rollout.helpers.js'

const WORKSPACE_AUTHENTICATIONIZABILITY_DOMAINS: Array<{
  domain: AuthenticationizabilityAdminDomain
  tableName: string
...
@Injectable()
export class AuthenticationizabilityStatusService {
  ...
  async getAuthenticationizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_AUTHENTICATIONIZABILITY_TABLES,
    )

    return {
      existingAuthenticationizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAuthenticationizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_AUTHENTICATIONIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_AUTHENTICATIONIZABILITY_DOMAINS.map(async (entry) => {
