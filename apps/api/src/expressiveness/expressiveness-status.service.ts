...
import type { ExpressivenessAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_EXPRESSIVENESS_TABLES } from './expressiveness-rollout.helpers.js'

const WORKSPACE_EXPRESSIVENESS_DOMAINS: Array<{
  domain: ExpressivenessAdminDomain
  tableName: string
...
@Injectable()
export class ExpressivenessStatusService {
  ...
  async getExpressivenessTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_EXPRESSIVENESS_TABLES,
    )

    return {
      existingExpressivenessTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceExpressivenessInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_EXPRESSIVENESS_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_EXPRESSIVENESS_DOMAINS.map(async (entry) => {
