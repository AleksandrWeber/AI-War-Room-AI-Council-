...
import type { SyntacticizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_SYNTACTICIZABILITY_TABLES } from './syntacticizability-rollout.helpers.js'

const WORKSPACE_SYNTACTICIZABILITY_DOMAINS: Array<{
  domain: SyntacticizabilityAdminDomain
  tableName: string
...
@Injectable()
export class SyntacticizabilityStatusService {
  ...
  async getSyntacticizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_SYNTACTICIZABILITY_TABLES,
    )

    return {
      existingSyntacticizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceSyntacticizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_SYNTACTICIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_SYNTACTICIZABILITY_DOMAINS.map(async (entry) => {
