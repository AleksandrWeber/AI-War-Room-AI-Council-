...
import type { ConvergizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CONVERGIZABILITY_TABLES } from './convergizability-rollout.helpers.js'

const WORKSPACE_CONVERGIZABILITY_DOMAINS: Array<{
  domain: ConvergizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ConvergizabilityStatusService {
  ...
  async getConvergizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CONVERGIZABILITY_TABLES,
    )

    return {
      existingConvergizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceConvergizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CONVERGIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CONVERGIZABILITY_DOMAINS.map(async (entry) => {
