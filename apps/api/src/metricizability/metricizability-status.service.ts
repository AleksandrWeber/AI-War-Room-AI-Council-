...
import type { MetricizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_METRICIZABILITY_TABLES } from './metricizability-rollout.helpers.js'

const WORKSPACE_METRICIZABILITY_DOMAINS: Array<{
  domain: MetricizabilityAdminDomain
  tableName: string
...
@Injectable()
export class MetricizabilityStatusService {
  ...
  async getMetricizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_METRICIZABILITY_TABLES,
    )

    return {
      existingMetricizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceMetricizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_METRICIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_METRICIZABILITY_DOMAINS.map(async (entry) => {
