...
import type { ConnectabilizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CONNECTABILIZABILITY_TABLES } from './connectabilizability-rollout.helpers.js'

const WORKSPACE_CONNECTABILIZABILITY_DOMAINS: Array<{
  domain: ConnectabilizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ConnectabilizabilityStatusService {
  ...
  async getConnectabilizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CONNECTABILIZABILITY_TABLES,
    )

    return {
      existingConnectabilizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceConnectabilizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CONNECTABILIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CONNECTABILIZABILITY_DOMAINS.map(async (entry) => {
