...
import type { ProvisioningizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_PROVISIONINGIZABILITY_TABLES } from './provisioningizability-rollout.helpers.js'

const WORKSPACE_PROVISIONINGIZABILITY_DOMAINS: Array<{
  domain: ProvisioningizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ProvisioningizabilityStatusService {
  ...
  async getProvisioningizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_PROVISIONINGIZABILITY_TABLES,
    )

    return {
      existingProvisioningizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceProvisioningizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_PROVISIONINGIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_PROVISIONINGIZABILITY_DOMAINS.map(async (entry) => {
