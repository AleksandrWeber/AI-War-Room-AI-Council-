...
import type { ValidityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_VALIDITYVAULTIZABILITY_TABLES } from './validityvaultizability-rollout.helpers.js'

const WORKSPACE_VALIDITYVAULTIZABILITY_DOMAINS: Array<{
  domain: ValidityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ValidityvaultizabilityStatusService {
  ...
  async getValidityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_VALIDITYVAULTIZABILITY_TABLES,
    )

    return {
      existingValidityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceValidityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_VALIDITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_VALIDITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
