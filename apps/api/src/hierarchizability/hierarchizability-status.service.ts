...
import type { HierarchizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_HIERARCHIZABILITY_TABLES } from './hierarchizability-rollout.helpers.js'

const WORKSPACE_HIERARCHIZABILITY_DOMAINS: Array<{
  domain: HierarchizabilityAdminDomain
  tableName: string
...
@Injectable()
export class HierarchizabilityStatusService {
  ...
  async getHierarchizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_HIERARCHIZABILITY_TABLES,
    )

    return {
      existingHierarchizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceHierarchizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_HIERARCHIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_HIERARCHIZABILITY_DOMAINS.map(async (entry) => {
