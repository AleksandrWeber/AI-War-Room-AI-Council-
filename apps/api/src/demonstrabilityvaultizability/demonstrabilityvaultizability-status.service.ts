...
import type { DemonstrabilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_DEMONSTRABILITYVAULTIZABILITY_TABLES } from './demonstrabilityvaultizability-rollout.helpers.js'

const WORKSPACE_DEMONSTRABILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: DemonstrabilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class DemonstrabilityvaultizabilityStatusService {
  ...
  async getDemonstrabilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_DEMONSTRABILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingDemonstrabilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceDemonstrabilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_DEMONSTRABILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_DEMONSTRABILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
