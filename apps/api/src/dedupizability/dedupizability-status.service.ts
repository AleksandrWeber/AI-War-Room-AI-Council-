...
import type { DedupizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_DEDUPIZABILITY_TABLES } from './dedupizability-rollout.helpers.js'

const WORKSPACE_DEDUPIZABILITY_DOMAINS: Array<{
  domain: DedupizabilityAdminDomain
  tableName: string
...
@Injectable()
export class DedupizabilityStatusService {
  ...
  async getDedupizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_DEDUPIZABILITY_TABLES,
    )

    return {
      existingDedupizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceDedupizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_DEDUPIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_DEDUPIZABILITY_DOMAINS.map(async (entry) => {
