...
import type { ComposabilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_COMPOSABILITYVAULTIZABILITY_TABLES } from './composabilityvaultizability-rollout.helpers.js'

const WORKSPACE_COMPOSABILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: ComposabilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ComposabilityvaultizabilityStatusService {
  ...
  async getComposabilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_COMPOSABILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingComposabilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceComposabilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_COMPOSABILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_COMPOSABILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
