...
import type { QueryizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_QUERYIZABILITY_TABLES } from './queryizability-rollout.helpers.js'

const WORKSPACE_QUERYIZABILITY_DOMAINS: Array<{
  domain: QueryizabilityAdminDomain
  tableName: string
...
@Injectable()
export class QueryizabilityStatusService {
  ...
  async getQueryizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_QUERYIZABILITY_TABLES,
    )

    return {
      existingQueryizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceQueryizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_QUERYIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_QUERYIZABILITY_DOMAINS.map(async (entry) => {
