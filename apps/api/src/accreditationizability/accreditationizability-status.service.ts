...
import type { AccreditationizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ACCREDITATIONIZABILITY_TABLES } from './accreditationizability-rollout.helpers.js'

const WORKSPACE_ACCREDITATIONIZABILITY_DOMAINS: Array<{
  domain: AccreditationizabilityAdminDomain
  tableName: string
...
@Injectable()
export class AccreditationizabilityStatusService {
  ...
  async getAccreditationizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ACCREDITATIONIZABILITY_TABLES,
    )

    return {
      existingAccreditationizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAccreditationizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ACCREDITATIONIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ACCREDITATIONIZABILITY_DOMAINS.map(async (entry) => {
