...
import type { ParsabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_PARSABILITY_TABLES } from './parsability-rollout.helpers.js'

const WORKSPACE_PARSABILITY_DOMAINS: Array<{
  domain: ParsabilityAdminDomain
  tableName: string
...
@Injectable()
export class ParsabilityStatusService {
  ...
  async getParsabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_PARSABILITY_TABLES,
    )

    return {
      existingParsabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceParsabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_PARSABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_PARSABILITY_DOMAINS.map(async (entry) => {
