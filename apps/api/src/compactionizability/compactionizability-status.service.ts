...
import type { CompactionizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_COMPACTIONIZABILITY_TABLES } from './compactionizability-rollout.helpers.js'

const WORKSPACE_COMPACTIONIZABILITY_DOMAINS: Array<{
  domain: CompactionizabilityAdminDomain
  tableName: string
...
@Injectable()
export class CompactionizabilityStatusService {
  ...
  async getCompactionizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_COMPACTIONIZABILITY_TABLES,
    )

    return {
      existingCompactionizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceCompactionizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_COMPACTIONIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_COMPACTIONIZABILITY_DOMAINS.map(async (entry) => {
