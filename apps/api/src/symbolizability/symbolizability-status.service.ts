...
import type { SymbolizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_SYMBOLIZABILITY_TABLES } from './symbolizability-rollout.helpers.js'

const WORKSPACE_SYMBOLIZABILITY_DOMAINS: Array<{
  domain: SymbolizabilityAdminDomain
  tableName: string
...
@Injectable()
export class SymbolizabilityStatusService {
  ...
  async getSymbolizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_SYMBOLIZABILITY_TABLES,
    )

    return {
      existingSymbolizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceSymbolizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_SYMBOLIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_SYMBOLIZABILITY_DOMAINS.map(async (entry) => {
