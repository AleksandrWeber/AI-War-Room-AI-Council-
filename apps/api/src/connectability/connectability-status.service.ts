...
import type { ConnectabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CONNECTABILITY_TABLES } from './connectability-rollout.helpers.js'

const WORKSPACE_CONNECTABILITY_DOMAINS: Array<{
  domain: ConnectabilityAdminDomain
  tableName: string
...
@Injectable()
export class ConnectabilityStatusService {
  ...
  async getConnectabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CONNECTABILITY_TABLES,
    )

    return {
      existingConnectabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceConnectabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CONNECTABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CONNECTABILITY_DOMAINS.map(async (entry) => {
