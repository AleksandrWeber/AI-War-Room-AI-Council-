...
import type { SchedulabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_SCHEDULABILITY_TABLES } from './schedulability-rollout.helpers.js'

const WORKSPACE_SCHEDULABILITY_DOMAINS: Array<{
  domain: SchedulabilityAdminDomain
  tableName: string
...
@Injectable()
export class SchedulabilityStatusService {
  ...
  async getSchedulabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_SCHEDULABILITY_TABLES,
    )

    return {
      existingSchedulabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceSchedulabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_SCHEDULABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_SCHEDULABILITY_DOMAINS.map(async (entry) => {
