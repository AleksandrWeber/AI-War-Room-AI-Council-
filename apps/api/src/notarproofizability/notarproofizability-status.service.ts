...
import type { NotarproofizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_NOTARPROOFIZABILITY_TABLES } from './notarproofizability-rollout.helpers.js'

const WORKSPACE_NOTARPROOFIZABILITY_DOMAINS: Array<{
  domain: NotarproofizabilityAdminDomain
  tableName: string
...
@Injectable()
export class NotarproofizabilityStatusService {
  ...
  async getNotarproofizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_NOTARPROOFIZABILITY_TABLES,
    )

    return {
      existingNotarproofizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceNotarproofizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_NOTARPROOFIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_NOTARPROOFIZABILITY_DOMAINS.map(async (entry) => {
