...
import type { NormalizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_NORMALIZABILITY_TABLES } from './normalizability-rollout.helpers.js'

const WORKSPACE_NORMALIZABILITY_DOMAINS: Array<{
  domain: NormalizabilityAdminDomain
  tableName: string
...
@Injectable()
export class NormalizabilityStatusService {
  ...
  async getNormalizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_NORMALIZABILITY_TABLES,
    )

    return {
      existingNormalizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceNormalizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_NORMALIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_NORMALIZABILITY_DOMAINS.map(async (entry) => {
