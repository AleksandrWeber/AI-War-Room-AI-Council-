...
import type { CanonicalizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CANONICALIZABILITY_TABLES } from './canonicalizability-rollout.helpers.js'

const WORKSPACE_CANONICALIZABILITY_DOMAINS: Array<{
  domain: CanonicalizabilityAdminDomain
  tableName: string
...
@Injectable()
export class CanonicalizabilityStatusService {
  ...
  async getCanonicalizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CANONICALIZABILITY_TABLES,
    )

    return {
      existingCanonicalizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceCanonicalizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CANONICALIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CANONICALIZABILITY_DOMAINS.map(async (entry) => {
