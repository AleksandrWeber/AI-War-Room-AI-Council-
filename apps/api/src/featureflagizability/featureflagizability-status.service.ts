...
import type { FeatureflagizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_FEATUREFLAGIZABILITY_TABLES } from './featureflagizability-rollout.helpers.js'

const WORKSPACE_FEATUREFLAGIZABILITY_DOMAINS: Array<{
  domain: FeatureflagizabilityAdminDomain
  tableName: string
...
@Injectable()
export class FeatureflagizabilityStatusService {
  ...
  async getFeatureflagizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_FEATUREFLAGIZABILITY_TABLES,
    )

    return {
      existingFeatureflagizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceFeatureflagizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_FEATUREFLAGIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_FEATUREFLAGIZABILITY_DOMAINS.map(async (entry) => {
