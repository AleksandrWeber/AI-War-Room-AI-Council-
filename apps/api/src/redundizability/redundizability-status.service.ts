...
import type { RedundizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_REDUNDIZABILITY_TABLES } from './redundizability-rollout.helpers.js'

const WORKSPACE_REDUNDIZABILITY_DOMAINS: Array<{
  domain: RedundizabilityAdminDomain
  tableName: string
...
@Injectable()
export class RedundizabilityStatusService {
  ...
  async getRedundizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_REDUNDIZABILITY_TABLES,
    )

    return {
      existingRedundizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceRedundizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_REDUNDIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_REDUNDIZABILITY_DOMAINS.map(async (entry) => {
