...
import type { RiskizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_RISKIZABILITY_TABLES } from './riskizability-rollout.helpers.js'

const WORKSPACE_RISKIZABILITY_DOMAINS: Array<{
  domain: RiskizabilityAdminDomain
  tableName: string
...
@Injectable()
export class RiskizabilityStatusService {
  ...
  async getRiskizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_RISKIZABILITY_TABLES,
    )

    return {
      existingRiskizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceRiskizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_RISKIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_RISKIZABILITY_DOMAINS.map(async (entry) => {
