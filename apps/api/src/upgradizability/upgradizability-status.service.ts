...
import type { UpgradizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_UPGRADIZABILITY_TABLES } from './upgradizability-rollout.helpers.js'

const WORKSPACE_UPGRADIZABILITY_DOMAINS: Array<{
  domain: UpgradizabilityAdminDomain
  tableName: string
...
@Injectable()
export class UpgradizabilityStatusService {
  ...
  async getUpgradizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_UPGRADIZABILITY_TABLES,
    )

    return {
      existingUpgradizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceUpgradizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_UPGRADIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_UPGRADIZABILITY_DOMAINS.map(async (entry) => {
