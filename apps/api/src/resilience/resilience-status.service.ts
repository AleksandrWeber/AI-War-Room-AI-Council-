...
import type { ResilienceAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_RESILIENCE_TABLES } from './resilience-rollout.helpers.js'

const WORKSPACE_RESILIENCE_DOMAINS: Array<{
  domain: ResilienceAdminDomain
  tableName: string
...
@Injectable()
export class ResilienceStatusService {
  ...
  async getResilienceTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_RESILIENCE_TABLES,
    )

    return {
      existingResilienceTableCount: existingTables.size,
      existingTables,
    }
  }

  async getWorkspaceResilienceInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_RESILIENCE_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_RESILIENCE_DOMAINS.map(async (entry) => {
