...
import type { RoutingizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ROUTINGIZABILITY_TABLES } from './routingizability-rollout.helpers.js'

const WORKSPACE_ROUTINGIZABILITY_DOMAINS: Array<{
  domain: RoutingizabilityAdminDomain
  tableName: string
...
@Injectable()
export class RoutingizabilityStatusService {
  ...
  async getRoutingizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ROUTINGIZABILITY_TABLES,
    )

    return {
      existingRoutingizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceRoutingizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ROUTINGIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ROUTINGIZABILITY_DOMAINS.map(async (entry) => {
