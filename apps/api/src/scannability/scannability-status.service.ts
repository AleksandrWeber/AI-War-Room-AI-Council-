...
import type { ScannabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_SCANNABILITY_TABLES } from './scannability-rollout.helpers.js'

const WORKSPACE_SCANNABILITY_DOMAINS: Array<{
  domain: ScannabilityAdminDomain
  tableName: string
...
@Injectable()
export class ScannabilityStatusService {
  ...
  async getScannabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_SCANNABILITY_TABLES,
    )

    return {
      existingScannabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceScannabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_SCANNABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_SCANNABILITY_DOMAINS.map(async (entry) => {
