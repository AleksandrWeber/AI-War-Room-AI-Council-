...
import type { GatewayizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_GATEWAYIZABILITY_TABLES } from './gatewayizability-rollout.helpers.js'

const WORKSPACE_GATEWAYIZABILITY_DOMAINS: Array<{
  domain: GatewayizabilityAdminDomain
  tableName: string
...
@Injectable()
export class GatewayizabilityStatusService {
  ...
  async getGatewayizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_GATEWAYIZABILITY_TABLES,
    )

    return {
      existingGatewayizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceGatewayizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_GATEWAYIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_GATEWAYIZABILITY_DOMAINS.map(async (entry) => {
