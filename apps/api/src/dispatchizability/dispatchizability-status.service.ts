...
import type { DispatchizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_DISPATCHIZABILITY_TABLES } from './dispatchizability-rollout.helpers.js'

const WORKSPACE_DISPATCHIZABILITY_DOMAINS: Array<{
  domain: DispatchizabilityAdminDomain
  tableName: string
...
@Injectable()
export class DispatchizabilityStatusService {
  ...
  async getDispatchizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_DISPATCHIZABILITY_TABLES,
    )

    return {
      existingDispatchizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceDispatchizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_DISPATCHIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_DISPATCHIZABILITY_DOMAINS.map(async (entry) => {
