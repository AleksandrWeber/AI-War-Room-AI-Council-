...
import type { DiscoverabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_DISCOVERABILITY_TABLES } from './discoverability-rollout.helpers.js'

const WORKSPACE_DISCOVERABILITY_DOMAINS: Array<{
  domain: DiscoverabilityAdminDomain
  tableName: string
...
@Injectable()
export class DiscoverabilityStatusService {
  ...
  async getDiscoverabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_DISCOVERABILITY_TABLES,
    )

    return {
      existingDiscoverabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceDiscoverabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_DISCOVERABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_DISCOVERABILITY_DOMAINS.map(async (entry) => {
