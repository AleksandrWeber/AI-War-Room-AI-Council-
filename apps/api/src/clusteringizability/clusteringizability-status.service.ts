...
import type { ClusteringizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CLUSTERINGIZABILITY_TABLES } from './clusteringizability-rollout.helpers.js'

const WORKSPACE_CLUSTERINGIZABILITY_DOMAINS: Array<{
  domain: ClusteringizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ClusteringizabilityStatusService {
  ...
  async getClusteringizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CLUSTERINGIZABILITY_TABLES,
    )

    return {
      existingClusteringizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceClusteringizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CLUSTERINGIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CLUSTERINGIZABILITY_DOMAINS.map(async (entry) => {
