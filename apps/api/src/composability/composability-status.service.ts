...
import type { ComposabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_COMPOSABILITY_TABLES } from './composability-rollout.helpers.js'

const WORKSPACE_COMPOSABILITY_DOMAINS: Array<{
  domain: ComposabilityAdminDomain
  tableName: string
...
@Injectable()
export class ComposabilityStatusService {
  ...
  async getComposabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_COMPOSABILITY_TABLES,
    )

    return {
      existingComposabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceComposabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_COMPOSABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_COMPOSABILITY_DOMAINS.map(async (entry) => {
