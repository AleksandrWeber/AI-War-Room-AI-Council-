...
import type { DesirabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_DESIRABILITY_TABLES } from './desirability-rollout.helpers.js'

const WORKSPACE_DESIRABILITY_DOMAINS: Array<{
  domain: DesirabilityAdminDomain
  tableName: string
...
@Injectable()
export class DesirabilityStatusService {
  ...
  async getDesirabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_DESIRABILITY_TABLES,
    )

    return {
      existingDesirabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceDesirabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_DESIRABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_DESIRABILITY_DOMAINS.map(async (entry) => {
