...
import type { RoutizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ROUTIZABILITY_TABLES } from './routizability-rollout.helpers.js'

const WORKSPACE_ROUTIZABILITY_DOMAINS: Array<{
  domain: RoutizabilityAdminDomain
  tableName: string
...
@Injectable()
export class RoutizabilityStatusService {
  ...
  async getRoutizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ROUTIZABILITY_TABLES,
    )

    return {
      existingRoutizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceRoutizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ROUTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ROUTIZABILITY_DOMAINS.map(async (entry) => {
