...
import type { LegibilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_LEGIBILITY_TABLES } from './legibility-rollout.helpers.js'

const WORKSPACE_LEGIBILITY_DOMAINS: Array<{
  domain: LegibilityAdminDomain
  tableName: string
...
@Injectable()
export class LegibilityStatusService {
  ...
  async getLegibilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_LEGIBILITY_TABLES,
    )

    return {
      existingLegibilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceLegibilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_LEGIBILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_LEGIBILITY_DOMAINS.map(async (entry) => {
