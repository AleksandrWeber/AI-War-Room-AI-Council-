...
import type { TaxonomizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_TAXONOMIZABILITY_TABLES } from './taxonomizability-rollout.helpers.js'

const WORKSPACE_TAXONOMIZABILITY_DOMAINS: Array<{
  domain: TaxonomizabilityAdminDomain
  tableName: string
...
@Injectable()
export class TaxonomizabilityStatusService {
  ...
  async getTaxonomizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_TAXONOMIZABILITY_TABLES,
    )

    return {
      existingTaxonomizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceTaxonomizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_TAXONOMIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_TAXONOMIZABILITY_DOMAINS.map(async (entry) => {
