...
import type { RepeatabilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_REPEATABILITYVAULTIZABILITY_TABLES } from './repeatabilityvaultizability-rollout.helpers.js'

const WORKSPACE_REPEATABILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: RepeatabilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class RepeatabilityvaultizabilityStatusService {
  ...
  async getRepeatabilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_REPEATABILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingRepeatabilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceRepeatabilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_REPEATABILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_REPEATABILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
