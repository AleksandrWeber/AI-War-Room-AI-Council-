...
import type { NoticeabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_NOTICEABILITY_TABLES } from './noticeability-rollout.helpers.js'

const WORKSPACE_NOTICEABILITY_DOMAINS: Array<{
  domain: NoticeabilityAdminDomain
  tableName: string
...
@Injectable()
export class NoticeabilityStatusService {
  ...
  async getNoticeabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_NOTICEABILITY_TABLES,
    )

    return {
      existingNoticeabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceNoticeabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_NOTICEABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_NOTICEABILITY_DOMAINS.map(async (entry) => {
