...
import type { InterpretabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_INTERPRETABILITY_TABLES } from './interpretability-rollout.helpers.js'

const WORKSPACE_INTERPRETABILITY_DOMAINS: Array<{
  domain: InterpretabilityAdminDomain
  tableName: string
...
@Injectable()
export class InterpretabilityStatusService {
  ...
  async getInterpretabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_INTERPRETABILITY_TABLES,
    )

    return {
      existingInterpretabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceInterpretabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_INTERPRETABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_INTERPRETABILITY_DOMAINS.map(async (entry) => {
