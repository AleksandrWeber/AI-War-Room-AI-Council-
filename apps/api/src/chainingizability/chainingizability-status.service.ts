...
import type { ChainingizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CHAININGIZABILITY_TABLES } from './chainingizability-rollout.helpers.js'

const WORKSPACE_CHAININGIZABILITY_DOMAINS: Array<{
  domain: ChainingizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ChainingizabilityStatusService {
  ...
  async getChainingizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CHAININGIZABILITY_TABLES,
    )

    return {
      existingChainingizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceChainingizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CHAININGIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CHAININGIZABILITY_DOMAINS.map(async (entry) => {
