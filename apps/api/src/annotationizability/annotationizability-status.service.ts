...
import type { AnnotationizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ANNOTATIONIZABILITY_TABLES } from './annotationizability-rollout.helpers.js'

const WORKSPACE_ANNOTATIONIZABILITY_DOMAINS: Array<{
  domain: AnnotationizabilityAdminDomain
  tableName: string
...
@Injectable()
export class AnnotationizabilityStatusService {
  ...
  async getAnnotationizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ANNOTATIONIZABILITY_TABLES,
    )

    return {
      existingAnnotationizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAnnotationizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ANNOTATIONIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ANNOTATIONIZABILITY_DOMAINS.map(async (entry) => {
