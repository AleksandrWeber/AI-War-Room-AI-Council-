...
import type { TraceabilizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_TRACEABILIZABILITY_TABLES } from './traceabilizability-rollout.helpers.js'

const WORKSPACE_TRACEABILIZABILITY_DOMAINS: Array<{
  domain: TraceabilizabilityAdminDomain
  tableName: string
...
@Injectable()
export class TraceabilizabilityStatusService {
  ...
  async getTraceabilizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_TRACEABILIZABILITY_TABLES,
    )

    return {
      existingTraceabilizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceTraceabilizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_TRACEABILIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_TRACEABILIZABILITY_DOMAINS.map(async (entry) => {
