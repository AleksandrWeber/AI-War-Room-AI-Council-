...
import type { ReplicabilizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_REPLICABILIZABILITY_TABLES } from './replicabilizability-rollout.helpers.js'

const WORKSPACE_REPLICABILIZABILITY_DOMAINS: Array<{
  domain: ReplicabilizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ReplicabilizabilityStatusService {
  ...
  async getReplicabilizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_REPLICABILIZABILITY_TABLES,
    )

    return {
      existingReplicabilizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceReplicabilizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_REPLICABILIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_REPLICABILIZABILITY_DOMAINS.map(async (entry) => {
