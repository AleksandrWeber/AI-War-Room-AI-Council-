...
import type { ProvenanceAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_PROVENANCE_TABLES } from './provenance-rollout.helpers.js'

const WORKSPACE_PROVENANCE_DOMAINS: Array<{
  domain: ProvenanceAdminDomain
  tableName: string
...
@Injectable()
export class ProvenanceStatusService {
  ...
  async getProvenanceTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_PROVENANCE_TABLES,
    )

    return {
      existingProvenanceTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceProvenanceInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_PROVENANCE_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_PROVENANCE_DOMAINS.map(async (entry) => {
