...
import type { DeducizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_DEDUCIZABILITY_TABLES } from './deducizability-rollout.helpers.js'

const WORKSPACE_DEDUCIZABILITY_DOMAINS: Array<{
  domain: DeducizabilityAdminDomain
  tableName: string
...
@Injectable()
export class DeducizabilityStatusService {
  ...
  async getDeducizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_DEDUCIZABILITY_TABLES,
    )

    return {
      existingDeducizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceDeducizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_DEDUCIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_DEDUCIZABILITY_DOMAINS.map(async (entry) => {
