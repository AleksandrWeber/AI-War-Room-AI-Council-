...
import type { ArchetypizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ARCHETYPIZABILITY_TABLES } from './archetypizability-rollout.helpers.js'

const WORKSPACE_ARCHETYPIZABILITY_DOMAINS: Array<{
  domain: ArchetypizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ArchetypizabilityStatusService {
  ...
  async getArchetypizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ARCHETYPIZABILITY_TABLES,
    )

    return {
      existingArchetypizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceArchetypizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ARCHETYPIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ARCHETYPIZABILITY_DOMAINS.map(async (entry) => {
