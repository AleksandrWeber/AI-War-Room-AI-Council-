...
import type { RelayizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_RELAYIZABILITY_TABLES } from './relayizability-rollout.helpers.js'

const WORKSPACE_RELAYIZABILITY_DOMAINS: Array<{
  domain: RelayizabilityAdminDomain
  tableName: string
...
@Injectable()
export class RelayizabilityStatusService {
  ...
  async getRelayizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_RELAYIZABILITY_TABLES,
    )

    return {
      existingRelayizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceRelayizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_RELAYIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_RELAYIZABILITY_DOMAINS.map(async (entry) => {
