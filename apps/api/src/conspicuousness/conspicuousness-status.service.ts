...
import type { ConspicuousnessAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CONSPICUOUSNESS_TABLES } from './conspicuousness-rollout.helpers.js'

const WORKSPACE_CONSPICUOUSNESS_DOMAINS: Array<{
  domain: ConspicuousnessAdminDomain
  tableName: string
...
@Injectable()
export class ConspicuousnessStatusService {
  ...
  async getConspicuousnessTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CONSPICUOUSNESS_TABLES,
    )

    return {
      existingConspicuousnessTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceConspicuousnessInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CONSPICUOUSNESS_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CONSPICUOUSNESS_DOMAINS.map(async (entry) => {
