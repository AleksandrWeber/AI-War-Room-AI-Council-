...
import type { HarmonizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_HARMONIZABILITY_TABLES } from './harmonizability-rollout.helpers.js'

const WORKSPACE_HARMONIZABILITY_DOMAINS: Array<{
  domain: HarmonizabilityAdminDomain
  tableName: string
...
@Injectable()
export class HarmonizabilityStatusService {
  ...
  async getHarmonizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_HARMONIZABILITY_TABLES,
    )

    return {
      existingHarmonizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceHarmonizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_HARMONIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_HARMONIZABILITY_DOMAINS.map(async (entry) => {
