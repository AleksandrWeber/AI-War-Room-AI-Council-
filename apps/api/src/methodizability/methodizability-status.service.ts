...
import type { MethodizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_METHODIZABILITY_TABLES } from './methodizability-rollout.helpers.js'

const WORKSPACE_METHODIZABILITY_DOMAINS: Array<{
  domain: MethodizabilityAdminDomain
  tableName: string
...
@Injectable()
export class MethodizabilityStatusService {
  ...
  async getMethodizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_METHODIZABILITY_TABLES,
    )

    return {
      existingMethodizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceMethodizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_METHODIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_METHODIZABILITY_DOMAINS.map(async (entry) => {
