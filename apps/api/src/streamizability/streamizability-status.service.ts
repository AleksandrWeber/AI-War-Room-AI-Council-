...
import type { StreamizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_STREAMIZABILITY_TABLES } from './streamizability-rollout.helpers.js'

const WORKSPACE_STREAMIZABILITY_DOMAINS: Array<{
  domain: StreamizabilityAdminDomain
  tableName: string
...
@Injectable()
export class StreamizabilityStatusService {
  ...
  async getStreamizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_STREAMIZABILITY_TABLES,
    )

    return {
      existingStreamizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceStreamizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_STREAMIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_STREAMIZABILITY_DOMAINS.map(async (entry) => {
