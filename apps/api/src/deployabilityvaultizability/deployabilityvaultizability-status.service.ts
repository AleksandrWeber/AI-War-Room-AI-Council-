...
import type { DeployabilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_DEPLOYABILITYVAULTIZABILITY_TABLES } from './deployabilityvaultizability-rollout.helpers.js'

const WORKSPACE_DEPLOYABILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: DeployabilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class DeployabilityvaultizabilityStatusService {
  ...
  async getDeployabilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_DEPLOYABILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingDeployabilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceDeployabilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_DEPLOYABILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_DEPLOYABILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
