...
import type { PaginizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_PAGINIZABILITY_TABLES } from './paginizability-rollout.helpers.js'

const WORKSPACE_PAGINIZABILITY_DOMAINS: Array<{
  domain: PaginizabilityAdminDomain
  tableName: string
...
@Injectable()
export class PaginizabilityStatusService {
  ...
  async getPaginizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_PAGINIZABILITY_TABLES,
    )

    return {
      existingPaginizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspacePaginizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_PAGINIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_PAGINIZABILITY_DOMAINS.map(async (entry) => {
