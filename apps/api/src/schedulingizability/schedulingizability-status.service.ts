...
import type { SchedulingizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_SCHEDULINGIZABILITY_TABLES } from './schedulingizability-rollout.helpers.js'

const WORKSPACE_SCHEDULINGIZABILITY_DOMAINS: Array<{
  domain: SchedulingizabilityAdminDomain
  tableName: string
...
@Injectable()
export class SchedulingizabilityStatusService {
  ...
  async getSchedulingizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_SCHEDULINGIZABILITY_TABLES,
    )

    return {
      existingSchedulingizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceSchedulingizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_SCHEDULINGIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_SCHEDULINGIZABILITY_DOMAINS.map(async (entry) => {
