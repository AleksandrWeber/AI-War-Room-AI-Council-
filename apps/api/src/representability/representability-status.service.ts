...
import type { RepresentabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_REPRESENTABILITY_TABLES } from './representability-rollout.helpers.js'

const WORKSPACE_REPRESENTABILITY_DOMAINS: Array<{
  domain: RepresentabilityAdminDomain
  tableName: string
...
@Injectable()
export class RepresentabilityStatusService {
  ...
  async getRepresentabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_REPRESENTABILITY_TABLES,
    )

    return {
      existingRepresentabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceRepresentabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_REPRESENTABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_REPRESENTABILITY_DOMAINS.map(async (entry) => {
