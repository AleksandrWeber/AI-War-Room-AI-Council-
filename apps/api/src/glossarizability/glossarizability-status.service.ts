...
import type { GlossarizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_GLOSSARIZABILITY_TABLES } from './glossarizability-rollout.helpers.js'

const WORKSPACE_GLOSSARIZABILITY_DOMAINS: Array<{
  domain: GlossarizabilityAdminDomain
  tableName: string
...
@Injectable()
export class GlossarizabilityStatusService {
  ...
  async getGlossarizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_GLOSSARIZABILITY_TABLES,
    )

    return {
      existingGlossarizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceGlossarizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_GLOSSARIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_GLOSSARIZABILITY_DOMAINS.map(async (entry) => {
