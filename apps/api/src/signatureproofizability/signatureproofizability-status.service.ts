...
import type { SignatureproofizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_SIGNATUREPROOFIZABILITY_TABLES } from './signatureproofizability-rollout.helpers.js'

const WORKSPACE_SIGNATUREPROOFIZABILITY_DOMAINS: Array<{
  domain: SignatureproofizabilityAdminDomain
  tableName: string
...
@Injectable()
export class SignatureproofizabilityStatusService {
  ...
  async getSignatureproofizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_SIGNATUREPROOFIZABILITY_TABLES,
    )

    return {
      existingSignatureproofizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceSignatureproofizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_SIGNATUREPROOFIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_SIGNATUREPROOFIZABILITY_DOMAINS.map(async (entry) => {
