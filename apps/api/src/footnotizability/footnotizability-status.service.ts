...
import type { FootnotizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_FOOTNOTIZABILITY_TABLES } from './footnotizability-rollout.helpers.js'

const WORKSPACE_FOOTNOTIZABILITY_DOMAINS: Array<{
  domain: FootnotizabilityAdminDomain
  tableName: string
...
@Injectable()
export class FootnotizabilityStatusService {
  ...
  async getFootnotizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_FOOTNOTIZABILITY_TABLES,
    )

    return {
      existingFootnotizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceFootnotizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_FOOTNOTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_FOOTNOTIZABILITY_DOMAINS.map(async (entry) => {
