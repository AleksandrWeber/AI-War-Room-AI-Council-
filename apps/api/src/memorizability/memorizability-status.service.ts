...
import type { MemorizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_MEMORIZABILITY_TABLES } from './memorizability-rollout.helpers.js'

const WORKSPACE_MEMORIZABILITY_DOMAINS: Array<{
  domain: MemorizabilityAdminDomain
  tableName: string
...
@Injectable()
export class MemorizabilityStatusService {
  ...
  async getMemorizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_MEMORIZABILITY_TABLES,
    )

    return {
      existingMemorizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceMemorizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_MEMORIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_MEMORIZABILITY_DOMAINS.map(async (entry) => {
