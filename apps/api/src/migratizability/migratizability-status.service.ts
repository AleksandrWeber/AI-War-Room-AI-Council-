...
import type { MigratizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_MIGRATIZABILITY_TABLES } from './migratizability-rollout.helpers.js'

const WORKSPACE_MIGRATIZABILITY_DOMAINS: Array<{
  domain: MigratizabilityAdminDomain
  tableName: string
...
@Injectable()
export class MigratizabilityStatusService {
  ...
  async getMigratizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_MIGRATIZABILITY_TABLES,
    )

    return {
      existingMigratizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceMigratizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_MIGRATIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_MIGRATIZABILITY_DOMAINS.map(async (entry) => {
