...
import type { EffectivenessAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_EFFECTIVENESS_TABLES } from './effectiveness-rollout.helpers.js'

const WORKSPACE_EFFECTIVENESS_DOMAINS: Array<{
  domain: EffectivenessAdminDomain
  tableName: string
...
@Injectable()
export class EffectivenessStatusService {
  ...
  async getEffectivenessTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_EFFECTIVENESS_TABLES,
    )

    return {
      existingEffectivenessTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceEffectivenessInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_EFFECTIVENESS_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_EFFECTIVENESS_DOMAINS.map(async (entry) => {
