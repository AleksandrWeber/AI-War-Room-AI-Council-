...
import type { TerminologizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_TERMINOLOGIZABILITY_TABLES } from './terminologizability-rollout.helpers.js'

const WORKSPACE_TERMINOLOGIZABILITY_DOMAINS: Array<{
  domain: TerminologizabilityAdminDomain
  tableName: string
...
@Injectable()
export class TerminologizabilityStatusService {
  ...
  async getTerminologizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_TERMINOLOGIZABILITY_TABLES,
    )

    return {
      existingTerminologizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceTerminologizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_TERMINOLOGIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_TERMINOLOGIZABILITY_DOMAINS.map(async (entry) => {
