...
import type { NotarjournalizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_NOTARJOURNALIZABILITY_TABLES } from './notarjournalizability-rollout.helpers.js'

const WORKSPACE_NOTARJOURNALIZABILITY_DOMAINS: Array<{
  domain: NotarjournalizabilityAdminDomain
  tableName: string
...
@Injectable()
export class NotarjournalizabilityStatusService {
  ...
  async getNotarjournalizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_NOTARJOURNALIZABILITY_TABLES,
    )

    return {
      existingNotarjournalizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceNotarjournalizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_NOTARJOURNALIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_NOTARJOURNALIZABILITY_DOMAINS.map(async (entry) => {
