...
import type { GnoseizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_GNOSEIZABILITY_TABLES } from './gnoseizability-rollout.helpers.js'

const WORKSPACE_GNOSEIZABILITY_DOMAINS: Array<{
  domain: GnoseizabilityAdminDomain
  tableName: string
...
@Injectable()
export class GnoseizabilityStatusService {
  ...
  async getGnoseizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_GNOSEIZABILITY_TABLES,
    )

    return {
      existingGnoseizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceGnoseizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_GNOSEIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_GNOSEIZABILITY_DOMAINS.map(async (entry) => {
