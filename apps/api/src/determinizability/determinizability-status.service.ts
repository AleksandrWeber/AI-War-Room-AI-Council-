...
import type { DeterminizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_DETERMINIZABILITY_TABLES } from './determinizability-rollout.helpers.js'

const WORKSPACE_DETERMINIZABILITY_DOMAINS: Array<{
  domain: DeterminizabilityAdminDomain
  tableName: string
...
@Injectable()
export class DeterminizabilityStatusService {
  ...
  async getDeterminizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_DETERMINIZABILITY_TABLES,
    )

    return {
      existingDeterminizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceDeterminizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_DETERMINIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_DETERMINIZABILITY_DOMAINS.map(async (entry) => {
