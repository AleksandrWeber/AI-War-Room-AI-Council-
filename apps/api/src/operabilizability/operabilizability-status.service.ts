...
import type { OperabilizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_OPERABILIZABILITY_TABLES } from './operabilizability-rollout.helpers.js'

const WORKSPACE_OPERABILIZABILITY_DOMAINS: Array<{
  domain: OperabilizabilityAdminDomain
  tableName: string
...
@Injectable()
export class OperabilizabilityStatusService {
  ...
  async getOperabilizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_OPERABILIZABILITY_TABLES,
    )

    return {
      existingOperabilizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceOperabilizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_OPERABILIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_OPERABILIZABILITY_DOMAINS.map(async (entry) => {
