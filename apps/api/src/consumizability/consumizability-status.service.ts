...
import type { ConsumizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CONSUMIZABILITY_TABLES } from './consumizability-rollout.helpers.js'

const WORKSPACE_CONSUMIZABILITY_DOMAINS: Array<{
  domain: ConsumizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ConsumizabilityStatusService {
  ...
  async getConsumizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CONSUMIZABILITY_TABLES,
    )

    return {
      existingConsumizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceConsumizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CONSUMIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CONSUMIZABILITY_DOMAINS.map(async (entry) => {
