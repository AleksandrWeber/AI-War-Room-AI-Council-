...
import type { VocabularizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_VOCABULARIZABILITY_TABLES } from './vocabularizability-rollout.helpers.js'

const WORKSPACE_VOCABULARIZABILITY_DOMAINS: Array<{
  domain: VocabularizabilityAdminDomain
  tableName: string
...
@Injectable()
export class VocabularizabilityStatusService {
  ...
  async getVocabularizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_VOCABULARIZABILITY_TABLES,
    )

    return {
      existingVocabularizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceVocabularizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_VOCABULARIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_VOCABULARIZABILITY_DOMAINS.map(async (entry) => {
