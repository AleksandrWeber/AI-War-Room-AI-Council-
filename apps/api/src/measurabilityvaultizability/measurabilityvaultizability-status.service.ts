...
import type { MeasurabilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_MEASURABILITYVAULTIZABILITY_TABLES } from './measurabilityvaultizability-rollout.helpers.js'

const WORKSPACE_MEASURABILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: MeasurabilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class MeasurabilityvaultizabilityStatusService {
  ...
  async getMeasurabilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_MEASURABILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingMeasurabilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceMeasurabilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_MEASURABILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_MEASURABILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
