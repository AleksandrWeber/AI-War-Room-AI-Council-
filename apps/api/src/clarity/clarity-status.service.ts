...
import type { ClarityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CLARITY_TABLES } from './clarity-rollout.helpers.js'

const WORKSPACE_CLARITY_DOMAINS: Array<{
  domain: ClarityAdminDomain
  tableName: string
...
@Injectable()
export class ClarityStatusService {
  ...
  async getClarityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CLARITY_TABLES,
    )

    return {
      existingClarityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceClarityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CLARITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CLARITY_DOMAINS.map(async (entry) => {
