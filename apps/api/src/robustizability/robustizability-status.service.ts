...
import type { RobustizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ROBUSTIZABILITY_TABLES } from './robustizability-rollout.helpers.js'

const WORKSPACE_ROBUSTIZABILITY_DOMAINS: Array<{
  domain: RobustizabilityAdminDomain
  tableName: string
...
@Injectable()
export class RobustizabilityStatusService {
  ...
  async getRobustizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ROBUSTIZABILITY_TABLES,
    )

    return {
      existingRobustizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceRobustizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ROBUSTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ROBUSTIZABILITY_DOMAINS.map(async (entry) => {
