...
import type { FlexibilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_FLEXIBILITY_TABLES } from './flexibility-rollout.helpers.js'

const WORKSPACE_FLEXIBILITY_DOMAINS: Array<{
  domain: FlexibilityAdminDomain
  tableName: string
...
@Injectable()
export class FlexibilityStatusService {
  ...
  async getFlexibilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_FLEXIBILITY_TABLES,
    )

    return {
      existingFlexibilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceFlexibilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_FLEXIBILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_FLEXIBILITY_DOMAINS.map(async (entry) => {
