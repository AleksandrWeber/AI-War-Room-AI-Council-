...
import type { InductizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_INDUCTIZABILITY_TABLES } from './inductizability-rollout.helpers.js'

const WORKSPACE_INDUCTIZABILITY_DOMAINS: Array<{
  domain: InductizabilityAdminDomain
  tableName: string
...
@Injectable()
export class InductizabilityStatusService {
  ...
  async getInductizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_INDUCTIZABILITY_TABLES,
    )

    return {
      existingInductizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceInductizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_INDUCTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_INDUCTIZABILITY_DOMAINS.map(async (entry) => {
