...
import type { MeasurabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_MEASURABILITY_TABLES } from './measurability-rollout.helpers.js'

const WORKSPACE_MEASURABILITY_DOMAINS: Array<{
  domain: MeasurabilityAdminDomain
  tableName: string
...
@Injectable()
export class MeasurabilityStatusService {
  ...
  async getMeasurabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_MEASURABILITY_TABLES,
    )

    return {
      existingMeasurabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceMeasurabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_MEASURABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_MEASURABILITY_DOMAINS.map(async (entry) => {
