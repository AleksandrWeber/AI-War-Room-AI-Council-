...
import type { ExtensibilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_EXTENSIBILITYVAULTIZABILITY_TABLES } from './extensibilityvaultizability-rollout.helpers.js'

const WORKSPACE_EXTENSIBILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: ExtensibilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ExtensibilityvaultizabilityStatusService {
  ...
  async getExtensibilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_EXTENSIBILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingExtensibilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceExtensibilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_EXTENSIBILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_EXTENSIBILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
