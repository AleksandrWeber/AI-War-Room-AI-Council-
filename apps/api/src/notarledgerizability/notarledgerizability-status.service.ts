...
import type { NotarledgerizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_NOTARLEDGERIZABILITY_TABLES } from './notarledgerizability-rollout.helpers.js'

const WORKSPACE_NOTARLEDGERIZABILITY_DOMAINS: Array<{
  domain: NotarledgerizabilityAdminDomain
  tableName: string
...
@Injectable()
export class NotarledgerizabilityStatusService {
  ...
  async getNotarledgerizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_NOTARLEDGERIZABILITY_TABLES,
    )

    return {
      existingNotarledgerizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceNotarledgerizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_NOTARLEDGERIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_NOTARLEDGERIZABILITY_DOMAINS.map(async (entry) => {
