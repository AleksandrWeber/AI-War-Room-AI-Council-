...
import type { BenchmarkizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_BENCHMARKIZABILITY_TABLES } from './benchmarkizability-rollout.helpers.js'

const WORKSPACE_BENCHMARKIZABILITY_DOMAINS: Array<{
  domain: BenchmarkizabilityAdminDomain
  tableName: string
...
@Injectable()
export class BenchmarkizabilityStatusService {
  ...
  async getBenchmarkizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_BENCHMARKIZABILITY_TABLES,
    )

    return {
      existingBenchmarkizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceBenchmarkizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_BENCHMARKIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_BENCHMARKIZABILITY_DOMAINS.map(async (entry) => {
