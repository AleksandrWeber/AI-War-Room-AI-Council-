...
import type { PermissionizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_PERMISSIONIZABILITY_TABLES } from './permissionizability-rollout.helpers.js'

const WORKSPACE_PERMISSIONIZABILITY_DOMAINS: Array<{
  domain: PermissionizabilityAdminDomain
  tableName: string
...
@Injectable()
export class PermissionizabilityStatusService {
  ...
  async getPermissionizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_PERMISSIONIZABILITY_TABLES,
    )

    return {
      existingPermissionizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspacePermissionizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_PERMISSIONIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_PERMISSIONIZABILITY_DOMAINS.map(async (entry) => {
