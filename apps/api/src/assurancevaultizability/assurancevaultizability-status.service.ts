...
import type { AssurancevaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ASSURANCEVAULTIZABILITY_TABLES } from './assurancevaultizability-rollout.helpers.js'

const WORKSPACE_ASSURANCEVAULTIZABILITY_DOMAINS: Array<{
  domain: AssurancevaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class AssurancevaultizabilityStatusService {
  ...
  async getAssurancevaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ASSURANCEVAULTIZABILITY_TABLES,
    )

    return {
      existingAssurancevaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAssurancevaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ASSURANCEVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ASSURANCEVAULTIZABILITY_DOMAINS.map(async (entry) => {
