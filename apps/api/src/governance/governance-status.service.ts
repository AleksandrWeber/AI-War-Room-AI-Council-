...
import type { GovernanceAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_GOVERNANCE_TABLES } from './governance-rollout.helpers.js'

const WORKSPACE_GOVERNANCE_DOMAINS: Array<{
  domain: GovernanceAdminDomain
  tableName: string
...
export class GovernanceStatusService {
  ...
  async getGovernanceTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_GOVERNANCE_TABLES,
    )

    return {
      existingGovernanceTableCount: existingTables.size,
      existingTables,
      workspaceMembershipsTableExists: existingTables.has(
        'workspace_memberships',
      ),
      providerCredentialsTableExists: existingTables.has(
  ...
  async getWorkspaceGovernanceInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_GOVERNANCE_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_GOVERNANCE_DOMAINS.map(async (entry) => {
