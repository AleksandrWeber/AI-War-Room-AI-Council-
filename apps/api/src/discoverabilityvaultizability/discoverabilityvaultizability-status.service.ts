...
import type { DiscoverabilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_DISCOVERABILITYVAULTIZABILITY_TABLES } from './discoverabilityvaultizability-rollout.helpers.js'

const WORKSPACE_DISCOVERABILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: DiscoverabilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class DiscoverabilityvaultizabilityStatusService {
  ...
  async getDiscoverabilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_DISCOVERABILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingDiscoverabilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceDiscoverabilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_DISCOVERABILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_DISCOVERABILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
