...
import type { MythicizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_MYTHICIZABILITY_TABLES } from './mythicizability-rollout.helpers.js'

const WORKSPACE_MYTHICIZABILITY_DOMAINS: Array<{
  domain: MythicizabilityAdminDomain
  tableName: string
...
@Injectable()
export class MythicizabilityStatusService {
  ...
  async getMythicizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_MYTHICIZABILITY_TABLES,
    )

    return {
      existingMythicizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceMythicizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_MYTHICIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_MYTHICIZABILITY_DOMAINS.map(async (entry) => {
