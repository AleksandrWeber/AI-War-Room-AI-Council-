...
import type { DemonstrabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_DEMONSTRABILITY_TABLES } from './demonstrability-rollout.helpers.js'

const WORKSPACE_DEMONSTRABILITY_DOMAINS: Array<{
  domain: DemonstrabilityAdminDomain
  tableName: string
...
@Injectable()
export class DemonstrabilityStatusService {
  ...
  async getDemonstrabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_DEMONSTRABILITY_TABLES,
    )

    return {
      existingDemonstrabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceDemonstrabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_DEMONSTRABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_DEMONSTRABILITY_DOMAINS.map(async (entry) => {
