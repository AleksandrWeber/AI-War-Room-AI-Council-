...
import type { RegistrarizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_REGISTRARIZABILITY_TABLES } from './registrarizability-rollout.helpers.js'

const WORKSPACE_REGISTRARIZABILITY_DOMAINS: Array<{
  domain: RegistrarizabilityAdminDomain
  tableName: string
...
@Injectable()
export class RegistrarizabilityStatusService {
  ...
  async getRegistrarizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_REGISTRARIZABILITY_TABLES,
    )

    return {
      existingRegistrarizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceRegistrarizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_REGISTRARIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_REGISTRARIZABILITY_DOMAINS.map(async (entry) => {
