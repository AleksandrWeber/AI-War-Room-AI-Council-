...
import type { CompatibilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_COMPATIBILITY_TABLES } from './compatibility-rollout.helpers.js'

const WORKSPACE_COMPATIBILITY_DOMAINS: Array<{
  domain: CompatibilityAdminDomain
  tableName: string
...
@Injectable()
export class CompatibilityStatusService {
  ...
  async getCompatibilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_COMPATIBILITY_TABLES,
    )

    return {
      existingCompatibilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceCompatibilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_COMPATIBILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_COMPATIBILITY_DOMAINS.map(async (entry) => {
