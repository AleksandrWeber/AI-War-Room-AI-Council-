...
import type { QueueizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_QUEUEIZABILITY_TABLES } from './queueizability-rollout.helpers.js'

const WORKSPACE_QUEUEIZABILITY_DOMAINS: Array<{
  domain: QueueizabilityAdminDomain
  tableName: string
...
@Injectable()
export class QueueizabilityStatusService {
  ...
  async getQueueizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_QUEUEIZABILITY_TABLES,
    )

    return {
      existingQueueizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceQueueizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_QUEUEIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_QUEUEIZABILITY_DOMAINS.map(async (entry) => {
