...
import type { DecentralizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_DECENTRALIZABILITY_TABLES } from './decentralizability-rollout.helpers.js'

const WORKSPACE_DECENTRALIZABILITY_DOMAINS: Array<{
  domain: DecentralizabilityAdminDomain
  tableName: string
...
@Injectable()
export class DecentralizabilityStatusService {
  ...
  async getDecentralizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_DECENTRALIZABILITY_TABLES,
    )

    return {
      existingDecentralizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceDecentralizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_DECENTRALIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_DECENTRALIZABILITY_DOMAINS.map(async (entry) => {
