...
import type { CustomizabilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CUSTOMIZABILITYVAULTIZABILITY_TABLES } from './customizabilityvaultizability-rollout.helpers.js'

const WORKSPACE_CUSTOMIZABILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: CustomizabilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class CustomizabilityvaultizabilityStatusService {
  ...
  async getCustomizabilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CUSTOMIZABILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingCustomizabilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceCustomizabilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CUSTOMIZABILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CUSTOMIZABILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
