...
import type { ReproducibilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_REPRODUCIBILITYVAULTIZABILITY_TABLES } from './reproducibilityvaultizability-rollout.helpers.js'

const WORKSPACE_REPRODUCIBILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: ReproducibilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ReproducibilityvaultizabilityStatusService {
  ...
  async getReproducibilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_REPRODUCIBILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingReproducibilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceReproducibilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_REPRODUCIBILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_REPRODUCIBILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
