...
import type { InspectabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_INSPECTABILITY_TABLES } from './inspectability-rollout.helpers.js'

const WORKSPACE_INSPECTABILITY_DOMAINS: Array<{
  domain: InspectabilityAdminDomain
  tableName: string
...
@Injectable()
export class InspectabilityStatusService {
  ...
  async getInspectabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_INSPECTABILITY_TABLES,
    )

    return {
      existingInspectabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceInspectabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_INSPECTABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_INSPECTABILITY_DOMAINS.map(async (entry) => {
