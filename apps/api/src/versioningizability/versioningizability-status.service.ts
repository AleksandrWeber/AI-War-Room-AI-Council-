...
import type { VersioningizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_VERSIONINGIZABILITY_TABLES } from './versioningizability-rollout.helpers.js'

const WORKSPACE_VERSIONINGIZABILITY_DOMAINS: Array<{
  domain: VersioningizabilityAdminDomain
  tableName: string
...
@Injectable()
export class VersioningizabilityStatusService {
  ...
  async getVersioningizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_VERSIONINGIZABILITY_TABLES,
    )

    return {
      existingVersioningizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceVersioningizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_VERSIONINGIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_VERSIONINGIZABILITY_DOMAINS.map(async (entry) => {
