...
import type { ExpirationizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_EXPIRATIONIZABILITY_TABLES } from './expirationizability-rollout.helpers.js'

const WORKSPACE_EXPIRATIONIZABILITY_DOMAINS: Array<{
  domain: ExpirationizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ExpirationizabilityStatusService {
  ...
  async getExpirationizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_EXPIRATIONIZABILITY_TABLES,
    )

    return {
      existingExpirationizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceExpirationizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_EXPIRATIONIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_EXPIRATIONIZABILITY_DOMAINS.map(async (entry) => {
