...
import type { RhetorizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_RHETORIZABILITY_TABLES } from './rhetorizability-rollout.helpers.js'

const WORKSPACE_RHETORIZABILITY_DOMAINS: Array<{
  domain: RhetorizabilityAdminDomain
  tableName: string
...
@Injectable()
export class RhetorizabilityStatusService {
  ...
  async getRhetorizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_RHETORIZABILITY_TABLES,
    )

    return {
      existingRhetorizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceRhetorizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_RHETORIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_RHETORIZABILITY_DOMAINS.map(async (entry) => {
