...
import type { ControllabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CONTROLLABILITY_TABLES } from './controllability-rollout.helpers.js'

const WORKSPACE_CONTROLLABILITY_DOMAINS: Array<{
  domain: ControllabilityAdminDomain
  tableName: string
...
@Injectable()
export class ControllabilityStatusService {
  ...
  async getControllabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CONTROLLABILITY_TABLES,
    )

    return {
      existingControllabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceControllabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CONTROLLABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CONTROLLABILITY_DOMAINS.map(async (entry) => {
