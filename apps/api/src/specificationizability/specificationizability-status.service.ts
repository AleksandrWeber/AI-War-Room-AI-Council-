...
import type { SpecificationizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_SPECIFICATIONIZABILITY_TABLES } from './specificationizability-rollout.helpers.js'

const WORKSPACE_SPECIFICATIONIZABILITY_DOMAINS: Array<{
  domain: SpecificationizabilityAdminDomain
  tableName: string
...
@Injectable()
export class SpecificationizabilityStatusService {
  ...
  async getSpecificationizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_SPECIFICATIONIZABILITY_TABLES,
    )

    return {
      existingSpecificationizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceSpecificationizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_SPECIFICATIONIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_SPECIFICATIONIZABILITY_DOMAINS.map(async (entry) => {
