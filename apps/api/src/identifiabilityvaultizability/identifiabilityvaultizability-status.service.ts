...
import type { IdentifiabilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_IDENTIFIABILITYVAULTIZABILITY_TABLES } from './identifiabilityvaultizability-rollout.helpers.js'

const WORKSPACE_IDENTIFIABILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: IdentifiabilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class IdentifiabilityvaultizabilityStatusService {
  ...
  async getIdentifiabilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_IDENTIFIABILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingIdentifiabilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceIdentifiabilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_IDENTIFIABILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_IDENTIFIABILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
