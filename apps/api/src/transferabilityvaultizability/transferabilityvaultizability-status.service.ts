...
import type { TransferabilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_TRANSFERABILITYVAULTIZABILITY_TABLES } from './transferabilityvaultizability-rollout.helpers.js'

const WORKSPACE_TRANSFERABILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: TransferabilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class TransferabilityvaultizabilityStatusService {
  ...
  async getTransferabilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_TRANSFERABILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingTransferabilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceTransferabilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_TRANSFERABILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_TRANSFERABILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
