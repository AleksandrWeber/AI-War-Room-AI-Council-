...
import type { FollowerizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_FOLLOWERIZABILITY_TABLES } from './followerizability-rollout.helpers.js'

const WORKSPACE_FOLLOWERIZABILITY_DOMAINS: Array<{
  domain: FollowerizabilityAdminDomain
  tableName: string
...
@Injectable()
export class FollowerizabilityStatusService {
  ...
  async getFollowerizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_FOLLOWERIZABILITY_TABLES,
    )

    return {
      existingFollowerizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceFollowerizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_FOLLOWERIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_FOLLOWERIZABILITY_DOMAINS.map(async (entry) => {
