...
import type { AttestationizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ATTESTATIONIZABILITY_TABLES } from './attestationizability-rollout.helpers.js'

const WORKSPACE_ATTESTATIONIZABILITY_DOMAINS: Array<{
  domain: AttestationizabilityAdminDomain
  tableName: string
...
@Injectable()
export class AttestationizabilityStatusService {
  ...
  async getAttestationizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ATTESTATIONIZABILITY_TABLES,
    )

    return {
      existingAttestationizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAttestationizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ATTESTATIONIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ATTESTATIONIZABILITY_DOMAINS.map(async (entry) => {
