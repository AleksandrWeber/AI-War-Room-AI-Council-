...
import type { MirroringizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_MIRRORINGIZABILITY_TABLES } from './mirroringizability-rollout.helpers.js'

const WORKSPACE_MIRRORINGIZABILITY_DOMAINS: Array<{
  domain: MirroringizabilityAdminDomain
  tableName: string
...
@Injectable()
export class MirroringizabilityStatusService {
  ...
  async getMirroringizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_MIRRORINGIZABILITY_TABLES,
    )

    return {
      existingMirroringizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceMirroringizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_MIRRORINGIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_MIRRORINGIZABILITY_DOMAINS.map(async (entry) => {
