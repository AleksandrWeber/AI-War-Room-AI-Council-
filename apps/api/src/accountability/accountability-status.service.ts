...
import type { AccountabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ACCOUNTABILITY_TABLES } from './accountability-rollout.helpers.js'

const WORKSPACE_ACCOUNTABILITY_DOMAINS: Array<{
  domain: AccountabilityAdminDomain
  tableName: string
...
@Injectable()
export class AccountabilityStatusService {
  ...
  async getAccountabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ACCOUNTABILITY_TABLES,
    )

    return {
      existingAccountabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAccountabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ACCOUNTABILITY_DOMAINS.flatMap(
    ...
    return Promise.all(
      WORKSPACE_ACCOUNTABILITY_DOMAINS.map(async (entry) => {
