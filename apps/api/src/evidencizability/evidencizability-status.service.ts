...
import type { EvidencizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_EVIDENCIZABILITY_TABLES } from './evidencizability-rollout.helpers.js'

const WORKSPACE_EVIDENCIZABILITY_DOMAINS: Array<{
  domain: EvidencizabilityAdminDomain
  tableName: string
...
@Injectable()
export class EvidencizabilityStatusService {
  ...
  async getEvidencizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_EVIDENCIZABILITY_TABLES,
    )

    return {
      existingEvidencizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceEvidencizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_EVIDENCIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_EVIDENCIZABILITY_DOMAINS.map(async (entry) => {
