...
import type { LedgerizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_LEDGERIZABILITY_TABLES } from './ledgerizability-rollout.helpers.js'

const WORKSPACE_LEDGERIZABILITY_DOMAINS: Array<{
  domain: LedgerizabilityAdminDomain
  tableName: string
...
@Injectable()
export class LedgerizabilityStatusService {
  ...
  async getLedgerizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_LEDGERIZABILITY_TABLES,
    )

    return {
      existingLedgerizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceLedgerizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_LEDGERIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_LEDGERIZABILITY_DOMAINS.map(async (entry) => {
