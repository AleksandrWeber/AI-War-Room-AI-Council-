...
import type { LocatabilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_LOCATABILITYVAULTIZABILITY_TABLES } from './locatabilityvaultizability-rollout.helpers.js'

const WORKSPACE_LOCATABILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: LocatabilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class LocatabilityvaultizabilityStatusService {
  ...
  async getLocatabilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_LOCATABILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingLocatabilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceLocatabilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_LOCATABILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_LOCATABILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
