...
import type { NomenclatizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_NOMENCLATIZABILITY_TABLES } from './nomenclatizability-rollout.helpers.js'

const WORKSPACE_NOMENCLATIZABILITY_DOMAINS: Array<{
  domain: NomenclatizabilityAdminDomain
  tableName: string
...
@Injectable()
export class NomenclatizabilityStatusService {
  ...
  async getNomenclatizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_NOMENCLATIZABILITY_TABLES,
    )

    return {
      existingNomenclatizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceNomenclatizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_NOMENCLATIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_NOMENCLATIZABILITY_DOMAINS.map(async (entry) => {
