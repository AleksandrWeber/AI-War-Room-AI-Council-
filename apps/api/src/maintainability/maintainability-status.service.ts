...
import type { MaintainabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_MAINTAINABILITY_TABLES } from './maintainability-rollout.helpers.js'

const WORKSPACE_MAINTAINABILITY_DOMAINS: Array<{
  domain: MaintainabilityAdminDomain
  tableName: string
...
@Injectable()
export class MaintainabilityStatusService {
  ...
  async getMaintainabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_MAINTAINABILITY_TABLES,
    )

    return {
      existingMaintainabilityTableCount: existingTables.size,
      existingTables,
      modelHealthEventTableExists: existingTables.has('model_health_events'),
    }
  }

  async getWorkspaceMaintainabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_MAINTAINABILITY_DOMAINS.flatMap(
          (entry) => entry.requiredTables,
        ),
      ),
    ]
    ...
    return Promise.all(
      WORKSPACE_MAINTAINABILITY_DOMAINS.map(async (entry) => {
