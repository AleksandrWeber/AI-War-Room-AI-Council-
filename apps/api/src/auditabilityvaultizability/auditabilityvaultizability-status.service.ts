...
import type { AuditabilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_AUDITABILITYVAULTIZABILITY_TABLES } from './auditabilityvaultizability-rollout.helpers.js'

const WORKSPACE_AUDITABILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: AuditabilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class AuditabilityvaultizabilityStatusService {
  ...
  async getAuditabilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_AUDITABILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingAuditabilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAuditabilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_AUDITABILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_AUDITABILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
