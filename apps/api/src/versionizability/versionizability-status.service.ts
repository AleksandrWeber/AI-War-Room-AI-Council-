...
import type { VersionizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_VERSIONIZABILITY_TABLES } from './versionizability-rollout.helpers.js'

const WORKSPACE_VERSIONIZABILITY_DOMAINS: Array<{
  domain: VersionizabilityAdminDomain
  tableName: string
...
@Injectable()
export class VersionizabilityStatusService {
  ...
  async getVersionizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_VERSIONIZABILITY_TABLES,
    )

    return {
      existingVersionizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceVersionizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_VERSIONIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_VERSIONIZABILITY_DOMAINS.map(async (entry) => {
