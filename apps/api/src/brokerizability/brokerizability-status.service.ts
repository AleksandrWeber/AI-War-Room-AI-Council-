...
import type { BrokerizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_BROKERIZABILITY_TABLES } from './brokerizability-rollout.helpers.js'

const WORKSPACE_BROKERIZABILITY_DOMAINS: Array<{
  domain: BrokerizabilityAdminDomain
  tableName: string
...
@Injectable()
export class BrokerizabilityStatusService {
  ...
  async getBrokerizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_BROKERIZABILITY_TABLES,
    )

    return {
      existingBrokerizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceBrokerizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_BROKERIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_BROKERIZABILITY_DOMAINS.map(async (entry) => {
