...
import type { RegressizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_REGRESSIZABILITY_TABLES } from './regressizability-rollout.helpers.js'

const WORKSPACE_REGRESSIZABILITY_DOMAINS: Array<{
  domain: RegressizabilityAdminDomain
  tableName: string
...
@Injectable()
export class RegressizabilityStatusService {
  ...
  async getRegressizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_REGRESSIZABILITY_TABLES,
    )

    return {
      existingRegressizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceRegressizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_REGRESSIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_REGRESSIZABILITY_DOMAINS.map(async (entry) => {
