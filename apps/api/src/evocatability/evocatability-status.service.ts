...
import type { EvocatabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_EVOCATABILITY_TABLES } from './evocatability-rollout.helpers.js'

const WORKSPACE_EVOCATABILITY_DOMAINS: Array<{
  domain: EvocatabilityAdminDomain
  tableName: string
...
@Injectable()
export class EvocatabilityStatusService {
  ...
  async getEvocatabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_EVOCATABILITY_TABLES,
    )

    return {
      existingEvocatabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceEvocatabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_EVOCATABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_EVOCATABILITY_DOMAINS.map(async (entry) => {
