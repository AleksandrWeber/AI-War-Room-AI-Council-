...
import type { CategorizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CATEGORIZABILITY_TABLES } from './categorizability-rollout.helpers.js'

const WORKSPACE_CATEGORIZABILITY_DOMAINS: Array<{
  domain: CategorizabilityAdminDomain
  tableName: string
...
@Injectable()
export class CategorizabilityStatusService {
  ...
  async getCategorizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CATEGORIZABILITY_TABLES,
    )

    return {
      existingCategorizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceCategorizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CATEGORIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CATEGORIZABILITY_DOMAINS.map(async (entry) => {
