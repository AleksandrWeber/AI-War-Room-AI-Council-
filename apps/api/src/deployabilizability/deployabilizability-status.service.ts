...
import type { DeployabilizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_DEPLOYABILIZABILITY_TABLES } from './deployabilizability-rollout.helpers.js'

const WORKSPACE_DEPLOYABILIZABILITY_DOMAINS: Array<{
  domain: DeployabilizabilityAdminDomain
  tableName: string
...
@Injectable()
export class DeployabilizabilityStatusService {
  ...
  async getDeployabilizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_DEPLOYABILIZABILITY_TABLES,
    )

    return {
      existingDeployabilizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceDeployabilizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_DEPLOYABILIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_DEPLOYABILIZABILITY_DOMAINS.map(async (entry) => {
