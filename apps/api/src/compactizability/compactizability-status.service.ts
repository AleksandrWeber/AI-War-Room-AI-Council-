...
import type { CompactizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_COMPACTIZABILITY_TABLES } from './compactizability-rollout.helpers.js'

const WORKSPACE_COMPACTIZABILITY_DOMAINS: Array<{
  domain: CompactizabilityAdminDomain
  tableName: string
...
@Injectable()
export class CompactizabilityStatusService {
  ...
  async getCompactizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_COMPACTIZABILITY_TABLES,
    )

    return {
      existingCompactizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceCompactizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_COMPACTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_COMPACTIZABILITY_DOMAINS.map(async (entry) => {
