...
import type { SandboxizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_SANDBOXIZABILITY_TABLES } from './sandboxizability-rollout.helpers.js'

const WORKSPACE_SANDBOXIZABILITY_DOMAINS: Array<{
  domain: SandboxizabilityAdminDomain
  tableName: string
...
@Injectable()
export class SandboxizabilityStatusService {
  ...
  async getSandboxizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_SANDBOXIZABILITY_TABLES,
    )

    return {
      existingSandboxizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceSandboxizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_SANDBOXIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_SANDBOXIZABILITY_DOMAINS.map(async (entry) => {
