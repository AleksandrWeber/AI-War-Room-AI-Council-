...
import type { BackpressureizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_BACKPRESSUREIZABILITY_TABLES } from './backpressureizability-rollout.helpers.js'

const WORKSPACE_BACKPRESSUREIZABILITY_DOMAINS: Array<{
  domain: BackpressureizabilityAdminDomain
  tableName: string
  ...
  },
  {
    ...
    countQuery: (workspaceId) => ({
      sql: `
        SELECT COUNT(*)::text AS count FROM workspace_usage_limits WHERE workspace_id = $1
      `,
      params: [workspaceId],
    }),
  },
...
@Injectable()
export class BackpressureizabilityStatusService {
  ...
  async getBackpressureizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_BACKPRESSUREIZABILITY_TABLES,
    )

    return {
      existingBackpressureizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceBackpressureizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_BACKPRESSUREIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_BACKPRESSUREIZABILITY_DOMAINS.map(async (entry) => {
