...
import type { CalibratizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CALIBRATIZABILITY_TABLES } from './calibratizability-rollout.helpers.js'

const WORKSPACE_CALIBRATIZABILITY_DOMAINS: Array<{
  domain: CalibratizabilityAdminDomain
  tableName: string
...
@Injectable()
export class CalibratizabilityStatusService {
  ...
  async getCalibratizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CALIBRATIZABILITY_TABLES,
    )

    return {
      existingCalibratizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceCalibratizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CALIBRATIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CALIBRATIZABILITY_DOMAINS.map(async (entry) => {
