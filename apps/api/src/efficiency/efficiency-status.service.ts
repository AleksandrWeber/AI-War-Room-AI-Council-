...
import type { EfficiencyAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_EFFICIENCY_TABLES } from './efficiency-rollout.helpers.js'

const WORKSPACE_EFFICIENCY_DOMAINS: Array<{
  domain: EfficiencyAdminDomain
  tableName: string
...
@Injectable()
export class EfficiencyStatusService {
  ...
  async getEfficiencyTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_EFFICIENCY_TABLES,
    )

    return {
      existingEfficiencyTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceEfficiencyInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_EFFICIENCY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_EFFICIENCY_DOMAINS.map(async (entry) => {
