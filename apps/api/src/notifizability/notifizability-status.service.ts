...
import type { NotifizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_NOTIFIZABILITY_TABLES } from './notifizability-rollout.helpers.js'

const WORKSPACE_NOTIFIZABILITY_DOMAINS: Array<{
  domain: NotifizabilityAdminDomain
  tableName: string
...
@Injectable()
export class NotifizabilityStatusService {
  ...
  async getNotifizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_NOTIFIZABILITY_TABLES,
    )

    return {
      existingNotifizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceNotifizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_NOTIFIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_NOTIFIZABILITY_DOMAINS.map(async (entry) => {
