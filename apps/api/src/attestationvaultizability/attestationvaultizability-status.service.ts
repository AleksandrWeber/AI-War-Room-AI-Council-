...
import type { AttestationvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ATTESTATIONVAULTIZABILITY_TABLES } from './attestationvaultizability-rollout.helpers.js'

const WORKSPACE_ATTESTATIONVAULTIZABILITY_DOMAINS: Array<{
  domain: AttestationvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class AttestationvaultizabilityStatusService {
  ...
  async getAttestationvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ATTESTATIONVAULTIZABILITY_TABLES,
    )

    return {
      existingAttestationvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAttestationvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ATTESTATIONVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ATTESTATIONVAULTIZABILITY_DOMAINS.map(async (entry) => {
