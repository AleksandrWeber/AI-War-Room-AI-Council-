...
import type { PublishizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_PUBLISHIZABILITY_TABLES } from './publishizability-rollout.helpers.js'

const WORKSPACE_PUBLISHIZABILITY_DOMAINS: Array<{
  domain: PublishizabilityAdminDomain
  tableName: string
...
@Injectable()
export class PublishizabilityStatusService {
  ...
  async getPublishizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_PUBLISHIZABILITY_TABLES,
    )

    return {
      existingPublishizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspacePublishizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_PUBLISHIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_PUBLISHIZABILITY_DOMAINS.map(async (entry) => {
