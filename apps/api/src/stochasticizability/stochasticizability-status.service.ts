...
import type { StochasticizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_STOCHASTICIZABILITY_TABLES } from './stochasticizability-rollout.helpers.js'

const WORKSPACE_STOCHASTICIZABILITY_DOMAINS: Array<{
  domain: StochasticizabilityAdminDomain
  tableName: string
...
@Injectable()
export class StochasticizabilityStatusService {
  ...
  async getStochasticizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_STOCHASTICIZABILITY_TABLES,
    )

    return {
      existingStochasticizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceStochasticizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_STOCHASTICIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_STOCHASTICIZABILITY_DOMAINS.map(async (entry) => {
