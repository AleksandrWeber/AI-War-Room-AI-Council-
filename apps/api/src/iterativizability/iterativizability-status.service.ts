...
import type { IterativizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ITERATIVIZABILITY_TABLES } from './iterativizability-rollout.helpers.js'

const WORKSPACE_ITERATIVIZABILITY_DOMAINS: Array<{
  domain: IterativizabilityAdminDomain
  tableName: string
...
@Injectable()
export class IterativizabilityStatusService {
  ...
  async getIterativizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ITERATIVIZABILITY_TABLES,
    )

    return {
      existingIterativizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceIterativizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ITERATIVIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ITERATIVIZABILITY_DOMAINS.map(async (entry) => {
