...
import type { AutomatabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_AUTOMATABILITY_TABLES } from './automatability-rollout.helpers.js'

const WORKSPACE_AUTOMATABILITY_DOMAINS: Array<{
  domain: AutomatabilityAdminDomain
  tableName: string
...
@Injectable()
export class AutomatabilityStatusService {
  ...
  async getAutomatabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_AUTOMATABILITY_TABLES,
    )

    return {
      existingAutomatabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAutomatabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_AUTOMATABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_AUTOMATABILITY_DOMAINS.map(async (entry) => {
