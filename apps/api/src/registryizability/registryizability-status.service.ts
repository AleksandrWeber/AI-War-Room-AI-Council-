...
import type { RegistryizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_REGISTRYIZABILITY_TABLES } from './registryizability-rollout.helpers.js'

const WORKSPACE_REGISTRYIZABILITY_DOMAINS: Array<{
  domain: RegistryizabilityAdminDomain
  tableName: string
...
@Injectable()
export class RegistryizabilityStatusService {
  ...
  async getRegistryizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_REGISTRYIZABILITY_TABLES,
    )

    return {
      existingRegistryizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceRegistryizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_REGISTRYIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_REGISTRYIZABILITY_DOMAINS.map(async (entry) => {
