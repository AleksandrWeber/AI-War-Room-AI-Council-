...
import type { EvictionizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_EVICTIONIZABILITY_TABLES } from './evictionizability-rollout.helpers.js'

const WORKSPACE_EVICTIONIZABILITY_DOMAINS: Array<{
  domain: EvictionizabilityAdminDomain
  tableName: string
...
@Injectable()
export class EvictionizabilityStatusService {
  ...
  async getEvictionizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_EVICTIONIZABILITY_TABLES,
    )

    return {
      existingEvictionizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceEvictionizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_EVICTIONIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_EVICTIONIZABILITY_DOMAINS.map(async (entry) => {
