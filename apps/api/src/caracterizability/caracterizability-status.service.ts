...
import type { CaracterizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CARACTERIZABILITY_TABLES } from './caracterizability-rollout.helpers.js'

const WORKSPACE_CARACTERIZABILITY_DOMAINS: Array<{
  domain: CaracterizabilityAdminDomain
  tableName: string
...
@Injectable()
export class CaracterizabilityStatusService {
  ...
  async getCaracterizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CARACTERIZABILITY_TABLES,
    )

    return {
      existingCaracterizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceCaracterizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CARACTERIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CARACTERIZABILITY_DOMAINS.map(async (entry) => {
