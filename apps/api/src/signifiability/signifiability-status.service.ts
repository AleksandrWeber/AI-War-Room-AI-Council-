...
import type { SignifiabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_SIGNIFIABILITY_TABLES } from './signifiability-rollout.helpers.js'

const WORKSPACE_SIGNIFIABILITY_DOMAINS: Array<{
  domain: SignifiabilityAdminDomain
  tableName: string
...
@Injectable()
export class SignifiabilityStatusService {
  ...
  async getSignifiabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_SIGNIFIABILITY_TABLES,
    )

    return {
      existingSignifiabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceSignifiabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_SIGNIFIABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_SIGNIFIABILITY_DOMAINS.map(async (entry) => {
