...
import type { MergeizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_MERGEIZABILITY_TABLES } from './mergeizability-rollout.helpers.js'

const WORKSPACE_MERGEIZABILITY_DOMAINS: Array<{
  domain: MergeizabilityAdminDomain
  tableName: string
...
@Injectable()
export class MergeizabilityStatusService {
  ...
  async getMergeizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_MERGEIZABILITY_TABLES,
    )

    return {
      existingMergeizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceMergeizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_MERGEIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_MERGEIZABILITY_DOMAINS.map(async (entry) => {
