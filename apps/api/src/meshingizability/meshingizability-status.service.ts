...
import type { MeshingizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_MESHINGIZABILITY_TABLES } from './meshingizability-rollout.helpers.js'

const WORKSPACE_MESHINGIZABILITY_DOMAINS: Array<{
  domain: MeshingizabilityAdminDomain
  tableName: string
...
@Injectable()
export class MeshingizabilityStatusService {
  ...
  async getMeshingizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_MESHINGIZABILITY_TABLES,
    )

    return {
      existingMeshingizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceMeshingizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_MESHINGIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_MESHINGIZABILITY_DOMAINS.map(async (entry) => {
