...
import type { HistorizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_HISTORIZABILITY_TABLES } from './historizability-rollout.helpers.js'

const WORKSPACE_HISTORIZABILITY_DOMAINS: Array<{
  domain: HistorizabilityAdminDomain
  tableName: string
...
@Injectable()
export class HistorizabilityStatusService {
  ...
  async getHistorizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_HISTORIZABILITY_TABLES,
    )

    return {
      existingHistorizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceHistorizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_HISTORIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_HISTORIZABILITY_DOMAINS.map(async (entry) => {
