...
import type { SurvivabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_SURVIVABILITY_TABLES } from './survivability-rollout.helpers.js'

const WORKSPACE_SURVIVABILITY_DOMAINS: Array<{
  domain: SurvivabilityAdminDomain
  tableName: string
...
@Injectable()
export class SurvivabilityStatusService {
  ...
  async getSurvivabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_SURVIVABILITY_TABLES,
    )

    return {
      existingSurvivabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceSurvivabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_SURVIVABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_SURVIVABILITY_DOMAINS.map(async (entry) => {
