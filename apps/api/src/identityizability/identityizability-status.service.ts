...
import type { IdentityizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_IDENTITYIZABILITY_TABLES } from './identityizability-rollout.helpers.js'

const WORKSPACE_IDENTITYIZABILITY_DOMAINS: Array<{
  domain: IdentityizabilityAdminDomain
  tableName: string
...
@Injectable()
export class IdentityizabilityStatusService {
  ...
  async getIdentityizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_IDENTITYIZABILITY_TABLES,
    )

    return {
      existingIdentityizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceIdentityizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_IDENTITYIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_IDENTITYIZABILITY_DOMAINS.map(async (entry) => {
