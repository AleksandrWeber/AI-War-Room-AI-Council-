...
import type { EvidencejournalizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_EVIDENCEJOURNALIZABILITY_TABLES } from './evidencejournalizability-rollout.helpers.js'

const WORKSPACE_EVIDENCEJOURNALIZABILITY_DOMAINS: Array<{
  domain: EvidencejournalizabilityAdminDomain
  tableName: string
...
@Injectable()
export class EvidencejournalizabilityStatusService {
  ...
  async getEvidencejournalizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_EVIDENCEJOURNALIZABILITY_TABLES,
    )

    return {
      existingEvidencejournalizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceEvidencejournalizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_EVIDENCEJOURNALIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_EVIDENCEJOURNALIZABILITY_DOMAINS.map(async (entry) => {
