...
import type { CredibilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CREDIBILITY_TABLES } from './credibility-rollout.helpers.js'

const WORKSPACE_CREDIBILITY_DOMAINS: Array<{
  domain: CredibilityAdminDomain
  tableName: string
...
@Injectable()
export class CredibilityStatusService {
  ...
  async getCredibilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CREDIBILITY_TABLES,
    )

    return {
      existingCredibilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceCredibilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CREDIBILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CREDIBILITY_DOMAINS.map(async (entry) => {
