...
import type { DirectoryizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_DIRECTORYIZABILITY_TABLES } from './directoryizability-rollout.helpers.js'

const WORKSPACE_DIRECTORYIZABILITY_DOMAINS: Array<{
  domain: DirectoryizabilityAdminDomain
  tableName: string
...
@Injectable()
export class DirectoryizabilityStatusService {
  ...
  async getDirectoryizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_DIRECTORYIZABILITY_TABLES,
    )

    return {
      existingDirectoryizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceDirectoryizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_DIRECTORYIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_DIRECTORYIZABILITY_DOMAINS.map(async (entry) => {
