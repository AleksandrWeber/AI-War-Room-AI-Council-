...
import type { SimplicityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_SIMPLICITY_TABLES } from './simplicity-rollout.helpers.js'

const WORKSPACE_SIMPLICITY_DOMAINS: Array<{
  domain: SimplicityAdminDomain
  tableName: string
...
@Injectable()
export class SimplicityStatusService {
  ...
  async getSimplicityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_SIMPLICITY_TABLES,
    )

    return {
      existingSimplicityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceSimplicityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_SIMPLICITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_SIMPLICITY_DOMAINS.map(async (entry) => {
