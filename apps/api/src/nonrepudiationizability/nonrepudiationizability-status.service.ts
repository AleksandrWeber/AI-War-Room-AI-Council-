...
import type { NonrepudiationizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_NONREPUDIATIONIZABILITY_TABLES } from './nonrepudiationizability-rollout.helpers.js'

const WORKSPACE_NONREPUDIATIONIZABILITY_DOMAINS: Array<{
  domain: NonrepudiationizabilityAdminDomain
  tableName: string
...
@Injectable()
export class NonrepudiationizabilityStatusService {
  ...
  async getNonrepudiationizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_NONREPUDIATIONIZABILITY_TABLES,
    )

    return {
      existingNonrepudiationizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceNonrepudiationizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_NONREPUDIATIONIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_NONREPUDIATIONIZABILITY_DOMAINS.map(async (entry) => {
