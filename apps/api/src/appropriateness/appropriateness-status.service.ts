...
import type { AppropriatenessAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_APPROPRIATENESS_TABLES } from './appropriateness-rollout.helpers.js'

const WORKSPACE_APPROPRIATENESS_DOMAINS: Array<{
  domain: AppropriatenessAdminDomain
  tableName: string
...
@Injectable()
export class AppropriatenessStatusService {
  ...
  async getAppropriatenessTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_APPROPRIATENESS_TABLES,
    )

    return {
      existingAppropriatenessTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAppropriatenessInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_APPROPRIATENESS_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_APPROPRIATENESS_DOMAINS.map(async (entry) => {
