...
import type { CacheizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CACHEIZABILITY_TABLES } from './cacheizability-rollout.helpers.js'

const WORKSPACE_CACHEIZABILITY_DOMAINS: Array<{
  domain: CacheizabilityAdminDomain
  tableName: string
...
@Injectable()
export class CacheizabilityStatusService {
  ...
  async getCacheizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CACHEIZABILITY_TABLES,
    )

    return {
      existingCacheizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceCacheizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CACHEIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CACHEIZABILITY_DOMAINS.map(async (entry) => {
