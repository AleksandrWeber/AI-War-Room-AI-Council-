...
import type { HeuristizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_HEURISTIZABILITY_TABLES } from './heuristizability-rollout.helpers.js'

const WORKSPACE_HEURISTIZABILITY_DOMAINS: Array<{
  domain: HeuristizabilityAdminDomain
  tableName: string
...
@Injectable()
export class HeuristizabilityStatusService {
  ...
  async getHeuristizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_HEURISTIZABILITY_TABLES,
    )

    return {
      existingHeuristizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceHeuristizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_HEURISTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_HEURISTIZABILITY_DOMAINS.map(async (entry) => {
