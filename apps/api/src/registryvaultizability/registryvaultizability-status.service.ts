...
import type { RegistryvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_REGISTRYVAULTIZABILITY_TABLES } from './registryvaultizability-rollout.helpers.js'

const WORKSPACE_REGISTRYVAULTIZABILITY_DOMAINS: Array<{
  domain: RegistryvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class RegistryvaultizabilityStatusService {
  ...
  async getRegistryvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_REGISTRYVAULTIZABILITY_TABLES,
    )

    return {
      existingRegistryvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceRegistryvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_REGISTRYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_REGISTRYVAULTIZABILITY_DOMAINS.map(async (entry) => {
