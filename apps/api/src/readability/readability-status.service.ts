...
import type { ReadabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_READABILITY_TABLES } from './readability-rollout.helpers.js'

const WORKSPACE_READABILITY_DOMAINS: Array<{
  domain: ReadabilityAdminDomain
  tableName: string
...
@Injectable()
export class ReadabilityStatusService {
  ...
  async getReadabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_READABILITY_TABLES,
    )

    return {
      existingReadabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceReadabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_READABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_READABILITY_DOMAINS.map(async (entry) => {
