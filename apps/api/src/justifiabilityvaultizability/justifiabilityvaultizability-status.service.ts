...
import type { JustifiabilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_JUSTIFIABILITYVAULTIZABILITY_TABLES } from './justifiabilityvaultizability-rollout.helpers.js'

const WORKSPACE_JUSTIFIABILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: JustifiabilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class JustifiabilityvaultizabilityStatusService {
  ...
  async getJustifiabilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_JUSTIFIABILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingJustifiabilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceJustifiabilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_JUSTIFIABILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_JUSTIFIABILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
