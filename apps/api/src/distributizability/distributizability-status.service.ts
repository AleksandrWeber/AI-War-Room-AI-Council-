...
import type { DistributizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_DISTRIBUTIZABILITY_TABLES } from './distributizability-rollout.helpers.js'

const WORKSPACE_DISTRIBUTIZABILITY_DOMAINS: Array<{
  domain: DistributizabilityAdminDomain
  tableName: string
...
@Injectable()
export class DistributizabilityStatusService {
  ...
  async getDistributizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_DISTRIBUTIZABILITY_TABLES,
    )

    return {
      existingDistributizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceDistributizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_DISTRIBUTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_DISTRIBUTIZABILITY_DOMAINS.map(async (entry) => {
