...
import type { PropagationizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_PROPAGATIONIZABILITY_TABLES } from './propagationizability-rollout.helpers.js'

const WORKSPACE_PROPAGATIONIZABILITY_DOMAINS: Array<{
  domain: PropagationizabilityAdminDomain
  tableName: string
...
@Injectable()
export class PropagationizabilityStatusService {
  ...
  async getPropagationizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_PROPAGATIONIZABILITY_TABLES,
    )

    return {
      existingPropagationizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspacePropagationizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_PROPAGATIONIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_PROPAGATIONIZABILITY_DOMAINS.map(async (entry) => {
