...
import type { UsabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_USABILITY_TABLES } from './usability-rollout.helpers.js'

const WORKSPACE_USABILITY_DOMAINS: Array<{
  domain: UsabilityAdminDomain
  tableName: string
...
@Injectable()
export class UsabilityStatusService {
  ...
  async getUsabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_USABILITY_TABLES,
    )

    return {
      existingUsabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceUsabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_USABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_USABILITY_DOMAINS.map(async (entry) => {
