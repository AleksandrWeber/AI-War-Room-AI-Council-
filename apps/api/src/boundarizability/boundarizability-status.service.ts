...
import type { BoundarizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_BOUNDARIZABILITY_TABLES } from './boundarizability-rollout.helpers.js'

const WORKSPACE_BOUNDARIZABILITY_DOMAINS: Array<{
  domain: BoundarizabilityAdminDomain
  tableName: string
...
@Injectable()
export class BoundarizabilityStatusService {
  ...
  async getBoundarizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_BOUNDARIZABILITY_TABLES,
    )

    return {
      existingBoundarizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceBoundarizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_BOUNDARIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_BOUNDARIZABILITY_DOMAINS.map(async (entry) => {
