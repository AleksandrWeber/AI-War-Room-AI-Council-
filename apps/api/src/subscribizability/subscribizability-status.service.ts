...
import type { SubscribizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_SUBSCRIBIZABILITY_TABLES } from './subscribizability-rollout.helpers.js'

const WORKSPACE_SUBSCRIBIZABILITY_DOMAINS: Array<{
  domain: SubscribizabilityAdminDomain
  tableName: string
...
@Injectable()
export class SubscribizabilityStatusService {
  ...
  async getSubscribizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_SUBSCRIBIZABILITY_TABLES,
    )

    return {
      existingSubscribizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceSubscribizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_SUBSCRIBIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_SUBSCRIBIZABILITY_DOMAINS.map(async (entry) => {
