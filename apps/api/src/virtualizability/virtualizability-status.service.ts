...
import type { VirtualizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_VIRTUALIZABILITY_TABLES } from './virtualizability-rollout.helpers.js'

const WORKSPACE_VIRTUALIZABILITY_DOMAINS: Array<{
  domain: VirtualizabilityAdminDomain
  tableName: string
...
@Injectable()
export class VirtualizabilityStatusService {
  ...
  async getVirtualizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_VIRTUALIZABILITY_TABLES,
    )

    return {
      existingVirtualizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceVirtualizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_VIRTUALIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_VIRTUALIZABILITY_DOMAINS.map(async (entry) => {
