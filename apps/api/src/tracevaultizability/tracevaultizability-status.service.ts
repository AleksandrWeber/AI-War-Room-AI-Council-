...
import type { TracevaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_TRACEVAULTIZABILITY_TABLES } from './tracevaultizability-rollout.helpers.js'

const WORKSPACE_TRACEVAULTIZABILITY_DOMAINS: Array<{
  domain: TracevaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class TracevaultizabilityStatusService {
  ...
  async getTracevaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_TRACEVAULTIZABILITY_TABLES,
    )

    return {
      existingTracevaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceTracevaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_TRACEVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_TRACEVAULTIZABILITY_DOMAINS.map(async (entry) => {
