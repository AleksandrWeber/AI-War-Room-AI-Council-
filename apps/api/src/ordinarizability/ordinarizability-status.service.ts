...
import type { OrdinarizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ORDINARIZABILITY_TABLES } from './ordinarizability-rollout.helpers.js'

const WORKSPACE_ORDINARIZABILITY_DOMAINS: Array<{
  domain: OrdinarizabilityAdminDomain
  tableName: string
...
@Injectable()
export class OrdinarizabilityStatusService {
  ...
  async getOrdinarizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ORDINARIZABILITY_TABLES,
    )

    return {
      existingOrdinarizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceOrdinarizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ORDINARIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ORDINARIZABILITY_DOMAINS.map(async (entry) => {
