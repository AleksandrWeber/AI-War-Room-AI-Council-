...
import type { SynchronizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_SYNCHRONIZABILITY_TABLES } from './synchronizability-rollout.helpers.js'

const WORKSPACE_SYNCHRONIZABILITY_DOMAINS: Array<{
  domain: SynchronizabilityAdminDomain
  tableName: string
...
@Injectable()
export class SynchronizabilityStatusService {
  ...
  async getSynchronizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_SYNCHRONIZABILITY_TABLES,
    )

    return {
      existingSynchronizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceSynchronizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_SYNCHRONIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_SYNCHRONIZABILITY_DOMAINS.map(async (entry) => {
