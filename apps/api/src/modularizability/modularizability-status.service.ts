...
import type { ModularizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_MODULARIZABILITY_TABLES } from './modularizability-rollout.helpers.js'

const WORKSPACE_MODULARIZABILITY_DOMAINS: Array<{
  domain: ModularizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ModularizabilityStatusService {
  ...
  async getModularizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_MODULARIZABILITY_TABLES,
    )

    return {
      existingModularizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceModularizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_MODULARIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_MODULARIZABILITY_DOMAINS.map(async (entry) => {
