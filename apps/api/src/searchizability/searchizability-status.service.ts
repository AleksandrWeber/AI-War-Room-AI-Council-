...
import type { SearchizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_SEARCHIZABILITY_TABLES } from './searchizability-rollout.helpers.js'

const WORKSPACE_SEARCHIZABILITY_DOMAINS: Array<{
  domain: SearchizabilityAdminDomain
  tableName: string
...
@Injectable()
export class SearchizabilityStatusService {
  ...
  async getSearchizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_SEARCHIZABILITY_TABLES,
    )

    return {
      existingSearchizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceSearchizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_SEARCHIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_SEARCHIZABILITY_DOMAINS.map(async (entry) => {
