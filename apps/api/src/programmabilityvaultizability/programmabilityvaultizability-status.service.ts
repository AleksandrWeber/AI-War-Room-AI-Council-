...
import type { ProgrammabilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_PROGRAMMABILITYVAULTIZABILITY_TABLES } from './programmabilityvaultizability-rollout.helpers.js'

const WORKSPACE_PROGRAMMABILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: ProgrammabilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ProgrammabilityvaultizabilityStatusService {
  ...
  async getProgrammabilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_PROGRAMMABILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingProgrammabilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceProgrammabilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_PROGRAMMABILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_PROGRAMMABILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
