...
import type { HydrationizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_HYDRATIONIZABILITY_TABLES } from './hydrationizability-rollout.helpers.js'

const WORKSPACE_HYDRATIONIZABILITY_DOMAINS: Array<{
  domain: HydrationizabilityAdminDomain
  tableName: string
...
@Injectable()
export class HydrationizabilityStatusService {
  ...
  async getHydrationizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_HYDRATIONIZABILITY_TABLES,
    )

    return {
      existingHydrationizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceHydrationizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_HYDRATIONIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_HYDRATIONIZABILITY_DOMAINS.map(async (entry) => {
