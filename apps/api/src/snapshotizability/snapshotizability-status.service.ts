...
import type { SnapshotizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_SNAPSHOTIZABILITY_TABLES } from './snapshotizability-rollout.helpers.js'

const WORKSPACE_SNAPSHOTIZABILITY_DOMAINS: Array<{
  domain: SnapshotizabilityAdminDomain
  tableName: string
...
@Injectable()
export class SnapshotizabilityStatusService {
  ...
  async getSnapshotizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_SNAPSHOTIZABILITY_TABLES,
    )

    return {
      existingSnapshotizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceSnapshotizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_SNAPSHOTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_SNAPSHOTIZABILITY_DOMAINS.map(async (entry) => {
