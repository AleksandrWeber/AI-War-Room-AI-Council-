...
import type { AttesttrackizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ATTESTTRACKIZABILITY_TABLES } from './attesttrackizability-rollout.helpers.js'

const WORKSPACE_ATTESTTRACKIZABILITY_DOMAINS: Array<{
  domain: AttesttrackizabilityAdminDomain
  tableName: string
...
@Injectable()
export class AttesttrackizabilityStatusService {
  ...
  async getAttesttrackizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ATTESTTRACKIZABILITY_TABLES,
    )

    return {
      existingAttesttrackizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAttesttrackizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ATTESTTRACKIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ATTESTTRACKIZABILITY_DOMAINS.map(async (entry) => {
