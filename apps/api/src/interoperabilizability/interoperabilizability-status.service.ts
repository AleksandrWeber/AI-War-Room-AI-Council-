...
import type { InteroperabilizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_INTEROPERABILIZABILITY_TABLES } from './interoperabilizability-rollout.helpers.js'

const WORKSPACE_INTEROPERABILIZABILITY_DOMAINS: Array<{
  domain: InteroperabilizabilityAdminDomain
  tableName: string
...
@Injectable()
export class InteroperabilizabilityStatusService {
  ...
  async getInteroperabilizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_INTEROPERABILIZABILITY_TABLES,
    )

    return {
      existingInteroperabilizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceInteroperabilizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_INTEROPERABILIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_INTEROPERABILIZABILITY_DOMAINS.map(async (entry) => {
