...
import type { LoadbalancizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_LOADBALANCIZABILITY_TABLES } from './loadbalancizability-rollout.helpers.js'

const WORKSPACE_LOADBALANCIZABILITY_DOMAINS: Array<{
  domain: LoadbalancizabilityAdminDomain
  tableName: string
...
@Injectable()
export class LoadbalancizabilityStatusService {
  ...
  async getLoadbalancizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_LOADBALANCIZABILITY_TABLES,
    )

    return {
      existingLoadbalancizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceLoadbalancizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_LOADBALANCIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_LOADBALANCIZABILITY_DOMAINS.map(async (entry) => {
