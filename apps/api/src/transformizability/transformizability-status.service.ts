...
import type { TransformizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_TRANSFORMIZABILITY_TABLES } from './transformizability-rollout.helpers.js'

const WORKSPACE_TRANSFORMIZABILITY_DOMAINS: Array<{
  domain: TransformizabilityAdminDomain
  tableName: string
...
@Injectable()
export class TransformizabilityStatusService {
  ...
  async getTransformizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_TRANSFORMIZABILITY_TABLES,
    )

    return {
      existingTransformizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceTransformizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_TRANSFORMIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_TRANSFORMIZABILITY_DOMAINS.map(async (entry) => {
