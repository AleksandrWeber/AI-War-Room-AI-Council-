...
import type { AbductizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ABDUCTIZABILITY_TABLES } from './abductizability-rollout.helpers.js'

const WORKSPACE_ABDUCTIZABILITY_DOMAINS: Array<{
  domain: AbductizabilityAdminDomain
  tableName: string
...
@Injectable()
export class AbductizabilityStatusService {
  ...
  async getAbductizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ABDUCTIZABILITY_TABLES,
    )

    return {
      existingAbductizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAbductizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ABDUCTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ABDUCTIZABILITY_DOMAINS.map(async (entry) => {
