...
import type { GovernanceizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_GOVERNANCEIZABILITY_TABLES } from './governanceizability-rollout.helpers.js'

const WORKSPACE_GOVERNANCEIZABILITY_DOMAINS: Array<{
  domain: GovernanceizabilityAdminDomain
  tableName: string
...
@Injectable()
export class GovernanceizabilityStatusService {
  ...
  async getGovernanceizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_GOVERNANCEIZABILITY_TABLES,
    )

    return {
      existingGovernanceizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceGovernanceizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_GOVERNANCEIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_GOVERNANCEIZABILITY_DOMAINS.map(async (entry) => {
