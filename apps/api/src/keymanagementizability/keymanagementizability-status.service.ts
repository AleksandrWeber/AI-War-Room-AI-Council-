...
import type { KeymanagementizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_KEYMANAGEMENTIZABILITY_TABLES } from './keymanagementizability-rollout.helpers.js'

const WORKSPACE_KEYMANAGEMENTIZABILITY_DOMAINS: Array<{
  domain: KeymanagementizabilityAdminDomain
  tableName: string
...
@Injectable()
export class KeymanagementizabilityStatusService {
  ...
  async getKeymanagementizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_KEYMANAGEMENTIZABILITY_TABLES,
    )

    return {
      existingKeymanagementizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceKeymanagementizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_KEYMANAGEMENTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_KEYMANAGEMENTIZABILITY_DOMAINS.map(async (entry) => {
