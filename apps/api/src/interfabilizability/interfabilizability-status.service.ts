...
import type { InterfabilizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_INTERFABILIZABILITY_TABLES } from './interfabilizability-rollout.helpers.js'

const WORKSPACE_INTERFABILIZABILITY_DOMAINS: Array<{
  domain: InterfabilizabilityAdminDomain
  tableName: string
...
@Injectable()
export class InterfabilizabilityStatusService {
  ...
  async getInterfabilizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_INTERFABILIZABILITY_TABLES,
    )

    return {
      existingInterfabilizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceInterfabilizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_INTERFABILIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_INTERFABILIZABILITY_DOMAINS.map(async (entry) => {
