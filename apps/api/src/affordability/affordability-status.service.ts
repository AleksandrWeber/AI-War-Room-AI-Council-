...
import type { AffordabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_AFFORDABILITY_TABLES } from './affordability-rollout.helpers.js'

const WORKSPACE_AFFORDABILITY_DOMAINS: Array<{
  domain: AffordabilityAdminDomain
  tableName: string
...
@Injectable()
export class AffordabilityStatusService {
  ...
  async getAffordabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_AFFORDABILITY_TABLES,
    )

    return {
      existingAffordabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAffordabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_AFFORDABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_AFFORDABILITY_DOMAINS.map(async (entry) => {
