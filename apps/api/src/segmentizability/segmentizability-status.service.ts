...
import type { SegmentizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_SEGMENTIZABILITY_TABLES } from './segmentizability-rollout.helpers.js'

const WORKSPACE_SEGMENTIZABILITY_DOMAINS: Array<{
  domain: SegmentizabilityAdminDomain
  tableName: string
...
@Injectable()
export class SegmentizabilityStatusService {
  ...
  async getSegmentizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_SEGMENTIZABILITY_TABLES,
    )

    return {
      existingSegmentizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceSegmentizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_SEGMENTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_SEGMENTIZABILITY_DOMAINS.map(async (entry) => {
