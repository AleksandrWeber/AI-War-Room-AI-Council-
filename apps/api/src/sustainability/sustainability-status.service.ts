...
import type { SustainabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_SUSTAINABILITY_TABLES } from './sustainability-rollout.helpers.js'

const WORKSPACE_SUSTAINABILITY_DOMAINS: Array<{
  domain: SustainabilityAdminDomain
  tableName: string
...
@Injectable()
export class SustainabilityStatusService {
  ...
  async getSustainabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_SUSTAINABILITY_TABLES,
    )

    return {
      existingSustainabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceSustainabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_SUSTAINABILITY_DOMAINS.flatMap(
    ...
    return Promise.all(
      WORKSPACE_SUSTAINABILITY_DOMAINS.map(async (entry) => {
