...
import type { DependabilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_DEPENDABILITYVAULTIZABILITY_TABLES } from './dependabilityvaultizability-rollout.helpers.js'

const WORKSPACE_DEPENDABILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: DependabilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class DependabilityvaultizabilityStatusService {
  ...
  async getDependabilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_DEPENDABILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingDependabilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceDependabilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_DEPENDABILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_DEPENDABILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
