...
import type { AxiologizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_AXIOLOGIZABILITY_TABLES } from './axiologizability-rollout.helpers.js'

const WORKSPACE_AXIOLOGIZABILITY_DOMAINS: Array<{
  domain: AxiologizabilityAdminDomain
  tableName: string
...
@Injectable()
export class AxiologizabilityStatusService {
  ...
  async getAxiologizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_AXIOLOGIZABILITY_TABLES,
    )

    return {
      existingAxiologizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAxiologizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_AXIOLOGIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_AXIOLOGIZABILITY_DOMAINS.map(async (entry) => {
