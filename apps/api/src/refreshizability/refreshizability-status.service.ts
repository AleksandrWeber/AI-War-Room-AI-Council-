...
import type { RefreshizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_REFRESHIZABILITY_TABLES } from './refreshizability-rollout.helpers.js'

const WORKSPACE_REFRESHIZABILITY_DOMAINS: Array<{
  domain: RefreshizabilityAdminDomain
  tableName: string
...
@Injectable()
export class RefreshizabilityStatusService {
  ...
  async getRefreshizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_REFRESHIZABILITY_TABLES,
    )

    return {
      existingRefreshizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceRefreshizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_REFRESHIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_REFRESHIZABILITY_DOMAINS.map(async (entry) => {
