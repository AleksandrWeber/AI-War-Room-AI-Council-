...
import type { SecretmanagementizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_SECRETMANAGEMENTIZABILITY_TABLES } from './secretmanagementizability-rollout.helpers.js'

const WORKSPACE_SECRETMANAGEMENTIZABILITY_DOMAINS: Array<{
  domain: SecretmanagementizabilityAdminDomain
  tableName: string
...
@Injectable()
export class SecretmanagementizabilityStatusService {
  ...
  async getSecretmanagementizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_SECRETMANAGEMENTIZABILITY_TABLES,
    )

    return {
      existingSecretmanagementizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceSecretmanagementizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_SECRETMANAGEMENTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_SECRETMANAGEMENTIZABILITY_DOMAINS.map(async (entry) => {
