...
import type { ShardingizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_SHARDINGIZABILITY_TABLES } from './shardingizability-rollout.helpers.js'

const WORKSPACE_SHARDINGIZABILITY_DOMAINS: Array<{
  domain: ShardingizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ShardingizabilityStatusService {
  ...
  async getShardingizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_SHARDINGIZABILITY_TABLES,
    )

    return {
      existingShardingizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceShardingizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_SHARDINGIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_SHARDINGIZABILITY_DOMAINS.map(async (entry) => {
