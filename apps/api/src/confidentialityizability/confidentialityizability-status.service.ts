...
import type { ConfidentialityizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CONFIDENTIALITYIZABILITY_TABLES } from './confidentialityizability-rollout.helpers.js'

const WORKSPACE_CONFIDENTIALITYIZABILITY_DOMAINS: Array<{
  domain: ConfidentialityizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ConfidentialityizabilityStatusService {
  ...
  async getConfidentialityizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CONFIDENTIALITYIZABILITY_TABLES,
    )

    return {
      existingConfidentialityizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceConfidentialityizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CONFIDENTIALITYIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CONFIDENTIALITYIZABILITY_DOMAINS.map(async (entry) => {
