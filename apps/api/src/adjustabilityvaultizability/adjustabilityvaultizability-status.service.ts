...
import type { AdjustabilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ADJUSTABILITYVAULTIZABILITY_TABLES } from './adjustabilityvaultizability-rollout.helpers.js'

const WORKSPACE_ADJUSTABILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: AdjustabilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class AdjustabilityvaultizabilityStatusService {
  ...
  async getAdjustabilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ADJUSTABILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingAdjustabilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAdjustabilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ADJUSTABILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ADJUSTABILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
