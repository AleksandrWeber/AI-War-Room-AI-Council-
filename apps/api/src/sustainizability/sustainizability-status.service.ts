...
import type { SustainizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_SUSTAINIZABILITY_TABLES } from './sustainizability-rollout.helpers.js'

const WORKSPACE_SUSTAINIZABILITY_DOMAINS: Array<{
  domain: SustainizabilityAdminDomain
  tableName: string
...
@Injectable()
export class SustainizabilityStatusService {
  ...
  async getSustainizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_SUSTAINIZABILITY_TABLES,
    )

    return {
      existingSustainizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceSustainizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_SUSTAINIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_SUSTAINIZABILITY_DOMAINS.map(async (entry) => {
