...
import type { AuthenticityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_AUTHENTICITY_TABLES } from './authenticity-rollout.helpers.js'

const WORKSPACE_AUTHENTICITY_DOMAINS: Array<{
  domain: AuthenticityAdminDomain
  tableName: string
...
@Injectable()
export class AuthenticityStatusService {
  ...
  async getAuthenticityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_AUTHENTICITY_TABLES,
    )

    return {
      existingAuthenticityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAuthenticityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_AUTHENTICITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_AUTHENTICITY_DOMAINS.map(async (entry) => {
