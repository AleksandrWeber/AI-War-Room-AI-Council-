...
import type { OrchestrabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ORCHESTRABILITY_TABLES } from './orchestrability-rollout.helpers.js'

const WORKSPACE_ORCHESTRABILITY_DOMAINS: Array<{
  domain: OrchestrabilityAdminDomain
  tableName: string
...
@Injectable()
export class OrchestrabilityStatusService {
  ...
  async getOrchestrabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ORCHESTRABILITY_TABLES,
    )

    return {
      existingOrchestrabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceOrchestrabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ORCHESTRABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ORCHESTRABILITY_DOMAINS.map(async (entry) => {
