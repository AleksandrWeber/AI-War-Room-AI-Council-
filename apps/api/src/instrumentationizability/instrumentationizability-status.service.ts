...
import type { InstrumentationizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_INSTRUMENTATIONIZABILITY_TABLES } from './instrumentationizability-rollout.helpers.js'

const WORKSPACE_INSTRUMENTATIONIZABILITY_DOMAINS: Array<{
  domain: InstrumentationizabilityAdminDomain
  tableName: string
...
@Injectable()
export class InstrumentationizabilityStatusService {
  ...
  async getInstrumentationizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_INSTRUMENTATIONIZABILITY_TABLES,
    )

    return {
      existingInstrumentationizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceInstrumentationizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_INSTRUMENTATIONIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_INSTRUMENTATIONIZABILITY_DOMAINS.map(async (entry) => {
