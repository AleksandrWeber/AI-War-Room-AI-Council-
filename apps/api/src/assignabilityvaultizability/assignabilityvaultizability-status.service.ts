...
import type { AssignabilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ASSIGNABILITYVAULTIZABILITY_TABLES } from './assignabilityvaultizability-rollout.helpers.js'

const WORKSPACE_ASSIGNABILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: AssignabilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class AssignabilityvaultizabilityStatusService {
  ...
  async getAssignabilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ASSIGNABILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingAssignabilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAssignabilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ASSIGNABILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ASSIGNABILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
