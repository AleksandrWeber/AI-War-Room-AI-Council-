...
import type { HermeneutizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_HERMENEUTIZABILITY_TABLES } from './hermeneutizability-rollout.helpers.js'

const WORKSPACE_HERMENEUTIZABILITY_DOMAINS: Array<{
  domain: HermeneutizabilityAdminDomain
  tableName: string
...
@Injectable()
export class HermeneutizabilityStatusService {
  ...
  async getHermeneutizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_HERMENEUTIZABILITY_TABLES,
    )

    return {
      existingHermeneutizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceHermeneutizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_HERMENEUTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_HERMENEUTIZABILITY_DOMAINS.map(async (entry) => {
