...
import type { ProgrammabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_PROGRAMMABILITY_TABLES } from './programmability-rollout.helpers.js'

const WORKSPACE_PROGRAMMABILITY_DOMAINS: Array<{
  domain: ProgrammabilityAdminDomain
  tableName: string
...
@Injectable()
export class ProgrammabilityStatusService {
  ...
  async getProgrammabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_PROGRAMMABILITY_TABLES,
    )

    return {
      existingProgrammabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceProgrammabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_PROGRAMMABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_PROGRAMMABILITY_DOMAINS.map(async (entry) => {
