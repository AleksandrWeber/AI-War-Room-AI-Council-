...
import type { NotarizationizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_NOTARIZATIONIZABILITY_TABLES } from './notarizationizability-rollout.helpers.js'

const WORKSPACE_NOTARIZATIONIZABILITY_DOMAINS: Array<{
  domain: NotarizationizabilityAdminDomain
  tableName: string
...
@Injectable()
export class NotarizationizabilityStatusService {
  ...
  async getNotarizationizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_NOTARIZATIONIZABILITY_TABLES,
    )

    return {
      existingNotarizationizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceNotarizationizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_NOTARIZATIONIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_NOTARIZATIONIZABILITY_DOMAINS.map(async (entry) => {
