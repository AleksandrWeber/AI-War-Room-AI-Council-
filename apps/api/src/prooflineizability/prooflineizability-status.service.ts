...
import type { ProoflineizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_PROOFLINEIZABILITY_TABLES } from './prooflineizability-rollout.helpers.js'

const WORKSPACE_PROOFLINEIZABILITY_DOMAINS: Array<{
  domain: ProoflineizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ProoflineizabilityStatusService {
  ...
  async getProoflineizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_PROOFLINEIZABILITY_TABLES,
    )

    return {
      existingProoflineizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceProoflineizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_PROOFLINEIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_PROOFLINEIZABILITY_DOMAINS.map(async (entry) => {
