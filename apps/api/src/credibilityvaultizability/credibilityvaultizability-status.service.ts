...
import type { CredibilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CREDIBILITYVAULTIZABILITY_TABLES } from './credibilityvaultizability-rollout.helpers.js'

const WORKSPACE_CREDIBILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: CredibilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class CredibilityvaultizabilityStatusService {
  ...
  async getCredibilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CREDIBILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingCredibilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceCredibilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CREDIBILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CREDIBILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
