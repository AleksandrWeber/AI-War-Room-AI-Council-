...
import type { ChainofcustodyizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CHAINOFCUSTODYIZABILITY_TABLES } from './chainofcustodyizability-rollout.helpers.js'

const WORKSPACE_CHAINOFCUSTODYIZABILITY_DOMAINS: Array<{
  domain: ChainofcustodyizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ChainofcustodyizabilityStatusService {
  ...
  async getChainofcustodyizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CHAINOFCUSTODYIZABILITY_TABLES,
    )

    return {
      existingChainofcustodyizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceChainofcustodyizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CHAINOFCUSTODYIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CHAINOFCUSTODYIZABILITY_DOMAINS.map(async (entry) => {
