...
import type { ManageabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_MANAGEABILITY_TABLES } from './manageability-rollout.helpers.js'

const WORKSPACE_MANAGEABILITY_DOMAINS: Array<{
  domain: ManageabilityAdminDomain
  tableName: string
...
@Injectable()
export class ManageabilityStatusService {
  ...
  async getManageabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_MANAGEABILITY_TABLES,
    )

    return {
      existingManageabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceManageabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_MANAGEABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_MANAGEABILITY_DOMAINS.map(async (entry) => {
