...
import type { ExtensibilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_EXTENSIBILITY_TABLES } from './extensibility-rollout.helpers.js'

const WORKSPACE_EXTENSIBILITY_DOMAINS: Array<{
  domain: ExtensibilityAdminDomain
  tableName: string
...
@Injectable()
export class ExtensibilityStatusService {
  ...
  async getExtensibilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_EXTENSIBILITY_TABLES,
    )

    return {
      existingExtensibilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceExtensibilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_EXTENSIBILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_EXTENSIBILITY_DOMAINS.map(async (entry) => {
