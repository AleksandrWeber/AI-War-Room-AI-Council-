...
import type { PartitionizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_PARTITIONIZABILITY_TABLES } from './partitionizability-rollout.helpers.js'

const WORKSPACE_PARTITIONIZABILITY_DOMAINS: Array<{
  domain: PartitionizabilityAdminDomain
  tableName: string
...
@Injectable()
export class PartitionizabilityStatusService {
  ...
  async getPartitionizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_PARTITIONIZABILITY_TABLES,
    )

    return {
      existingPartitionizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspacePartitionizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_PARTITIONIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_PARTITIONIZABILITY_DOMAINS.map(async (entry) => {
