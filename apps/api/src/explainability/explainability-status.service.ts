...
import type { ExplainabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_EXPLAINABILITY_TABLES } from './explainability-rollout.helpers.js'

const WORKSPACE_EXPLAINABILITY_DOMAINS: Array<{
  domain: ExplainabilityAdminDomain
  tableName: string
...
@Injectable()
export class ExplainabilityStatusService {
  ...
  async getExplainabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_EXPLAINABILITY_TABLES,
    )

    return {
      existingExplainabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceExplainabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_EXPLAINABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_EXPLAINABILITY_DOMAINS.map(async (entry) => {
