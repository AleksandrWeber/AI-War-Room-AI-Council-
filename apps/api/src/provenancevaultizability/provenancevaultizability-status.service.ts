...
import type { ProvenancevaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_PROVENANCEVAULTIZABILITY_TABLES } from './provenancevaultizability-rollout.helpers.js'

const WORKSPACE_PROVENANCEVAULTIZABILITY_DOMAINS: Array<{
  domain: ProvenancevaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ProvenancevaultizabilityStatusService {
  ...
  async getProvenancevaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_PROVENANCEVAULTIZABILITY_TABLES,
    )

    return {
      existingProvenancevaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceProvenancevaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_PROVENANCEVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_PROVENANCEVAULTIZABILITY_DOMAINS.map(async (entry) => {
