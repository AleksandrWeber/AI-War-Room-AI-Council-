...
import type { AuthorizationizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_AUTHORIZATIONIZABILITY_TABLES } from './authorizationizability-rollout.helpers.js'

const WORKSPACE_AUTHORIZATIONIZABILITY_DOMAINS: Array<{
  domain: AuthorizationizabilityAdminDomain
  tableName: string
...
@Injectable()
export class AuthorizationizabilityStatusService {
  ...
  async getAuthorizationizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_AUTHORIZATIONIZABILITY_TABLES,
    )

    return {
      existingAuthorizationizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAuthorizationizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_AUTHORIZATIONIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_AUTHORIZATIONIZABILITY_DOMAINS.map(async (entry) => {
