...
import type { NarratabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_NARRATABILITY_TABLES } from './narratability-rollout.helpers.js'

const WORKSPACE_NARRATABILITY_DOMAINS: Array<{
  domain: NarratabilityAdminDomain
  tableName: string
...
@Injectable()
export class NarratabilityStatusService {
  ...
  async getNarratabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_NARRATABILITY_TABLES,
    )

    return {
      existingNarratabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceNarratabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_NARRATABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_NARRATABILITY_DOMAINS.map(async (entry) => {
