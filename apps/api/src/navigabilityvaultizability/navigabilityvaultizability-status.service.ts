...
import type { NavigabilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_NAVIGABILITYVAULTIZABILITY_TABLES } from './navigabilityvaultizability-rollout.helpers.js'

const WORKSPACE_NAVIGABILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: NavigabilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class NavigabilityvaultizabilityStatusService {
  ...
  async getNavigabilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_NAVIGABILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingNavigabilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceNavigabilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_NAVIGABILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_NAVIGABILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
