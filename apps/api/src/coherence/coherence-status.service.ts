...
import type { CoherenceAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_COHERENCE_TABLES } from './coherence-rollout.helpers.js'

const WORKSPACE_COHERENCE_DOMAINS: Array<{
  domain: CoherenceAdminDomain
  tableName: string
...
@Injectable()
export class CoherenceStatusService {
  ...
  async getCoherenceTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_COHERENCE_TABLES,
    )

    return {
      existingCoherenceTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceCoherenceInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_COHERENCE_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_COHERENCE_DOMAINS.map(async (entry) => {
