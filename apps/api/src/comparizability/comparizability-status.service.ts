...
import type { ComparizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_COMPARIZABILITY_TABLES } from './comparizability-rollout.helpers.js'

const WORKSPACE_COMPARIZABILITY_DOMAINS: Array<{
  domain: ComparizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ComparizabilityStatusService {
  ...
  async getComparizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_COMPARIZABILITY_TABLES,
    )

    return {
      existingComparizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceComparizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_COMPARIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_COMPARIZABILITY_DOMAINS.map(async (entry) => {
