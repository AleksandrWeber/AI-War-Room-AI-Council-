...
import type { OrchestrabilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ORCHESTRABILITYVAULTIZABILITY_TABLES } from './orchestrabilityvaultizability-rollout.helpers.js'

const WORKSPACE_ORCHESTRABILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: OrchestrabilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class OrchestrabilityvaultizabilityStatusService {
  ...
  async getOrchestrabilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ORCHESTRABILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingOrchestrabilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceOrchestrabilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ORCHESTRABILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ORCHESTRABILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
