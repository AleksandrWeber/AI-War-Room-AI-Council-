...
import type { VerificationvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_VERIFICATIONVAULTIZABILITY_TABLES } from './verificationvaultizability-rollout.helpers.js'

const WORKSPACE_VERIFICATIONVAULTIZABILITY_DOMAINS: Array<{
  domain: VerificationvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class VerificationvaultizabilityStatusService {
  ...
  async getVerificationvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_VERIFICATIONVAULTIZABILITY_TABLES,
    )

    return {
      existingVerificationvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceVerificationvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_VERIFICATIONVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_VERIFICATIONVAULTIZABILITY_DOMAINS.map(async (entry) => {
