...
import type { AsynchronizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ASYNCHRONIZABILITY_TABLES } from './asynchronizability-rollout.helpers.js'

const WORKSPACE_ASYNCHRONIZABILITY_DOMAINS: Array<{
  domain: AsynchronizabilityAdminDomain
  tableName: string
...
@Injectable()
export class AsynchronizabilityStatusService {
  ...
  async getAsynchronizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ASYNCHRONIZABILITY_TABLES,
    )

    return {
      existingAsynchronizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAsynchronizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ASYNCHRONIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ASYNCHRONIZABILITY_DOMAINS.map(async (entry) => {
