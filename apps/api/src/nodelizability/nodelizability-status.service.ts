...
import type { NodelizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_NODELIZABILITY_TABLES } from './nodelizability-rollout.helpers.js'

const WORKSPACE_NODELIZABILITY_DOMAINS: Array<{
  domain: NodelizabilityAdminDomain
  tableName: string
...
@Injectable()
export class NodelizabilityStatusService {
  ...
  async getNodelizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_NODELIZABILITY_TABLES,
    )

    return {
      existingNodelizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceNodelizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_NODELIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_NODELIZABILITY_DOMAINS.map(async (entry) => {
