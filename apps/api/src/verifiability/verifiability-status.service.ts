...
import type { VerifiabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_VERIFIABILITY_TABLES } from './verifiability-rollout.helpers.js'

const WORKSPACE_VERIFIABILITY_DOMAINS: Array<{
  domain: VerifiabilityAdminDomain
  tableName: string
...
@Injectable()
export class VerifiabilityStatusService {
  ...
  async getVerifiabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_VERIFIABILITY_TABLES,
    )

    return {
      existingVerifiabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceVerifiabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_VERIFIABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_VERIFIABILITY_DOMAINS.map(async (entry) => {
