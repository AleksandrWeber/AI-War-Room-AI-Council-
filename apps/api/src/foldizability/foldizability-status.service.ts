...
import type { FoldizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_FOLDIZABILITY_TABLES } from './foldizability-rollout.helpers.js'

const WORKSPACE_FOLDIZABILITY_DOMAINS: Array<{
  domain: FoldizabilityAdminDomain
  tableName: string
...
@Injectable()
export class FoldizabilityStatusService {
  ...
  async getFoldizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_FOLDIZABILITY_TABLES,
    )

    return {
      existingFoldizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceFoldizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_FOLDIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_FOLDIZABILITY_DOMAINS.map(async (entry) => {
