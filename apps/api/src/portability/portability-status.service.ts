...
import type { PortabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_PORTABILITY_TABLES } from './portability-rollout.helpers.js'

const WORKSPACE_PORTABILITY_DOMAINS: Array<{
  domain: PortabilityAdminDomain
  tableName: string
...
@Injectable()
export class PortabilityStatusService {
  ...
  async getPortabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_PORTABILITY_TABLES,
    )

    return {
      existingPortabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspacePortabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_PORTABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_PORTABILITY_DOMAINS.map(async (entry) => {
