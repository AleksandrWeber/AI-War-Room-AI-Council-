...
import type { ProofregistryizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_PROOFREGISTRYIZABILITY_TABLES } from './proofregistryizability-rollout.helpers.js'

const WORKSPACE_PROOFREGISTRYIZABILITY_DOMAINS: Array<{
  domain: ProofregistryizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ProofregistryizabilityStatusService {
  ...
  async getProofregistryizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_PROOFREGISTRYIZABILITY_TABLES,
    )

    return {
      existingProofregistryizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceProofregistryizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_PROOFREGISTRYIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_PROOFREGISTRYIZABILITY_DOMAINS.map(async (entry) => {
