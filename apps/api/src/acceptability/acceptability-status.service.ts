...
import type { AcceptabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ACCEPTABILITY_TABLES } from './acceptability-rollout.helpers.js'

const WORKSPACE_ACCEPTABILITY_DOMAINS: Array<{
  domain: AcceptabilityAdminDomain
  tableName: string
...
@Injectable()
export class AcceptabilityStatusService {
  ...
  async getAcceptabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ACCEPTABILITY_TABLES,
    )

    return {
      existingAcceptabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAcceptabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ACCEPTABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ACCEPTABILITY_DOMAINS.map(async (entry) => {
