...
import type { AttributabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ATTRIBUTABILITY_TABLES } from './attributability-rollout.helpers.js'

const WORKSPACE_ATTRIBUTABILITY_DOMAINS: Array<{
  domain: AttributabilityAdminDomain
  tableName: string
...
@Injectable()
export class AttributabilityStatusService {
  ...
  async getAttributabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ATTRIBUTABILITY_TABLES,
    )

    return {
      existingAttributabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAttributabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ATTRIBUTABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ATTRIBUTABILITY_DOMAINS.map(async (entry) => {
