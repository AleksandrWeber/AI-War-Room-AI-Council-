...
import type { AlertabilizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ALERTABILIZABILITY_TABLES } from './alertabilizability-rollout.helpers.js'

const WORKSPACE_ALERTABILIZABILITY_DOMAINS: Array<{
  domain: AlertabilizabilityAdminDomain
  tableName: string
...
@Injectable()
export class AlertabilizabilityStatusService {
  ...
  async getAlertabilizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ALERTABILIZABILITY_TABLES,
    )

    return {
      existingAlertabilizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAlertabilizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ALERTABILIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ALERTABILIZABILITY_DOMAINS.map(async (entry) => {
