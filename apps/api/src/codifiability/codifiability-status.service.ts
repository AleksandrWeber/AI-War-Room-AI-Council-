...
import type { CodifiabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CODIFIABILITY_TABLES } from './codifiability-rollout.helpers.js'

const WORKSPACE_CODIFIABILITY_DOMAINS: Array<{
  domain: CodifiabilityAdminDomain
  tableName: string
...
@Injectable()
export class CodifiabilityStatusService {
  ...
  async getCodifiabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CODIFIABILITY_TABLES,
    )

    return {
      existingCodifiabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceCodifiabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CODIFIABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CODIFIABILITY_DOMAINS.map(async (entry) => {
