...
import type { ScalabilizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_SCALABILIZABILITY_TABLES } from './scalabilizability-rollout.helpers.js'

const WORKSPACE_SCALABILIZABILITY_DOMAINS: Array<{
  domain: ScalabilizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ScalabilizabilityStatusService {
  ...
  async getScalabilizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_SCALABILIZABILITY_TABLES,
    )

    return {
      existingScalabilizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceScalabilizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_SCALABILIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_SCALABILIZABILITY_DOMAINS.map(async (entry) => {
