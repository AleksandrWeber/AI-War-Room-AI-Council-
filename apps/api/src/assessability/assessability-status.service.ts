...
import type { AssessabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ASSESSABILITY_TABLES } from './assessability-rollout.helpers.js'

const WORKSPACE_ASSESSABILITY_DOMAINS: Array<{
  domain: AssessabilityAdminDomain
  tableName: string
  ...
  },
  {
    ...
    countQuery: (workspaceId) => ({
      sql: `
        SELECT COUNT(*)::text AS count FROM model_health_events WHERE workspace_id = $1
      `,
      params: [workspaceId],
    }),
  },
...
@Injectable()
export class AssessabilityStatusService {
  ...
  async getAssessabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ASSESSABILITY_TABLES,
    )

    return {
      existingAssessabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAssessabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ASSESSABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ASSESSABILITY_DOMAINS.map(async (entry) => {
