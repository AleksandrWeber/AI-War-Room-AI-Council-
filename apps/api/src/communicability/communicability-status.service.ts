...
import type { CommunicabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_COMMUNICABILITY_TABLES } from './communicability-rollout.helpers.js'

const WORKSPACE_COMMUNICABILITY_DOMAINS: Array<{
  domain: CommunicabilityAdminDomain
  tableName: string
...
@Injectable()
export class CommunicabilityStatusService {
  ...
  async getCommunicabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_COMMUNICABILITY_TABLES,
    )

    return {
      existingCommunicabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceCommunicabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_COMMUNICABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_COMMUNICABILITY_DOMAINS.map(async (entry) => {
