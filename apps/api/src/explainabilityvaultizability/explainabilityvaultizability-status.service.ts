...
import type { ExplainabilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_EXPLAINABILITYVAULTIZABILITY_TABLES } from './explainabilityvaultizability-rollout.helpers.js'

const WORKSPACE_EXPLAINABILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: ExplainabilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ExplainabilityvaultizabilityStatusService {
  ...
  async getExplainabilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_EXPLAINABILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingExplainabilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceExplainabilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_EXPLAINABILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_EXPLAINABILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
