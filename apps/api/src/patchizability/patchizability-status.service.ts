...
import type { PatchizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_PATCHIZABILITY_TABLES } from './patchizability-rollout.helpers.js'

const WORKSPACE_PATCHIZABILITY_DOMAINS: Array<{
  domain: PatchizabilityAdminDomain
  tableName: string
...
@Injectable()
export class PatchizabilityStatusService {
  ...
  async getPatchizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_PATCHIZABILITY_TABLES,
    )

    return {
      existingPatchizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspacePatchizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_PATCHIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_PATCHIZABILITY_DOMAINS.map(async (entry) => {
