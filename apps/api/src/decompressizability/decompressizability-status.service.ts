...
import type { DecompressizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_DECOMPRESSIZABILITY_TABLES } from './decompressizability-rollout.helpers.js'

const WORKSPACE_DECOMPRESSIZABILITY_DOMAINS: Array<{
  domain: DecompressizabilityAdminDomain
  tableName: string
...
@Injectable()
export class DecompressizabilityStatusService {
  ...
  async getDecompressizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_DECOMPRESSIZABILITY_TABLES,
    )

    return {
      existingDecompressizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceDecompressizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_DECOMPRESSIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_DECOMPRESSIZABILITY_DOMAINS.map(async (entry) => {
