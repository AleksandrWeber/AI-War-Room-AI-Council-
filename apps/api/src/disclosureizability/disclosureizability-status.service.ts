...
import type { DisclosureizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_DISCLOSUREIZABILITY_TABLES } from './disclosureizability-rollout.helpers.js'

const WORKSPACE_DISCLOSUREIZABILITY_DOMAINS: Array<{
  domain: DisclosureizabilityAdminDomain
  tableName: string
...
@Injectable()
export class DisclosureizabilityStatusService {
  ...
  async getDisclosureizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_DISCLOSUREIZABILITY_TABLES,
    )

    return {
      existingDisclosureizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceDisclosureizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_DISCLOSUREIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_DISCLOSUREIZABILITY_DOMAINS.map(async (entry) => {
