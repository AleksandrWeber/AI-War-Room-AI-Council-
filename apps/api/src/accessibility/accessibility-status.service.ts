...
import type { AccessibilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ACCESSIBILITY_TABLES } from './accessibility-rollout.helpers.js'

const WORKSPACE_ACCESSIBILITY_DOMAINS: Array<{
  domain: AccessibilityAdminDomain
  tableName: string
...
export class AccessibilityStatusService {
  ...
  async getAccessibilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ACCESSIBILITY_TABLES,
    )

    return {
      existingAccessibilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAccessibilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ACCESSIBILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ACCESSIBILITY_DOMAINS.map(async (entry) => {
