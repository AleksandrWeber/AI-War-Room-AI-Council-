...
import type { AccountabilityizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ACCOUNTABILITYIZABILITY_TABLES } from './accountabilityizability-rollout.helpers.js'

const WORKSPACE_ACCOUNTABILITYIZABILITY_DOMAINS: Array<{
  domain: AccountabilityizabilityAdminDomain
  tableName: string
...
@Injectable()
export class AccountabilityizabilityStatusService {
  ...
  async getAccountabilityizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ACCOUNTABILITYIZABILITY_TABLES,
    )

    return {
      existingAccountabilityizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAccountabilityizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ACCOUNTABILITYIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ACCOUNTABILITYIZABILITY_DOMAINS.map(async (entry) => {
