...
import type { CertifiabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CERTIFIABILITY_TABLES } from './certifiability-rollout.helpers.js'

const WORKSPACE_CERTIFIABILITY_DOMAINS: Array<{
  domain: CertifiabilityAdminDomain
  tableName: string
...
@Injectable()
export class CertifiabilityStatusService {
  ...
  async getCertifiabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CERTIFIABILITY_TABLES,
    )

    return {
      existingCertifiabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceCertifiabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CERTIFIABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CERTIFIABILITY_DOMAINS.map(async (entry) => {
