...
import type { BufferizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_BUFFERIZABILITY_TABLES } from './bufferizability-rollout.helpers.js'

const WORKSPACE_BUFFERIZABILITY_DOMAINS: Array<{
  domain: BufferizabilityAdminDomain
  tableName: string
...
@Injectable()
export class BufferizabilityStatusService {
  ...
  async getBufferizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_BUFFERIZABILITY_TABLES,
    )

    return {
      existingBufferizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceBufferizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_BUFFERIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_BUFFERIZABILITY_DOMAINS.map(async (entry) => {
