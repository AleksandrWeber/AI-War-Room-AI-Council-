...
import type { MaterializationizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_MATERIALIZATIONIZABILITY_TABLES } from './materializationizability-rollout.helpers.js'

const WORKSPACE_MATERIALIZATIONIZABILITY_DOMAINS: Array<{
  domain: MaterializationizabilityAdminDomain
  tableName: string
...
@Injectable()
export class MaterializationizabilityStatusService {
  ...
  async getMaterializationizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_MATERIALIZATIONIZABILITY_TABLES,
    )

    return {
      existingMaterializationizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceMaterializationizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_MATERIALIZATIONIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_MATERIALIZATIONIZABILITY_DOMAINS.map(async (entry) => {
