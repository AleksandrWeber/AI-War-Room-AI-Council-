...
import type { TroubleshootizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_TROUBLESHOOTIZABILITY_TABLES } from './troubleshootizability-rollout.helpers.js'

const WORKSPACE_TROUBLESHOOTIZABILITY_DOMAINS: Array<{
  domain: TroubleshootizabilityAdminDomain
  tableName: string
...
@Injectable()
export class TroubleshootizabilityStatusService {
  ...
  async getTroubleshootizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_TROUBLESHOOTIZABILITY_TABLES,
    )

    return {
      existingTroubleshootizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceTroubleshootizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_TROUBLESHOOTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_TROUBLESHOOTIZABILITY_DOMAINS.map(async (entry) => {
