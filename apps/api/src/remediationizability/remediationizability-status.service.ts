...
import type { RemediationizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_REMEDIATIONIZABILITY_TABLES } from './remediationizability-rollout.helpers.js'

const WORKSPACE_REMEDIATIONIZABILITY_DOMAINS: Array<{
  domain: RemediationizabilityAdminDomain
  tableName: string
...
@Injectable()
export class RemediationizabilityStatusService {
  ...
  async getRemediationizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_REMEDIATIONIZABILITY_TABLES,
    )

    return {
      existingRemediationizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceRemediationizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_REMEDIATIONIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_REMEDIATIONIZABILITY_DOMAINS.map(async (entry) => {
