...
import type { RetryizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ABATCHIZABILITY_TABLES } from './retryizability-rollout.helpers.js'

const WORKSPACE_ABATCHIZABILITY_DOMAINS: Array<{
  domain: RetryizabilityAdminDomain
  tableName: string
...
@Injectable()
export class RetryizabilityStatusService {
  ...
  async getRetryizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ABATCHIZABILITY_TABLES,
    )

    return {
      existingRetryizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceRetryizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ABATCHIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ABATCHIZABILITY_DOMAINS.map(async (entry) => {
