...
import type { LocatabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_LOCATABILITY_TABLES } from './locatability-rollout.helpers.js'

const WORKSPACE_LOCATABILITY_DOMAINS: Array<{
  domain: LocatabilityAdminDomain
  tableName: string
...
@Injectable()
export class LocatabilityStatusService {
  ...
  async getLocatabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_LOCATABILITY_TABLES,
    )

    return {
      existingLocatabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceLocatabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_LOCATABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_LOCATABILITY_DOMAINS.map(async (entry) => {
