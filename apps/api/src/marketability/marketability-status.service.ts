...
import type { MarketabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_MARKETABILITY_TABLES } from './marketability-rollout.helpers.js'

const WORKSPACE_MARKETABILITY_DOMAINS: Array<{
  domain: MarketabilityAdminDomain
  tableName: string
...
@Injectable()
export class MarketabilityStatusService {
  ...
  async getMarketabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_MARKETABILITY_TABLES,
    )

    return {
      existingMarketabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceMarketabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_MARKETABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_MARKETABILITY_DOMAINS.map(async (entry) => {
