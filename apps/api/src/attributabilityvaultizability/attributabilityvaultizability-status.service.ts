...
import type { AttributabilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ATTRIBUTABILITYVAULTIZABILITY_TABLES } from './attributabilityvaultizability-rollout.helpers.js'

const WORKSPACE_ATTRIBUTABILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: AttributabilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class AttributabilityvaultizabilityStatusService {
  ...
  async getAttributabilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ATTRIBUTABILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingAttributabilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAttributabilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ATTRIBUTABILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ATTRIBUTABILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
