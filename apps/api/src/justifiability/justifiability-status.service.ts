...
import type { JustifiabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_JUSTIFIABILITY_TABLES } from './justifiability-rollout.helpers.js'

const WORKSPACE_JUSTIFIABILITY_DOMAINS: Array<{
  domain: JustifiabilityAdminDomain
  tableName: string
...
@Injectable()
export class JustifiabilityStatusService {
  ...
  async getJustifiabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_JUSTIFIABILITY_TABLES,
    )

    return {
      existingJustifiabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceJustifiabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_JUSTIFIABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_JUSTIFIABILITY_DOMAINS.map(async (entry) => {
