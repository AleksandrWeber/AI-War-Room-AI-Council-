...
import type { PolicyizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_POLICYIZABILITY_TABLES } from './policyizability-rollout.helpers.js'

const WORKSPACE_POLICYIZABILITY_DOMAINS: Array<{
  domain: PolicyizabilityAdminDomain
  tableName: string
...
@Injectable()
export class PolicyizabilityStatusService {
  ...
  async getPolicyizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_POLICYIZABILITY_TABLES,
    )

    return {
      existingPolicyizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspacePolicyizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_POLICYIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_POLICYIZABILITY_DOMAINS.map(async (entry) => {
