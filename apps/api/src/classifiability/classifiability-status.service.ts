...
import type { ClassifiabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CLASSIFIABILITY_TABLES } from './classifiability-rollout.helpers.js'

const WORKSPACE_CLASSIFIABILITY_DOMAINS: Array<{
  domain: ClassifiabilityAdminDomain
  tableName: string
...
@Injectable()
export class ClassifiabilityStatusService {
  ...
  async getClassifiabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CLASSIFIABILITY_TABLES,
    )

    return {
      existingClassifiabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceClassifiabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CLASSIFIABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CLASSIFIABILITY_DOMAINS.map(async (entry) => {
