...
import type { ThreatizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_THREATIZABILITY_TABLES } from './threatizability-rollout.helpers.js'

const WORKSPACE_THREATIZABILITY_DOMAINS: Array<{
  domain: ThreatizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ThreatizabilityStatusService {
  ...
  async getThreatizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_THREATIZABILITY_TABLES,
    )

    return {
      existingThreatizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceThreatizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_THREATIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_THREATIZABILITY_DOMAINS.map(async (entry) => {
