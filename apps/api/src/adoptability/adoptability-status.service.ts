...
import type { AdoptabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ADOPTABILITY_TABLES } from './adoptability-rollout.helpers.js'

const WORKSPACE_ADOPTABILITY_DOMAINS: Array<{
  domain: AdoptabilityAdminDomain
  tableName: string
...
@Injectable()
export class AdoptabilityStatusService {
  ...
  async getAdoptabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ADOPTABILITY_TABLES,
    )

    return {
      existingAdoptabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAdoptabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ADOPTABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ADOPTABILITY_DOMAINS.map(async (entry) => {
