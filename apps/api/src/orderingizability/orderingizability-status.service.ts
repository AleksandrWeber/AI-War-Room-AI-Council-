...
import type { OrderingizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ORDERINGIZABILITY_TABLES } from './orderingizability-rollout.helpers.js'

const WORKSPACE_ORDERINGIZABILITY_DOMAINS: Array<{
  domain: OrderingizabilityAdminDomain
  tableName: string
...
@Injectable()
export class OrderingizabilityStatusService {
  ...
  async getOrderingizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ORDERINGIZABILITY_TABLES,
    )

    return {
      existingOrderingizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceOrderingizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ORDERINGIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ORDERINGIZABILITY_DOMAINS.map(async (entry) => {
