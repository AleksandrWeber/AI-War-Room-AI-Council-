...
import type { WarrantabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_WARRANTABILITY_TABLES } from './warrantability-rollout.helpers.js'

const WORKSPACE_WARRANTABILITY_DOMAINS: Array<{
  domain: WarrantabilityAdminDomain
  tableName: string
...
@Injectable()
export class WarrantabilityStatusService {
  ...
  async getWarrantabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_WARRANTABILITY_TABLES,
    )

    return {
      existingWarrantabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceWarrantabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_WARRANTABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_WARRANTABILITY_DOMAINS.map(async (entry) => {
