...
import type { ReleasizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_RELEASIZABILITY_TABLES } from './releasizability-rollout.helpers.js'

const WORKSPACE_RELEASIZABILITY_DOMAINS: Array<{
  domain: ReleasizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ReleasizabilityStatusService {
  ...
  async getReleasizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_RELEASIZABILITY_TABLES,
    )

    return {
      existingReleasizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceReleasizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_RELEASIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_RELEASIZABILITY_DOMAINS.map(async (entry) => {
