...
import type { PrivacyizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_PRIVACYIZABILITY_TABLES } from './privacyizability-rollout.helpers.js'

const WORKSPACE_PRIVACYIZABILITY_DOMAINS: Array<{
  domain: PrivacyizabilityAdminDomain
  tableName: string
...
@Injectable()
export class PrivacyizabilityStatusService {
  ...
  async getPrivacyizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_PRIVACYIZABILITY_TABLES,
    )

    return {
      existingPrivacyizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspacePrivacyizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_PRIVACYIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_PRIVACYIZABILITY_DOMAINS.map(async (entry) => {
