...
import type { ModifiabilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_MODIFIABILITYVAULTIZABILITY_TABLES } from './modifiabilityvaultizability-rollout.helpers.js'

const WORKSPACE_MODIFIABILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: ModifiabilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ModifiabilityvaultizabilityStatusService {
  ...
  async getModifiabilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_MODIFIABILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingModifiabilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceModifiabilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_MODIFIABILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_MODIFIABILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
