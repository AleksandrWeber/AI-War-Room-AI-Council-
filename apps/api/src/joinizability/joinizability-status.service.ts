...
import type { JoinizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_JOINIZABILITY_TABLES } from './joinizability-rollout.helpers.js'

const WORKSPACE_JOINIZABILITY_DOMAINS: Array<{
  domain: JoinizabilityAdminDomain
  tableName: string
...
@Injectable()
export class JoinizabilityStatusService {
  ...
  async getJoinizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_JOINIZABILITY_TABLES,
    )

    return {
      existingJoinizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceJoinizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_JOINIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_JOINIZABILITY_DOMAINS.map(async (entry) => {
