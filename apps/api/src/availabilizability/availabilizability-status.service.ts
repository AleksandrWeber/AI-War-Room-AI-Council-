...
import type { AvailabilizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_AVAILABILIZABILITY_TABLES } from './availabilizability-rollout.helpers.js'

const WORKSPACE_AVAILABILIZABILITY_DOMAINS: Array<{
  domain: AvailabilizabilityAdminDomain
  tableName: string
...
@Injectable()
export class AvailabilizabilityStatusService {
  ...
  async getAvailabilizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_AVAILABILIZABILITY_TABLES,
    )

    return {
      existingAvailabilizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAvailabilizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_AVAILABILIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_AVAILABILIZABILITY_DOMAINS.map(async (entry) => {
