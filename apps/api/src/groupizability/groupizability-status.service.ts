...
import type { GroupizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_GROUPIZABILITY_TABLES } from './groupizability-rollout.helpers.js'

const WORKSPACE_GROUPIZABILITY_DOMAINS: Array<{
  domain: GroupizabilityAdminDomain
  tableName: string
...
@Injectable()
export class GroupizabilityStatusService {
  ...
  async getGroupizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_GROUPIZABILITY_TABLES,
    )

    return {
      existingGroupizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceGroupizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_GROUPIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_GROUPIZABILITY_DOMAINS.map(async (entry) => {
