...
import type { FilterizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_FILTERIZABILITY_TABLES } from './filterizability-rollout.helpers.js'

const WORKSPACE_FILTERIZABILITY_DOMAINS: Array<{
  domain: FilterizabilityAdminDomain
  tableName: string
...
@Injectable()
export class FilterizabilityStatusService {
  ...
  async getFilterizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_FILTERIZABILITY_TABLES,
    )

    return {
      existingFilterizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceFilterizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_FILTERIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_FILTERIZABILITY_DOMAINS.map(async (entry) => {
