...
import type { AuditjournalizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_AUDITJOURNALIZABILITY_TABLES } from './auditjournalizability-rollout.helpers.js'

const WORKSPACE_AUDITJOURNALIZABILITY_DOMAINS: Array<{
  domain: AuditjournalizabilityAdminDomain
  tableName: string
...
@Injectable()
export class AuditjournalizabilityStatusService {
  ...
  async getAuditjournalizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_AUDITJOURNALIZABILITY_TABLES,
    )

    return {
      existingAuditjournalizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAuditjournalizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_AUDITJOURNALIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_AUDITJOURNALIZABILITY_DOMAINS.map(async (entry) => {
