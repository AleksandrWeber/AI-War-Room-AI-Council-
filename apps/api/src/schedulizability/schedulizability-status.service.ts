...
import type { SchedulizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_SCHEDULIZABILITY_TABLES } from './schedulizability-rollout.helpers.js'

const WORKSPACE_SCHEDULIZABILITY_DOMAINS: Array<{
  domain: SchedulizabilityAdminDomain
  tableName: string
...
@Injectable()
export class SchedulizabilityStatusService {
  ...
  async getSchedulizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_SCHEDULIZABILITY_TABLES,
    )

    return {
      existingSchedulizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceSchedulizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_SCHEDULIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_SCHEDULIZABILITY_DOMAINS.map(async (entry) => {
