...
import type { EnforcementizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ENFORCEMENTIZABILITY_TABLES } from './enforcementizability-rollout.helpers.js'

const WORKSPACE_ENFORCEMENTIZABILITY_DOMAINS: Array<{
  domain: EnforcementizabilityAdminDomain
  tableName: string
...
@Injectable()
export class EnforcementizabilityStatusService {
  ...
  async getEnforcementizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ENFORCEMENTIZABILITY_TABLES,
    )

    return {
      existingEnforcementizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceEnforcementizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ENFORCEMENTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ENFORCEMENTIZABILITY_DOMAINS.map(async (entry) => {
