...
import type { PrefetchizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_PREFETCHIZABILITY_TABLES } from './prefetchizability-rollout.helpers.js'

const WORKSPACE_PREFETCHIZABILITY_DOMAINS: Array<{
  domain: PrefetchizabilityAdminDomain
  tableName: string
...
@Injectable()
export class PrefetchizabilityStatusService {
  ...
  async getPrefetchizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_PREFETCHIZABILITY_TABLES,
    )

    return {
      existingPrefetchizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspacePrefetchizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_PREFETCHIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_PREFETCHIZABILITY_DOMAINS.map(async (entry) => {
