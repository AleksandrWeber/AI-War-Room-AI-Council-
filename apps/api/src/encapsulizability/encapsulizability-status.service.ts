...
import type { EncapsulizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ENCAPSULIZABILITY_TABLES } from './encapsulizability-rollout.helpers.js'

const WORKSPACE_ENCAPSULIZABILITY_DOMAINS: Array<{
  domain: EncapsulizabilityAdminDomain
  tableName: string
...
@Injectable()
export class EncapsulizabilityStatusService {
  ...
  async getEncapsulizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ENCAPSULIZABILITY_TABLES,
    )

    return {
      existingEncapsulizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceEncapsulizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ENCAPSULIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ENCAPSULIZABILITY_DOMAINS.map(async (entry) => {
