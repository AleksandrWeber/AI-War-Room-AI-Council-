...
import type { ProgressiveizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_PROGRESSIVEIZABILITY_TABLES } from './progressiveizability-rollout.helpers.js'

const WORKSPACE_PROGRESSIVEIZABILITY_DOMAINS: Array<{
  domain: ProgressiveizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ProgressiveizabilityStatusService {
  ...
  async getProgressiveizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_PROGRESSIVEIZABILITY_TABLES,
    )

    return {
      existingProgressiveizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceProgressiveizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_PROGRESSIVEIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_PROGRESSIVEIZABILITY_DOMAINS.map(async (entry) => {
