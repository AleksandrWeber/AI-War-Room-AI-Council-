...
import type { ExportizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_EXPORTIZABILITY_TABLES } from './exportizability-rollout.helpers.js'

const WORKSPACE_EXPORTIZABILITY_DOMAINS: Array<{
  domain: ExportizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ExportizabilityStatusService {
  ...
  async getExportizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_EXPORTIZABILITY_TABLES,
    )

    return {
      existingExportizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceExportizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_EXPORTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_EXPORTIZABILITY_DOMAINS.map(async (entry) => {
