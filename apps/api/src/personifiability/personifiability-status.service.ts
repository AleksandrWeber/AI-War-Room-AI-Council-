...
import type { PersonifiabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_PERSONIFIABILITY_TABLES } from './personifiability-rollout.helpers.js'

const WORKSPACE_PERSONIFIABILITY_DOMAINS: Array<{
  domain: PersonifiabilityAdminDomain
  tableName: string
...
@Injectable()
export class PersonifiabilityStatusService {
  ...
  async getPersonifiabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_PERSONIFIABILITY_TABLES,
    )

    return {
      existingPersonifiabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspacePersonifiabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_PERSONIFIABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_PERSONIFIABILITY_DOMAINS.map(async (entry) => {
