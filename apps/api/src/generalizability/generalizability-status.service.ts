...
import type { GeneralizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_GENERALIZABILITY_TABLES } from './generalizability-rollout.helpers.js'

const WORKSPACE_GENERALIZABILITY_DOMAINS: Array<{
  domain: GeneralizabilityAdminDomain
  tableName: string
...
@Injectable()
export class GeneralizabilityStatusService {
  ...
  async getGeneralizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_GENERALIZABILITY_TABLES,
    )

    return {
      existingGeneralizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceGeneralizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_GENERALIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_GENERALIZABILITY_DOMAINS.map(async (entry) => {
