...
import type { ScanizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_SCANIZABILITY_TABLES } from './scanizability-rollout.helpers.js'

const WORKSPACE_SCANIZABILITY_DOMAINS: Array<{
  domain: ScanizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ScanizabilityStatusService {
  ...
  async getScanizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_SCANIZABILITY_TABLES,
    )

    return {
      existingScanizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceScanizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_SCANIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_SCANIZABILITY_DOMAINS.map(async (entry) => {
