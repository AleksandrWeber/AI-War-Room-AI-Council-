...
import type { PipeliningizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_PIPELININGIZABILITY_TABLES } from './pipeliningizability-rollout.helpers.js'

const WORKSPACE_PIPELININGIZABILITY_DOMAINS: Array<{
  domain: PipeliningizabilityAdminDomain
  tableName: string
...
@Injectable()
export class PipeliningizabilityStatusService {
  ...
  async getPipeliningizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_PIPELININGIZABILITY_TABLES,
    )

    return {
      existingPipeliningizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspacePipeliningizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_PIPELININGIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_PIPELININGIZABILITY_DOMAINS.map(async (entry) => {
