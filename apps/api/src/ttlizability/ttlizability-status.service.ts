...
import type { TtlizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_TTLIZABILITY_TABLES } from './ttlizability-rollout.helpers.js'

const WORKSPACE_TTLIZABILITY_DOMAINS: Array<{
  domain: TtlizabilityAdminDomain
  tableName: string
...
@Injectable()
export class TtlizabilityStatusService {
  ...
  async getTtlizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_TTLIZABILITY_TABLES,
    )

    return {
      existingTtlizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceTtlizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_TTLIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_TTLIZABILITY_DOMAINS.map(async (entry) => {
