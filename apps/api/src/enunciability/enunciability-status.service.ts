...
import type { EnunciabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ENUNCIABILITY_TABLES } from './enunciability-rollout.helpers.js'

const WORKSPACE_ENUNCIABILITY_DOMAINS: Array<{
  domain: EnunciabilityAdminDomain
  tableName: string
...
@Injectable()
export class EnunciabilityStatusService {
  ...
  async getEnunciabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ENUNCIABILITY_TABLES,
    )

    return {
      existingEnunciabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceEnunciabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ENUNCIABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ENUNCIABILITY_DOMAINS.map(async (entry) => {
