...
import type { LexicalizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_LEXICALIZABILITY_TABLES } from './lexicalizability-rollout.helpers.js'

const WORKSPACE_LEXICALIZABILITY_DOMAINS: Array<{
  domain: LexicalizabilityAdminDomain
  tableName: string
...
@Injectable()
export class LexicalizabilityStatusService {
  ...
  async getLexicalizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_LEXICALIZABILITY_TABLES,
    )

    return {
      existingLexicalizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceLexicalizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_LEXICALIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_LEXICALIZABILITY_DOMAINS.map(async (entry) => {
