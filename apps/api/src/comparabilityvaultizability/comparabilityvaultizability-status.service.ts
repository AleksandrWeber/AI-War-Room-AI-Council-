...
import type { ComparabilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_COMPARABILITYVAULTIZABILITY_TABLES } from './comparabilityvaultizability-rollout.helpers.js'

const WORKSPACE_COMPARABILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: ComparabilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ComparabilityvaultizabilityStatusService {
  ...
  async getComparabilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_COMPARABILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingComparabilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceComparabilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_COMPARABILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_COMPARABILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
