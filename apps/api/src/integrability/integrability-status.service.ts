...
import type { IntegrabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_INTEGRABILITY_TABLES } from './integrability-rollout.helpers.js'

const WORKSPACE_INTEGRABILITY_DOMAINS: Array<{
  domain: IntegrabilityAdminDomain
  tableName: string
...
@Injectable()
export class IntegrabilityStatusService {
  ...
  async getIntegrabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_INTEGRABILITY_TABLES,
    )

    return {
      existingIntegrabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceIntegrabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_INTEGRABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_INTEGRABILITY_DOMAINS.map(async (entry) => {
