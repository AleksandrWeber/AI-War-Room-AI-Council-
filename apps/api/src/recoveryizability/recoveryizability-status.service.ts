...
import type { RecoveryizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_RECOVERYIZABILITY_TABLES } from './recoveryizability-rollout.helpers.js'

const WORKSPACE_RECOVERYIZABILITY_DOMAINS: Array<{
  domain: RecoveryizabilityAdminDomain
  tableName: string
...
@Injectable()
export class RecoveryizabilityStatusService {
  ...
  async getRecoveryizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_RECOVERYIZABILITY_TABLES,
    )

    return {
      existingRecoveryizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceRecoveryizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_RECOVERYIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_RECOVERYIZABILITY_DOMAINS.map(async (entry) => {
