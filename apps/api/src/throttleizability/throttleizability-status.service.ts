...
import type { ThrottleizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_THROTTLEIZABILITY_TABLES } from './throttleizability-rollout.helpers.js'

const WORKSPACE_THROTTLEIZABILITY_DOMAINS: Array<{
  domain: ThrottleizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ThrottleizabilityStatusService {
  ...
  async getThrottleizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_THROTTLEIZABILITY_TABLES,
    )

    return {
      existingThrottleizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceThrottleizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_THROTTLEIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_THROTTLEIZABILITY_DOMAINS.map(async (entry) => {
