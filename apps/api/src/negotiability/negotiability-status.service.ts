...
import type { NegotiabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_NEGOTIABILITY_TABLES } from './negotiability-rollout.helpers.js'

const WORKSPACE_NEGOTIABILITY_DOMAINS: Array<{
  domain: NegotiabilityAdminDomain
  tableName: string
...
@Injectable()
export class NegotiabilityStatusService {
  ...
  async getNegotiabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_NEGOTIABILITY_TABLES,
    )

    return {
      existingNegotiabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceNegotiabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_NEGOTIABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_NEGOTIABILITY_DOMAINS.map(async (entry) => {
