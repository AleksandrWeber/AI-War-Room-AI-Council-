...
import type { ResilientizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_RESILIENTIZABILITY_TABLES } from './resilientizability-rollout.helpers.js'

const WORKSPACE_RESILIENTIZABILITY_DOMAINS: Array<{
  domain: ResilientizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ResilientizabilityStatusService {
  ...
  async getResilientizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_RESILIENTIZABILITY_TABLES,
    )

    return {
      existingResilientizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceResilientizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_RESILIENTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_RESILIENTIZABILITY_DOMAINS.map(async (entry) => {
