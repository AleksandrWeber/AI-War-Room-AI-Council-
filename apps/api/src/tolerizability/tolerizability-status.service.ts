...
import type { TolerizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_TOLERIZABILITY_TABLES } from './tolerizability-rollout.helpers.js'

const WORKSPACE_TOLERIZABILITY_DOMAINS: Array<{
  domain: TolerizabilityAdminDomain
  tableName: string
...
@Injectable()
export class TolerizabilityStatusService {
  ...
  async getTolerizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_TOLERIZABILITY_TABLES,
    )

    return {
      existingTolerizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceTolerizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_TOLERIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_TOLERIZABILITY_DOMAINS.map(async (entry) => {
