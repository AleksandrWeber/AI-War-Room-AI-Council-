...
import type { OptimizationAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_OPTIMIZATION_TABLES } from './optimization-rollout.helpers.js'

const WORKSPACE_OPTIMIZATION_DOMAINS: Array<{
  domain: OptimizationAdminDomain
  tableName: string
...
@Injectable()
export class OptimizationStatusService {
  ...
  async getOptimizationTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_OPTIMIZATION_TABLES,
    )

    return {
      existingOptimizationTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceOptimizationInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_OPTIMIZATION_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_OPTIMIZATION_DOMAINS.map(async (entry) => {
