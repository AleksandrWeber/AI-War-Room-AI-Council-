...
import type { LinkabilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_LINKABILITYVAULTIZABILITY_TABLES } from './linkabilityvaultizability-rollout.helpers.js'

const WORKSPACE_LINKABILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: LinkabilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class LinkabilityvaultizabilityStatusService {
  ...
  async getLinkabilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_LINKABILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingLinkabilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceLinkabilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_LINKABILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_LINKABILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
