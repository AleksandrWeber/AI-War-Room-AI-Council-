...
import type { ComposabilizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_COMPOSABILIZABILITY_TABLES } from './composabilizability-rollout.helpers.js'

const WORKSPACE_COMPOSABILIZABILITY_DOMAINS: Array<{
  domain: ComposabilizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ComposabilizabilityStatusService {
  ...
  async getComposabilizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_COMPOSABILIZABILITY_TABLES,
    )

    return {
      existingComposabilizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceComposabilizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_COMPOSABILIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_COMPOSABILIZABILITY_DOMAINS.map(async (entry) => {
