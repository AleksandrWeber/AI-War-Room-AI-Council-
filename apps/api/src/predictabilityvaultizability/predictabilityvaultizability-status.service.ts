...
import type { PredictabilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_PREDICTABILITYVAULTIZABILITY_TABLES } from './predictabilityvaultizability-rollout.helpers.js'

const WORKSPACE_PREDICTABILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: PredictabilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class PredictabilityvaultizabilityStatusService {
  ...
  async getPredictabilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_PREDICTABILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingPredictabilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspacePredictabilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_PREDICTABILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_PREDICTABILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
