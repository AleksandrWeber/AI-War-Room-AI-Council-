...
import type { NackizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_NACKIZABILITY_TABLES } from './nackizability-rollout.helpers.js'

const WORKSPACE_NACKIZABILITY_DOMAINS: Array<{
  domain: NackizabilityAdminDomain
  tableName: string
...
@Injectable()
export class NackizabilityStatusService {
  ...
  async getNackizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_NACKIZABILITY_TABLES,
    )

    return {
      existingNackizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceNackizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_NACKIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_NACKIZABILITY_DOMAINS.map(async (entry) => {
