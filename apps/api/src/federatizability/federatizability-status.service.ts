...
import type { FederatizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_FEDERATIZABILITY_TABLES } from './federatizability-rollout.helpers.js'

const WORKSPACE_FEDERATIZABILITY_DOMAINS: Array<{
  domain: FederatizabilityAdminDomain
  tableName: string
...
@Injectable()
export class FederatizabilityStatusService {
  ...
  async getFederatizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_FEDERATIZABILITY_TABLES,
    )

    return {
      existingFederatizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceFederatizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_FEDERATIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_FEDERATIZABILITY_DOMAINS.map(async (entry) => {
