...
import type { ReferencabilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_REFERENCABILITYVAULTIZABILITY_TABLES } from './referencabilityvaultizability-rollout.helpers.js'

const WORKSPACE_REFERENCABILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: ReferencabilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ReferencabilityvaultizabilityStatusService {
  ...
  async getReferencabilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_REFERENCABILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingReferencabilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceReferencabilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_REFERENCABILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_REFERENCABILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
