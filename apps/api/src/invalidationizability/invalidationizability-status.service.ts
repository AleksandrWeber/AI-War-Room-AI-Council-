...
import type { InvalidationizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_INVALIDATIONIZABILITY_TABLES } from './invalidationizability-rollout.helpers.js'

const WORKSPACE_INVALIDATIONIZABILITY_DOMAINS: Array<{
  domain: InvalidationizabilityAdminDomain
  tableName: string
...
@Injectable()
export class InvalidationizabilityStatusService {
  ...
  async getInvalidationizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_INVALIDATIONIZABILITY_TABLES,
    )

    return {
      existingInvalidationizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceInvalidationizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_INVALIDATIONIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_INVALIDATIONIZABILITY_DOMAINS.map(async (entry) => {
