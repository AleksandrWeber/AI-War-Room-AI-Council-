...
import type { AttestationAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ATTESTATION_TABLES } from './attestation-rollout.helpers.js'

const WORKSPACE_ATTESTATION_DOMAINS: Array<{
  domain: AttestationAdminDomain
  tableName: string
...
@Injectable()
export class AttestationStatusService {
  ...
  async getAttestationTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ATTESTATION_TABLES,
    )

    return {
      existingAttestationTableCount: existingTables.size,
      existingTables,
      ...
      providerCredentialsTableExists: existingTables.has('workspace_provider_credentials'),
    }
  }

  async getWorkspaceAttestationInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ATTESTATION_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ATTESTATION_DOMAINS.map(async (entry) => {
