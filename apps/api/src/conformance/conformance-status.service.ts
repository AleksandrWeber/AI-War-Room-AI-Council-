...
import type { ConformanceAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CONFORMANCE_TABLES } from './conformance-rollout.helpers.js'

const WORKSPACE_CONFORMANCE_DOMAINS: Array<{
  domain: ConformanceAdminDomain
  tableName: string
...
@Injectable()
export class ConformanceStatusService {
  ...
  async getConformanceTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CONFORMANCE_TABLES,
    )

    return {
      existingConformanceTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceConformanceInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CONFORMANCE_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CONFORMANCE_DOMAINS.map(async (entry) => {
