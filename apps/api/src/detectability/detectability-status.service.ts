...
import type { DetectabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_DETECTABILITY_TABLES } from './detectability-rollout.helpers.js'

const WORKSPACE_DETECTABILITY_DOMAINS: Array<{
  domain: DetectabilityAdminDomain
  tableName: string
...
@Injectable()
export class DetectabilityStatusService {
  ...
  async getDetectabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_DETECTABILITY_TABLES,
    )

    return {
      existingDetectabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceDetectabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_DETECTABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_DETECTABILITY_DOMAINS.map(async (entry) => {
