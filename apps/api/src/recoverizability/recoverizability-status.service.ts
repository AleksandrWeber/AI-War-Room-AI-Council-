...
import type { RecoverizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_RECOVERIZABILITY_TABLES } from './recoverizability-rollout.helpers.js'

const WORKSPACE_RECOVERIZABILITY_DOMAINS: Array<{
  domain: RecoverizabilityAdminDomain
  tableName: string
...
@Injectable()
export class RecoverizabilityStatusService {
  ...
  async getRecoverizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_RECOVERIZABILITY_TABLES,
    )

    return {
      existingRecoverizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceRecoverizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_RECOVERIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_RECOVERIZABILITY_DOMAINS.map(async (entry) => {
