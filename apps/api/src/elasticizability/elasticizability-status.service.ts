...
import type { ElasticizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ELASTICIZABILITY_TABLES } from './elasticizability-rollout.helpers.js'

const WORKSPACE_ELASTICIZABILITY_DOMAINS: Array<{
  domain: ElasticizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ElasticizabilityStatusService {
  ...
  async getElasticizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ELASTICIZABILITY_TABLES,
    )

    return {
      existingElasticizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceElasticizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ELASTICIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ELASTICIZABILITY_DOMAINS.map(async (entry) => {
