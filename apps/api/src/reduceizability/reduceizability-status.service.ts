...
import type { ReduceizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_REDUCEIZABILITY_TABLES } from './reduceizability-rollout.helpers.js'

const WORKSPACE_REDUCEIZABILITY_DOMAINS: Array<{
  domain: ReduceizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ReduceizabilityStatusService {
  ...
  async getReduceizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_REDUCEIZABILITY_TABLES,
    )

    return {
      existingReduceizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceReduceizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_REDUCEIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_REDUCEIZABILITY_DOMAINS.map(async (entry) => {
