...
import type { CompliancechainizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_COMPLIANCECHAINIZABILITY_TABLES } from './compliancechainizability-rollout.helpers.js'

const WORKSPACE_COMPLIANCECHAINIZABILITY_DOMAINS: Array<{
  domain: CompliancechainizabilityAdminDomain
  tableName: string
...
@Injectable()
export class CompliancechainizabilityStatusService {
  ...
  async getCompliancechainizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_COMPLIANCECHAINIZABILITY_TABLES,
    )

    return {
      existingCompliancechainizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceCompliancechainizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_COMPLIANCECHAINIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_COMPLIANCECHAINIZABILITY_DOMAINS.map(async (entry) => {
