...
import type { SubstantiabilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_SUBSTANTIABILITYVAULTIZABILITY_TABLES } from './substantiabilityvaultizability-rollout.helpers.js'

const WORKSPACE_SUBSTANTIABILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: SubstantiabilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class SubstantiabilityvaultizabilityStatusService {
  ...
  async getSubstantiabilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_SUBSTANTIABILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingSubstantiabilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceSubstantiabilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_SUBSTANTIABILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_SUBSTANTIABILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
