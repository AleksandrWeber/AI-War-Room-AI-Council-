...
import type { SemiotizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_SEMIOTIZABILITY_TABLES } from './semiotizability-rollout.helpers.js'

const WORKSPACE_SEMIOTIZABILITY_DOMAINS: Array<{
  domain: SemiotizabilityAdminDomain
  tableName: string
...
@Injectable()
export class SemiotizabilityStatusService {
  ...
  async getSemiotizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_SEMIOTIZABILITY_TABLES,
    )

    return {
      existingSemiotizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceSemiotizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_SEMIOTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_SEMIOTIZABILITY_DOMAINS.map(async (entry) => {
