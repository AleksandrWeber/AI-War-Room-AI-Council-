...
import type { HermeticizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_HERMETICIZABILITY_TABLES } from './hermeticizability-rollout.helpers.js'

const WORKSPACE_HERMETICIZABILITY_DOMAINS: Array<{
  domain: HermeticizabilityAdminDomain
  tableName: string
...
@Injectable()
export class HermeticizabilityStatusService {
  ...
  async getHermeticizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_HERMETICIZABILITY_TABLES,
    )

    return {
      existingHermeticizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceHermeticizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_HERMETICIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_HERMETICIZABILITY_DOMAINS.map(async (entry) => {
