...
import type { ReferencizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_REFERENCIZABILITY_TABLES } from './referencizability-rollout.helpers.js'

const WORKSPACE_REFERENCIZABILITY_DOMAINS: Array<{
  domain: ReferencizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ReferencizabilityStatusService {
  ...
  async getReferencizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_REFERENCIZABILITY_TABLES,
    )

    return {
      existingReferencizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceReferencizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_REFERENCIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_REFERENCIZABILITY_DOMAINS.map(async (entry) => {
