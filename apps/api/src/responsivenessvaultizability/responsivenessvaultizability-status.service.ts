...
import type { ResponsivenessvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_RESPONSIVENESSVAULTIZABILITY_TABLES } from './responsivenessvaultizability-rollout.helpers.js'

const WORKSPACE_RESPONSIVENESSVAULTIZABILITY_DOMAINS: Array<{
  domain: ResponsivenessvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ResponsivenessvaultizabilityStatusService {
  ...
  async getResponsivenessvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_RESPONSIVENESSVAULTIZABILITY_TABLES,
    )

    return {
      existingResponsivenessvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceResponsivenessvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_RESPONSIVENESSVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_RESPONSIVENESSVAULTIZABILITY_DOMAINS.map(async (entry) => {
