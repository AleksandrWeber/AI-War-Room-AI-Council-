...
import type { CompressizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_COMPRESSIZABILITY_TABLES } from './compressizability-rollout.helpers.js'

const WORKSPACE_COMPRESSIZABILITY_DOMAINS: Array<{
  domain: CompressizabilityAdminDomain
  tableName: string
...
@Injectable()
export class CompressizabilityStatusService {
  ...
  async getCompressizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_COMPRESSIZABILITY_TABLES,
    )

    return {
      existingCompressizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceCompressizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_COMPRESSIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_COMPRESSIZABILITY_DOMAINS.map(async (entry) => {
