...
import type { AssignabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ASSIGNABILITY_TABLES } from './assignability-rollout.helpers.js'

const WORKSPACE_ASSIGNABILITY_DOMAINS: Array<{
  domain: AssignabilityAdminDomain
  tableName: string
...
@Injectable()
export class AssignabilityStatusService {
  ...
  async getAssignabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ASSIGNABILITY_TABLES,
    )

    return {
      existingAssignabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAssignabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ASSIGNABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ASSIGNABILITY_DOMAINS.map(async (entry) => {
