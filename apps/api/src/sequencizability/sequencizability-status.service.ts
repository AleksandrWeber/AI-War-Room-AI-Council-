...
import type { SequencizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_SEQUENCIZABILITY_TABLES } from './sequencizability-rollout.helpers.js'

const WORKSPACE_SEQUENCIZABILITY_DOMAINS: Array<{
  domain: SequencizabilityAdminDomain
  tableName: string
...
@Injectable()
export class SequencizabilityStatusService {
  ...
  async getSequencizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_SEQUENCIZABILITY_TABLES,
    )

    return {
      existingSequencizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceSequencizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_SEQUENCIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_SEQUENCIZABILITY_DOMAINS.map(async (entry) => {
