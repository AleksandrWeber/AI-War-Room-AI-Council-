...
import type { AppendizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_APPENDIZABILITY_TABLES } from './appendizability-rollout.helpers.js'

const WORKSPACE_APPENDIZABILITY_DOMAINS: Array<{
  domain: AppendizabilityAdminDomain
  tableName: string
...
@Injectable()
export class AppendizabilityStatusService {
  ...
  async getAppendizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_APPENDIZABILITY_TABLES,
    )

    return {
      existingAppendizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAppendizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_APPENDIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_APPENDIZABILITY_DOMAINS.map(async (entry) => {
