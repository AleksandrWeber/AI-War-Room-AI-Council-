...
import type { ControllabilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CONTROLLABILITYVAULTIZABILITY_TABLES } from './controllabilityvaultizability-rollout.helpers.js'

const WORKSPACE_CONTROLLABILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: ControllabilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ControllabilityvaultizabilityStatusService {
  ...
  async getControllabilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CONTROLLABILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingControllabilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceControllabilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CONTROLLABILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CONTROLLABILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
