...
import type { TimeoutizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_TIMEOUTIZABILITY_TABLES } from './timeoutizability-rollout.helpers.js'

const WORKSPACE_TIMEOUTIZABILITY_DOMAINS: Array<{
  domain: TimeoutizabilityAdminDomain
  tableName: string
...
@Injectable()
export class TimeoutizabilityStatusService {
  ...
  async getTimeoutizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_TIMEOUTIZABILITY_TABLES,
    )

    return {
      existingTimeoutizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceTimeoutizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_TIMEOUTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_TIMEOUTIZABILITY_DOMAINS.map(async (entry) => {
