...
import type { RestoreizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_RESTOREIZABILITY_TABLES } from './restoreizability-rollout.helpers.js'

const WORKSPACE_RESTOREIZABILITY_DOMAINS: Array<{
  domain: RestoreizabilityAdminDomain
  tableName: string
...
@Injectable()
export class RestoreizabilityStatusService {
  ...
  async getRestoreizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_RESTOREIZABILITY_TABLES,
    )

    return {
      existingRestoreizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceRestoreizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_RESTOREIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_RESTOREIZABILITY_DOMAINS.map(async (entry) => {
