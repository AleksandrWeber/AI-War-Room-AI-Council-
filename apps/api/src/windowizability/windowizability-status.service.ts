...
import type { WindowizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_WINDOWIZABILITY_TABLES } from './windowizability-rollout.helpers.js'

const WORKSPACE_WINDOWIZABILITY_DOMAINS: Array<{
  domain: WindowizabilityAdminDomain
  tableName: string
...
@Injectable()
export class WindowizabilityStatusService {
  ...
  async getWindowizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_WINDOWIZABILITY_TABLES,
    )

    return {
      existingWindowizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceWindowizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_WINDOWIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_WINDOWIZABILITY_DOMAINS.map(async (entry) => {
