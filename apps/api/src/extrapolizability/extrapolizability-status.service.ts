...
import type { ExtrapolizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_EXTRAPOLIZABILITY_TABLES } from './extrapolizability-rollout.helpers.js'

const WORKSPACE_EXTRAPOLIZABILITY_DOMAINS: Array<{
  domain: ExtrapolizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ExtrapolizabilityStatusService {
  ...
  async getExtrapolizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_EXTRAPOLIZABILITY_TABLES,
    )

    return {
      existingExtrapolizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceExtrapolizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_EXTRAPOLIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_EXTRAPOLIZABILITY_DOMAINS.map(async (entry) => {
