...
import type { EmblemizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_EMBLEMIZABILITY_TABLES } from './emblemizability-rollout.helpers.js'

const WORKSPACE_EMBLEMIZABILITY_DOMAINS: Array<{
  domain: EmblemizabilityAdminDomain
  tableName: string
...
@Injectable()
export class EmblemizabilityStatusService {
  ...
  async getEmblemizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_EMBLEMIZABILITY_TABLES,
    )

    return {
      existingEmblemizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceEmblemizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_EMBLEMIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_EMBLEMIZABILITY_DOMAINS.map(async (entry) => {
