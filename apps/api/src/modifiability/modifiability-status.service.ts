...
import type { ModifiabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_MODIFIABILITY_TABLES } from './modifiability-rollout.helpers.js'

const WORKSPACE_MODIFIABILITY_DOMAINS: Array<{
  domain: ModifiabilityAdminDomain
  tableName: string
...
@Injectable()
export class ModifiabilityStatusService {
  ...
  async getModifiabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_MODIFIABILITY_TABLES,
    )

    return {
      existingModifiabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceModifiabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_MODIFIABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_MODIFIABILITY_DOMAINS.map(async (entry) => {
