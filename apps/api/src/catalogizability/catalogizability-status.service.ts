...
import type { CatalogizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CATALOGIZABILITY_TABLES } from './catalogizability-rollout.helpers.js'

const WORKSPACE_CATALOGIZABILITY_DOMAINS: Array<{
  domain: CatalogizabilityAdminDomain
  tableName: string
...
@Injectable()
export class CatalogizabilityStatusService {
  ...
  async getCatalogizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CATALOGIZABILITY_TABLES,
    )

    return {
      existingCatalogizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceCatalogizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CATALOGIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CATALOGIZABILITY_DOMAINS.map(async (entry) => {
