...
import type { ParametrizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_PARAMETRIZABILITY_TABLES } from './parametrizability-rollout.helpers.js'

const WORKSPACE_PARAMETRIZABILITY_DOMAINS: Array<{
  domain: ParametrizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ParametrizabilityStatusService {
  ...
  async getParametrizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_PARAMETRIZABILITY_TABLES,
    )

    return {
      existingParametrizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceParametrizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_PARAMETRIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_PARAMETRIZABILITY_DOMAINS.map(async (entry) => {
