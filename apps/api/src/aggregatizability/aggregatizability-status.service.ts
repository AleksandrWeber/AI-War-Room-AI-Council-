...
import type { AggregatizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_AGGREGATIZABILITY_TABLES } from './aggregatizability-rollout.helpers.js'

const WORKSPACE_AGGREGATIZABILITY_DOMAINS: Array<{
  domain: AggregatizabilityAdminDomain
  tableName: string
...
@Injectable()
export class AggregatizabilityStatusService {
  ...
  async getAggregatizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_AGGREGATIZABILITY_TABLES,
    )

    return {
      existingAggregatizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAggregatizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_AGGREGATIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_AGGREGATIZABILITY_DOMAINS.map(async (entry) => {
