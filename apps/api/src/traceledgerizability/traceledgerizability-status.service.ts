...
import type { TraceledgerizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_TRACELEDGERIZABILITY_TABLES } from './traceledgerizability-rollout.helpers.js'

const WORKSPACE_TRACELEDGERIZABILITY_DOMAINS: Array<{
  domain: TraceledgerizabilityAdminDomain
  tableName: string
...
@Injectable()
export class TraceledgerizabilityStatusService {
  ...
  async getTraceledgerizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_TRACELEDGERIZABILITY_TABLES,
    )

    return {
      existingTraceledgerizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceTraceledgerizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_TRACELEDGERIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_TRACELEDGERIZABILITY_DOMAINS.map(async (entry) => {
