...
import type { DeliverizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_DELIVERIZABILITY_TABLES } from './deliverizability-rollout.helpers.js'

const WORKSPACE_DELIVERIZABILITY_DOMAINS: Array<{
  domain: DeliverizabilityAdminDomain
  tableName: string
...
@Injectable()
export class DeliverizabilityStatusService {
  ...
  async getDeliverizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_DELIVERIZABILITY_TABLES,
    )

    return {
      existingDeliverizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceDeliverizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_DELIVERIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_DELIVERIZABILITY_DOMAINS.map(async (entry) => {
