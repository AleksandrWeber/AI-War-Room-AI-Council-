...
import type { RecoverabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_RECOVERABILITY_TABLES } from './recoverability-rollout.helpers.js'

const WORKSPACE_RECOVERABILITY_DOMAINS: Array<{
  domain: RecoverabilityAdminDomain
  tableName: string
...
@Injectable()
export class RecoverabilityStatusService {
  ...
  async getRecoverabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_RECOVERABILITY_TABLES,
    )

    return {
      existingRecoverabilityTableCount: existingTables.size,
      existingTables,
      runWorkflowsTableExists: existingTables.has('run_workflows'),
    }
  }

  async getWorkspaceRecoverabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_RECOVERABILITY_DOMAINS.flatMap(
    ...
    return Promise.all(
      WORKSPACE_RECOVERABILITY_DOMAINS.map(async (entry) => {
