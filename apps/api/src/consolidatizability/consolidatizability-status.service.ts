...
import type { ConsolidatizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CONSOLIDATIZABILITY_TABLES } from './consolidatizability-rollout.helpers.js'

const WORKSPACE_CONSOLIDATIZABILITY_DOMAINS: Array<{
  domain: ConsolidatizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ConsolidatizabilityStatusService {
  ...
  async getConsolidatizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CONSOLIDATIZABILITY_TABLES,
    )

    return {
      existingConsolidatizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceConsolidatizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CONSOLIDATIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CONSOLIDATIZABILITY_DOMAINS.map(async (entry) => {
