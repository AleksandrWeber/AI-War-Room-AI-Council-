...
import type { ManageabilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_MANAGEABILITYVAULTIZABILITY_TABLES } from './manageabilityvaultizability-rollout.helpers.js'

const WORKSPACE_MANAGEABILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: ManageabilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ManageabilityvaultizabilityStatusService {
  ...
  async getManageabilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_MANAGEABILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingManageabilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceManageabilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_MANAGEABILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_MANAGEABILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
