...
import type { DebouncizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_DEBOUNCIZABILITY_TABLES } from './debouncizability-rollout.helpers.js'

const WORKSPACE_DEBOUNCIZABILITY_DOMAINS: Array<{
  domain: DebouncizabilityAdminDomain
  tableName: string
...
@Injectable()
export class DebouncizabilityStatusService {
  ...
  async getDebouncizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_DEBOUNCIZABILITY_TABLES,
    )

    return {
      existingDebouncizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceDebouncizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_DEBOUNCIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_DEBOUNCIZABILITY_DOMAINS.map(async (entry) => {
