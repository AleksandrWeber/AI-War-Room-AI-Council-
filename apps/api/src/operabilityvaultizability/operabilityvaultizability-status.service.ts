...
import type { OperabilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_OPERABILITYVAULTIZABILITY_TABLES } from './operabilityvaultizability-rollout.helpers.js'

const WORKSPACE_OPERABILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: OperabilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class OperabilityvaultizabilityStatusService {
  ...
  async getOperabilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_OPERABILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingOperabilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceOperabilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_OPERABILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_OPERABILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
