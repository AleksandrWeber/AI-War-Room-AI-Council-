...
import type { AdaptizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ADAPTIZABILITY_TABLES } from './adaptizability-rollout.helpers.js'

const WORKSPACE_ADAPTIZABILITY_DOMAINS: Array<{
  domain: AdaptizabilityAdminDomain
  tableName: string
...
@Injectable()
export class AdaptizabilityStatusService {
  ...
  async getAdaptizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ADAPTIZABILITY_TABLES,
    )

    return {
      existingAdaptizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAdaptizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ADAPTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ADAPTIZABILITY_DOMAINS.map(async (entry) => {
