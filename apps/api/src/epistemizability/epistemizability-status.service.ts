...
import type { EpistemizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_EPISTEMIZABILITY_TABLES } from './epistemizability-rollout.helpers.js'

const WORKSPACE_EPISTEMIZABILITY_DOMAINS: Array<{
  domain: EpistemizabilityAdminDomain
  tableName: string
...
@Injectable()
export class EpistemizabilityStatusService {
  ...
  async getEpistemizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_EPISTEMIZABILITY_TABLES,
    )

    return {
      existingEpistemizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceEpistemizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_EPISTEMIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_EPISTEMIZABILITY_DOMAINS.map(async (entry) => {
