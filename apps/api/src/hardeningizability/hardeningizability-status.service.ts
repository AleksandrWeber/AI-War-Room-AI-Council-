...
import type { HardeningizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_HARDENINGIZABILITY_TABLES } from './hardeningizability-rollout.helpers.js'

const WORKSPACE_HARDENINGIZABILITY_DOMAINS: Array<{
  domain: HardeningizabilityAdminDomain
  tableName: string
...
@Injectable()
export class HardeningizabilityStatusService {
  ...
  async getHardeningizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_HARDENINGIZABILITY_TABLES,
    )

    return {
      existingHardeningizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceHardeningizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_HARDENINGIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_HARDENINGIZABILITY_DOMAINS.map(async (entry) => {
