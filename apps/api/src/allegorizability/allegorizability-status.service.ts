...
import type { AllegorizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ALLEGORIZABILITY_TABLES } from './allegorizability-rollout.helpers.js'

const WORKSPACE_ALLEGORIZABILITY_DOMAINS: Array<{
  domain: AllegorizabilityAdminDomain
  tableName: string
...
@Injectable()
export class AllegorizabilityStatusService {
  ...
  async getAllegorizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ALLEGORIZABILITY_TABLES,
    )

    return {
      existingAllegorizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAllegorizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ALLEGORIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ALLEGORIZABILITY_DOMAINS.map(async (entry) => {
