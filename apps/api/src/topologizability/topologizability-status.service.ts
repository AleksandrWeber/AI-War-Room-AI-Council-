...
import type { TopologizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_TOPOLOGIZABILITY_TABLES } from './topologizability-rollout.helpers.js'

const WORKSPACE_TOPOLOGIZABILITY_DOMAINS: Array<{
  domain: TopologizabilityAdminDomain
  tableName: string
...
@Injectable()
export class TopologizabilityStatusService {
  ...
  async getTopologizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_TOPOLOGIZABILITY_TABLES,
    )

    return {
      existingTopologizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceTopologizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_TOPOLOGIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_TOPOLOGIZABILITY_DOMAINS.map(async (entry) => {
