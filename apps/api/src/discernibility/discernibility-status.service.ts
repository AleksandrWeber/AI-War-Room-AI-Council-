...
import type { DiscernibilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_DISCERNIBILITY_TABLES } from './discernibility-rollout.helpers.js'

const WORKSPACE_DISCERNIBILITY_DOMAINS: Array<{
  domain: DiscernibilityAdminDomain
  tableName: string
...
@Injectable()
export class DiscernibilityStatusService {
  ...
  async getDiscernibilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_DISCERNIBILITY_TABLES,
    )

    return {
      existingDiscernibilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceDiscernibilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_DISCERNIBILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_DISCERNIBILITY_DOMAINS.map(async (entry) => {
