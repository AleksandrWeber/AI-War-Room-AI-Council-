...
import type { OrchestrationizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ORCHESTRATIONIZABILITY_TABLES } from './orchestrationizability-rollout.helpers.js'

const WORKSPACE_ORCHESTRATIONIZABILITY_DOMAINS: Array<{
  domain: OrchestrationizabilityAdminDomain
  tableName: string
...
@Injectable()
export class OrchestrationizabilityStatusService {
  ...
  async getOrchestrationizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ORCHESTRATIONIZABILITY_TABLES,
    )

    return {
      existingOrchestrationizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceOrchestrationizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ORCHESTRATIONIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ORCHESTRATIONIZABILITY_DOMAINS.map(async (entry) => {
