...
import type { InterpolizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_INTERPOLIZABILITY_TABLES } from './interpolizability-rollout.helpers.js'

const WORKSPACE_INTERPOLIZABILITY_DOMAINS: Array<{
  domain: InterpolizabilityAdminDomain
  tableName: string
...
@Injectable()
export class InterpolizabilityStatusService {
  ...
  async getInterpolizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_INTERPOLIZABILITY_TABLES,
    )

    return {
      existingInterpolizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceInterpolizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_INTERPOLIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_INTERPOLIZABILITY_DOMAINS.map(async (entry) => {
