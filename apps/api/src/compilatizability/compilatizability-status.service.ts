...
import type { CompilatizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_COMPILATIZABILITY_TABLES } from './compilatizability-rollout.helpers.js'

const WORKSPACE_COMPILATIZABILITY_DOMAINS: Array<{
  domain: CompilatizabilityAdminDomain
  tableName: string
...
@Injectable()
export class CompilatizabilityStatusService {
  ...
  async getCompilatizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_COMPILATIZABILITY_TABLES,
    )

    return {
      existingCompilatizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceCompilatizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_COMPILATIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_COMPILATIZABILITY_DOMAINS.map(async (entry) => {
