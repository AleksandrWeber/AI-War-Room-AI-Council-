...
import type { OversightAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_OVERSIGHT_TABLES } from './oversight-rollout.helpers.js'

const WORKSPACE_OVERSIGHT_DOMAINS: Array<{
  domain: OversightAdminDomain
  tableName: string
...
@Injectable()
export class OversightStatusService {
  ...
  async getOversightTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_OVERSIGHT_TABLES,
    )

    return {
      existingOversightTableCount: existingTables.size,
      existingTables,
      billingInvoicesTableExists: existingTables.has('billing_invoices'),
      billingWebhookEventsTableExists: existingTables.has(
  ...
  async getWorkspaceOversightInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_OVERSIGHT_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_OVERSIGHT_DOMAINS.map(async (entry) => {
