...
import type { AllocationizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ALLOCATIONIZABILITY_TABLES } from './allocationizability-rollout.helpers.js'

const WORKSPACE_ALLOCATIONIZABILITY_DOMAINS: Array<{
  domain: AllocationizabilityAdminDomain
  tableName: string
...
@Injectable()
export class AllocationizabilityStatusService {
  ...
  async getAllocationizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ALLOCATIONIZABILITY_TABLES,
    )

    return {
      existingAllocationizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAllocationizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ALLOCATIONIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ALLOCATIONIZABILITY_DOMAINS.map(async (entry) => {
