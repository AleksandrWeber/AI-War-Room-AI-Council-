...
import type { TransparencyizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_TRANSPARENCYIZABILITY_TABLES } from './transparencyizability-rollout.helpers.js'

const WORKSPACE_TRANSPARENCYIZABILITY_DOMAINS: Array<{
  domain: TransparencyizabilityAdminDomain
  tableName: string
...
@Injectable()
export class TransparencyizabilityStatusService {
  ...
  async getTransparencyizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_TRANSPARENCYIZABILITY_TABLES,
    )

    return {
      existingTransparencyizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceTransparencyizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_TRANSPARENCYIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_TRANSPARENCYIZABILITY_DOMAINS.map(async (entry) => {
