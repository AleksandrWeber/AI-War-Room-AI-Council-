...
import type { TriggerizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_TRIGGERIZABILITY_TABLES } from './triggerizability-rollout.helpers.js'

const WORKSPACE_TRIGGERIZABILITY_DOMAINS: Array<{
  domain: TriggerizabilityAdminDomain
  tableName: string
...
@Injectable()
export class TriggerizabilityStatusService {
  ...
  async getTriggerizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_TRIGGERIZABILITY_TABLES,
    )

    return {
      existingTriggerizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceTriggerizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_TRIGGERIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_TRIGGERIZABILITY_DOMAINS.map(async (entry) => {
