...
import type { QuorumizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_QUORUMIZABILITY_TABLES } from './quorumizability-rollout.helpers.js'

const WORKSPACE_QUORUMIZABILITY_DOMAINS: Array<{
  domain: QuorumizabilityAdminDomain
  tableName: string
...
@Injectable()
export class QuorumizabilityStatusService {
  ...
  async getQuorumizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_QUORUMIZABILITY_TABLES,
    )

    return {
      existingQuorumizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceQuorumizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_QUORUMIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_QUORUMIZABILITY_DOMAINS.map(async (entry) => {
