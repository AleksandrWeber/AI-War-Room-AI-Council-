...
import type { DefensibilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_DEFENSIBILITYVAULTIZABILITY_TABLES } from './defensibilityvaultizability-rollout.helpers.js'

const WORKSPACE_DEFENSIBILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: DefensibilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class DefensibilityvaultizabilityStatusService {
  ...
  async getDefensibilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_DEFENSIBILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingDefensibilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceDefensibilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_DEFENSIBILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_DEFENSIBILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
