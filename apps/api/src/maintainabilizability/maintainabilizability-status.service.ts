...
import type { MaintainabilizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_MAINTAINABILIZABILITY_TABLES } from './maintainabilizability-rollout.helpers.js'

const WORKSPACE_MAINTAINABILIZABILITY_DOMAINS: Array<{
  domain: MaintainabilizabilityAdminDomain
  tableName: string
...
@Injectable()
export class MaintainabilizabilityStatusService {
  ...
  async getMaintainabilizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_MAINTAINABILIZABILITY_TABLES,
    )

    return {
      existingMaintainabilizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceMaintainabilizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_MAINTAINABILIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_MAINTAINABILIZABILITY_DOMAINS.map(async (entry) => {
