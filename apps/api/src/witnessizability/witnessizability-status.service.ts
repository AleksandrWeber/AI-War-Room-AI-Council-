...
import type { WitnessizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_WITNESSIZABILITY_TABLES } from './witnessizability-rollout.helpers.js'

const WORKSPACE_WITNESSIZABILITY_DOMAINS: Array<{
  domain: WitnessizabilityAdminDomain
  tableName: string
...
@Injectable()
export class WitnessizabilityStatusService {
  ...
  async getWitnessizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_WITNESSIZABILITY_TABLES,
    )

    return {
      existingWitnessizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceWitnessizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_WITNESSIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_WITNESSIZABILITY_DOMAINS.map(async (entry) => {
