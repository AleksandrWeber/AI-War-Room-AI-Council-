...
import type { AuditproofizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_AUDITPROOFIZABILITY_TABLES } from './auditproofizability-rollout.helpers.js'

const WORKSPACE_AUDITPROOFIZABILITY_DOMAINS: Array<{
  domain: AuditproofizabilityAdminDomain
  tableName: string
...
@Injectable()
export class AuditproofizabilityStatusService {
  ...
  async getAuditproofizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_AUDITPROOFIZABILITY_TABLES,
    )

    return {
      existingAuditproofizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAuditproofizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_AUDITPROOFIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_AUDITPROOFIZABILITY_DOMAINS.map(async (entry) => {
