...
import type { BluegreenizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_BLUEGREENIZABILITY_TABLES } from './bluegreenizability-rollout.helpers.js'

const WORKSPACE_BLUEGREENIZABILITY_DOMAINS: Array<{
  domain: BluegreenizabilityAdminDomain
  tableName: string
...
@Injectable()
export class BluegreenizabilityStatusService {
  ...
  async getBluegreenizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_BLUEGREENIZABILITY_TABLES,
    )

    return {
      existingBluegreenizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceBluegreenizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_BLUEGREENIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_BLUEGREENIZABILITY_DOMAINS.map(async (entry) => {
