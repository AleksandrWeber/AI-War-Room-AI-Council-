...
import type { LeaderizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_LEADERIZABILITY_TABLES } from './leaderizability-rollout.helpers.js'

const WORKSPACE_LEADERIZABILITY_DOMAINS: Array<{
  domain: LeaderizabilityAdminDomain
  tableName: string
...
@Injectable()
export class LeaderizabilityStatusService {
  ...
  async getLeaderizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_LEADERIZABILITY_TABLES,
    )

    return {
      existingLeaderizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceLeaderizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_LEADERIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_LEADERIZABILITY_DOMAINS.map(async (entry) => {
