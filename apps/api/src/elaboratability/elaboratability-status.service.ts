...
import type { ElaboratabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ELABORATABILITY_TABLES } from './elaboratability-rollout.helpers.js'

const WORKSPACE_ELABORATABILITY_DOMAINS: Array<{
  domain: ElaboratabilityAdminDomain
  tableName: string
...
@Injectable()
export class ElaboratabilityStatusService {
  ...
  async getElaboratabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ELABORATABILITY_TABLES,
    )

    return {
      existingElaboratabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceElaboratabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ELABORATABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ELABORATABILITY_DOMAINS.map(async (entry) => {
