...
import type { UnderstandabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_UNDERSTANDABILITY_TABLES } from './understandability-rollout.helpers.js'

const WORKSPACE_UNDERSTANDABILITY_DOMAINS: Array<{
  domain: UnderstandabilityAdminDomain
  tableName: string
...
@Injectable()
export class UnderstandabilityStatusService {
  ...
  async getUnderstandabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_UNDERSTANDABILITY_TABLES,
    )

    return {
      existingUnderstandabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceUnderstandabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_UNDERSTANDABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_UNDERSTANDABILITY_DOMAINS.map(async (entry) => {
