...
import type { EventizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_EVENTIZABILITY_TABLES } from './eventizability-rollout.helpers.js'

const WORKSPACE_EVENTIZABILITY_DOMAINS: Array<{
  domain: EventizabilityAdminDomain
  tableName: string
...
@Injectable()
export class EventizabilityStatusService {
  ...
  async getEventizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_EVENTIZABILITY_TABLES,
    )

    return {
      existingEventizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceEventizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_EVENTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_EVENTIZABILITY_DOMAINS.map(async (entry) => {
