...
import type { TriggeringizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_TRIGGERINGIZABILITY_TABLES } from './triggeringizability-rollout.helpers.js'

const WORKSPACE_TRIGGERINGIZABILITY_DOMAINS: Array<{
  domain: TriggeringizabilityAdminDomain
  tableName: string
...
@Injectable()
export class TriggeringizabilityStatusService {
  ...
  async getTriggeringizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_TRIGGERINGIZABILITY_TABLES,
    )

    return {
      existingTriggeringizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceTriggeringizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_TRIGGERINGIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_TRIGGERINGIZABILITY_DOMAINS.map(async (entry) => {
