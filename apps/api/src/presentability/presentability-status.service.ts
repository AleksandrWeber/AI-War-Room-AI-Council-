...
import type { PresentabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_PRESENTABILITY_TABLES } from './presentability-rollout.helpers.js'

const WORKSPACE_PRESENTABILITY_DOMAINS: Array<{
  domain: PresentabilityAdminDomain
  tableName: string
...
@Injectable()
export class PresentabilityStatusService {
  ...
  async getPresentabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_PRESENTABILITY_TABLES,
    )

    return {
      existingPresentabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspacePresentabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_PRESENTABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_PRESENTABILITY_DOMAINS.map(async (entry) => {
