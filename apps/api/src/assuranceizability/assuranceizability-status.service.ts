...
import type { AssuranceizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ASSURANCEIZABILITY_TABLES } from './assuranceizability-rollout.helpers.js'

const WORKSPACE_ASSURANCEIZABILITY_DOMAINS: Array<{
  domain: AssuranceizabilityAdminDomain
  tableName: string
...
@Injectable()
export class AssuranceizabilityStatusService {
  ...
  async getAssuranceizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ASSURANCEIZABILITY_TABLES,
    )

    return {
      existingAssuranceizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAssuranceizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ASSURANCEIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ASSURANCEIZABILITY_DOMAINS.map(async (entry) => {
