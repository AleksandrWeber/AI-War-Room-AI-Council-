...
import type { BatchingizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_BATCHINGIZABILITY_TABLES } from './batchingizability-rollout.helpers.js'

const WORKSPACE_BATCHINGIZABILITY_DOMAINS: Array<{
  domain: BatchingizabilityAdminDomain
  tableName: string
...
@Injectable()
export class BatchingizabilityStatusService {
  ...
  async getBatchingizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_BATCHINGIZABILITY_TABLES,
    )

    return {
      existingBatchingizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceBatchingizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_BATCHINGIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_BATCHINGIZABILITY_DOMAINS.map(async (entry) => {
