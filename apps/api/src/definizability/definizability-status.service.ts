...
import type { DefinizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_DEFINIZABILITY_TABLES } from './definizability-rollout.helpers.js'

const WORKSPACE_DEFINIZABILITY_DOMAINS: Array<{
  domain: DefinizabilityAdminDomain
  tableName: string
...
@Injectable()
export class DefinizabilityStatusService {
  ...
  async getDefinizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_DEFINIZABILITY_TABLES,
    )

    return {
      existingDefinizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceDefinizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_DEFINIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_DEFINIZABILITY_DOMAINS.map(async (entry) => {
