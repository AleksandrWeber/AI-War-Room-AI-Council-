...
import type { SchedulabilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_SCHEDULABILITYVAULTIZABILITY_TABLES } from './schedulabilityvaultizability-rollout.helpers.js'

const WORKSPACE_SCHEDULABILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: SchedulabilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class SchedulabilityvaultizabilityStatusService {
  ...
  async getSchedulabilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_SCHEDULABILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingSchedulabilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceSchedulabilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_SCHEDULABILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_SCHEDULABILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
