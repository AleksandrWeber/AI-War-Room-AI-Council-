...
import type { TypologizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_TYPOLOGIZABILITY_TABLES } from './typologizability-rollout.helpers.js'

const WORKSPACE_TYPOLOGIZABILITY_DOMAINS: Array<{
  domain: TypologizabilityAdminDomain
  tableName: string
...
@Injectable()
export class TypologizabilityStatusService {
  ...
  async getTypologizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_TYPOLOGIZABILITY_TABLES,
    )

    return {
      existingTypologizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceTypologizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_TYPOLOGIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_TYPOLOGIZABILITY_DOMAINS.map(async (entry) => {
