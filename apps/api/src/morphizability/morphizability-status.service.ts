...
import type { MorphizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_MORPHIZABILITY_TABLES } from './morphizability-rollout.helpers.js'

const WORKSPACE_MORPHIZABILITY_DOMAINS: Array<{
  domain: MorphizabilityAdminDomain
  tableName: string
...
@Injectable()
export class MorphizabilityStatusService {
  ...
  async getMorphizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_MORPHIZABILITY_TABLES,
    )

    return {
      existingMorphizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceMorphizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_MORPHIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_MORPHIZABILITY_DOMAINS.map(async (entry) => {
