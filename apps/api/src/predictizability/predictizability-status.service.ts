...
import type { PredictizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_PREDICTIZABILITY_TABLES } from './predictizability-rollout.helpers.js'

const WORKSPACE_PREDICTIZABILITY_DOMAINS: Array<{
  domain: PredictizabilityAdminDomain
  tableName: string
...
@Injectable()
export class PredictizabilityStatusService {
  ...
  async getPredictizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_PREDICTIZABILITY_TABLES,
    )

    return {
      existingPredictizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspacePredictizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_PREDICTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_PREDICTIZABILITY_DOMAINS.map(async (entry) => {
