...
import type { AdaptabilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ADAPTABILITYVAULTIZABILITY_TABLES } from './adaptabilityvaultizability-rollout.helpers.js'

const WORKSPACE_ADAPTABILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: AdaptabilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class AdaptabilityvaultizabilityStatusService {
  ...
  async getAdaptabilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ADAPTABILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingAdaptabilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAdaptabilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ADAPTABILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ADAPTABILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
