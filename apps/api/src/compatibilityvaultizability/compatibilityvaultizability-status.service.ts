...
import type { CompatibilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_COMPATIBILITYVAULTIZABILITY_TABLES } from './compatibilityvaultizability-rollout.helpers.js'

const WORKSPACE_COMPATIBILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: CompatibilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class CompatibilityvaultizabilityStatusService {
  ...
  async getCompatibilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_COMPATIBILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingCompatibilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceCompatibilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_COMPATIBILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_COMPATIBILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
