...
import type { AdjustabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ADJUSTABILITY_TABLES } from './adjustability-rollout.helpers.js'

const WORKSPACE_ADJUSTABILITY_DOMAINS: Array<{
  domain: AdjustabilityAdminDomain
  tableName: string
...
@Injectable()
export class AdjustabilityStatusService {
  ...
  async getAdjustabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ADJUSTABILITY_TABLES,
    )

    return {
      existingAdjustabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAdjustabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ADJUSTABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ADJUSTABILITY_DOMAINS.map(async (entry) => {
