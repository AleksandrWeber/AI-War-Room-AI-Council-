...
import type { RetrievabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_RETRIEVABILITY_TABLES } from './retrievability-rollout.helpers.js'

const WORKSPACE_RETRIEVABILITY_DOMAINS: Array<{
  domain: RetrievabilityAdminDomain
  tableName: string
...
@Injectable()
export class RetrievabilityStatusService {
  ...
  async getRetrievabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_RETRIEVABILITY_TABLES,
    )

    return {
      existingRetrievabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceRetrievabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_RETRIEVABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_RETRIEVABILITY_DOMAINS.map(async (entry) => {
