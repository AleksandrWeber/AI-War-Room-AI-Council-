...
import type { IdentityproofizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_IDENTITYPROOFIZABILITY_TABLES } from './identityproofizability-rollout.helpers.js'

const WORKSPACE_IDENTITYPROOFIZABILITY_DOMAINS: Array<{
  domain: IdentityproofizabilityAdminDomain
  tableName: string
...
@Injectable()
export class IdentityproofizabilityStatusService {
  ...
  async getIdentityproofizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_IDENTITYPROOFIZABILITY_TABLES,
    )

    return {
      existingIdentityproofizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceIdentityproofizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_IDENTITYPROOFIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_IDENTITYPROOFIZABILITY_DOMAINS.map(async (entry) => {
