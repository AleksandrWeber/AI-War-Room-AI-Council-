...
import type { VisualizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_VISUALIZABILITY_TABLES } from './visualizability-rollout.helpers.js'

const WORKSPACE_VISUALIZABILITY_DOMAINS: Array<{
  domain: VisualizabilityAdminDomain
  tableName: string
...
@Injectable()
export class VisualizabilityStatusService {
  ...
  async getVisualizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_VISUALIZABILITY_TABLES,
    )

    return {
      existingVisualizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceVisualizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_VISUALIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_VISUALIZABILITY_DOMAINS.map(async (entry) => {
