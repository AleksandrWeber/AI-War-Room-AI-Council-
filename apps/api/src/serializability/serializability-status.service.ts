...
import type { SerializabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_SERIALIZABILITY_TABLES } from './serializability-rollout.helpers.js'

const WORKSPACE_SERIALIZABILITY_DOMAINS: Array<{
  domain: SerializabilityAdminDomain
  tableName: string
...
@Injectable()
export class SerializabilityStatusService {
  ...
  async getSerializabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_SERIALIZABILITY_TABLES,
    )

    return {
      existingSerializabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceSerializabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_SERIALIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_SERIALIZABILITY_DOMAINS.map(async (entry) => {
