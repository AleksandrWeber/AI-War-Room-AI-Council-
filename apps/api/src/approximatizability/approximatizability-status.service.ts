...
import type { ApproximatizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_APPROXIMATIZABILITY_TABLES } from './approximatizability-rollout.helpers.js'

const WORKSPACE_APPROXIMATIZABILITY_DOMAINS: Array<{
  domain: ApproximatizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ApproximatizabilityStatusService {
  ...
  async getApproximatizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_APPROXIMATIZABILITY_TABLES,
    )

    return {
      existingApproximatizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceApproximatizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_APPROXIMATIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_APPROXIMATIZABILITY_DOMAINS.map(async (entry) => {
