...
import type { DeployabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_DEPLOYABILITY_TABLES } from './deployability-rollout.helpers.js'

const WORKSPACE_DEPLOYABILITY_DOMAINS: Array<{
  domain: DeployabilityAdminDomain
  tableName: string
...
@Injectable()
export class DeployabilityStatusService {
  ...
  async getDeployabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_DEPLOYABILITY_TABLES,
    )

    return {
      existingDeployabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceDeployabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_DEPLOYABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_DEPLOYABILITY_DOMAINS.map(async (entry) => {
