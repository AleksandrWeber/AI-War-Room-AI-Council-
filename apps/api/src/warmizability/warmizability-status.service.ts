...
import type { WarmizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_WARMIZABILITY_TABLES } from './warmizability-rollout.helpers.js'

const WORKSPACE_WARMIZABILITY_DOMAINS: Array<{
  domain: WarmizabilityAdminDomain
  tableName: string
...
@Injectable()
export class WarmizabilityStatusService {
  ...
  async getWarmizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_WARMIZABILITY_TABLES,
    )

    return {
      existingWarmizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceWarmizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_WARMIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_WARMIZABILITY_DOMAINS.map(async (entry) => {
