...
import type { CompatibilizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_COMPATIBILIZABILITY_TABLES } from './compatibilizability-rollout.helpers.js'

const WORKSPACE_COMPATIBILIZABILITY_DOMAINS: Array<{
  domain: CompatibilizabilityAdminDomain
  tableName: string
...
@Injectable()
export class CompatibilizabilityStatusService {
  ...
  async getCompatibilizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_COMPATIBILIZABILITY_TABLES,
    )

    return {
      existingCompatibilizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceCompatibilizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_COMPATIBILIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_COMPATIBILIZABILITY_DOMAINS.map(async (entry) => {
