...
import type { IndexizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_INDEXIZABILITY_TABLES } from './indexizability-rollout.helpers.js'

const WORKSPACE_INDEXIZABILITY_DOMAINS: Array<{
  domain: IndexizabilityAdminDomain
  tableName: string
...
@Injectable()
export class IndexizabilityStatusService {
  ...
  async getIndexizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_INDEXIZABILITY_TABLES,
    )

    return {
      existingIndexizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceIndexizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_INDEXIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_INDEXIZABILITY_DOMAINS.map(async (entry) => {
