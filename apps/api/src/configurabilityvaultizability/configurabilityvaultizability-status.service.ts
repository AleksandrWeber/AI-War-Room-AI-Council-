...
import type { ConfigurabilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CONFIGURABILITYVAULTIZABILITY_TABLES } from './configurabilityvaultizability-rollout.helpers.js'

const WORKSPACE_CONFIGURABILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: ConfigurabilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ConfigurabilityvaultizabilityStatusService {
  ...
  async getConfigurabilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CONFIGURABILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingConfigurabilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceConfigurabilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CONFIGURABILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CONFIGURABILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
