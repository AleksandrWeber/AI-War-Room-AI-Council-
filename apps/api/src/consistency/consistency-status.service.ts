...
import type { ConsistencyAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CONSISTENCY_TABLES } from './consistency-rollout.helpers.js'

const WORKSPACE_CONSISTENCY_DOMAINS: Array<{
  domain: ConsistencyAdminDomain
  tableName: string
...
@Injectable()
export class ConsistencyStatusService {
  ...
  async getConsistencyTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CONSISTENCY_TABLES,
    )

    return {
      existingConsistencyTableCount: existingTables.size,
      existingTables,
      runWorkflowsTableExists: existingTables.has('run_workflows'),
    }
  }

  async getWorkspaceConsistencyInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CONSISTENCY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CONSISTENCY_DOMAINS.map(async (entry) => {
