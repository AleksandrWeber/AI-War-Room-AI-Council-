...
import type { OversightizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_OVERSIGHTIZABILITY_TABLES } from './oversightizability-rollout.helpers.js'

const WORKSPACE_OVERSIGHTIZABILITY_DOMAINS: Array<{
  domain: OversightizabilityAdminDomain
  tableName: string
...
@Injectable()
export class OversightizabilityStatusService {
  ...
  async getOversightizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_OVERSIGHTIZABILITY_TABLES,
    )

    return {
      existingOversightizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceOversightizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_OVERSIGHTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_OVERSIGHTIZABILITY_DOMAINS.map(async (entry) => {
