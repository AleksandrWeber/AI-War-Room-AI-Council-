...
import type { WitnessjournalizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_WITNESSJOURNALIZABILITY_TABLES } from './witnessjournalizability-rollout.helpers.js'

const WORKSPACE_WITNESSJOURNALIZABILITY_DOMAINS: Array<{
  domain: WitnessjournalizabilityAdminDomain
  tableName: string
...
@Injectable()
export class WitnessjournalizabilityStatusService {
  ...
  async getWitnessjournalizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_WITNESSJOURNALIZABILITY_TABLES,
    )

    return {
      existingWitnessjournalizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceWitnessjournalizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_WITNESSJOURNALIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_WITNESSJOURNALIZABILITY_DOMAINS.map(async (entry) => {
