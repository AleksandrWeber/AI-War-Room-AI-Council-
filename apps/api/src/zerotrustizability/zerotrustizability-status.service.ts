...
import type { ZerotrustizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ZEROTRUSTIZABILITY_TABLES } from './zerotrustizability-rollout.helpers.js'

const WORKSPACE_ZEROTRUSTIZABILITY_DOMAINS: Array<{
  domain: ZerotrustizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ZerotrustizabilityStatusService {
  ...
  async getZerotrustizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ZEROTRUSTIZABILITY_TABLES,
    )

    return {
      existingZerotrustizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceZerotrustizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ZEROTRUSTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ZEROTRUSTIZABILITY_DOMAINS.map(async (entry) => {
