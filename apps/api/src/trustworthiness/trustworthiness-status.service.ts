...
import type { TrustworthinessAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_TRUSTWORTHINESS_TABLES } from './trustworthiness-rollout.helpers.js'

const WORKSPACE_TRUSTWORTHINESS_DOMAINS: Array<{
  domain: TrustworthinessAdminDomain
  tableName: string
...
@Injectable()
export class TrustworthinessStatusService {
  ...
  async getTrustworthinessTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_TRUSTWORTHINESS_TABLES,
    )

    return {
      existingTrustworthinessTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceTrustworthinessInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_TRUSTWORTHINESS_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_TRUSTWORTHINESS_DOMAINS.map(async (entry) => {
