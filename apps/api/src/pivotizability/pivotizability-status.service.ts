...
import type { PivotizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_PIVOTIZABILITY_TABLES } from './pivotizability-rollout.helpers.js'

const WORKSPACE_PIVOTIZABILITY_DOMAINS: Array<{
  domain: PivotizabilityAdminDomain
  tableName: string
...
@Injectable()
export class PivotizabilityStatusService {
  ...
  async getPivotizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_PIVOTIZABILITY_TABLES,
    )

    return {
      existingPivotizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspacePivotizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_PIVOTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_PIVOTIZABILITY_DOMAINS.map(async (entry) => {
