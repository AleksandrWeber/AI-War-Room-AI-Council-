...
import type { FormalizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_FORMALIZABILITY_TABLES } from './formalizability-rollout.helpers.js'

const WORKSPACE_FORMALIZABILITY_DOMAINS: Array<{
  domain: FormalizabilityAdminDomain
  tableName: string
...
@Injectable()
export class FormalizabilityStatusService {
  ...
  async getFormalizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_FORMALIZABILITY_TABLES,
    )

    return {
      existingFormalizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceFormalizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_FORMALIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_FORMALIZABILITY_DOMAINS.map(async (entry) => {
