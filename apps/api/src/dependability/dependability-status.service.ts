...
import type { DependabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_DEPENDABILITY_TABLES } from './dependability-rollout.helpers.js'

const WORKSPACE_DEPENDABILITY_DOMAINS: Array<{
  domain: DependabilityAdminDomain
  tableName: string
...
@Injectable()
export class DependabilityStatusService {
  ...
  async getDependabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_DEPENDABILITY_TABLES,
    )

    return {
      existingDependabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceDependabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_DEPENDABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_DEPENDABILITY_DOMAINS.map(async (entry) => {
