...
import type { CompliancejournalizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_COMPLIANCEJOURNALIZABILITY_TABLES } from './compliancejournalizability-rollout.helpers.js'

const WORKSPACE_COMPLIANCEJOURNALIZABILITY_DOMAINS: Array<{
  domain: CompliancejournalizabilityAdminDomain
  tableName: string
...
@Injectable()
export class CompliancejournalizabilityStatusService {
  ...
  async getCompliancejournalizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_COMPLIANCEJOURNALIZABILITY_TABLES,
    )

    return {
      existingCompliancejournalizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceCompliancejournalizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_COMPLIANCEJOURNALIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_COMPLIANCEJOURNALIZABILITY_DOMAINS.map(async (entry) => {
