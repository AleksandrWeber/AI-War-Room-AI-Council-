...
import type { RetrievabilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_RETRIEVABILITYVAULTIZABILITY_TABLES } from './retrievabilityvaultizability-rollout.helpers.js'

const WORKSPACE_RETRIEVABILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: RetrievabilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class RetrievabilityvaultizabilityStatusService {
  ...
  async getRetrievabilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_RETRIEVABILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingRetrievabilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceRetrievabilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_RETRIEVABILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_RETRIEVABILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
