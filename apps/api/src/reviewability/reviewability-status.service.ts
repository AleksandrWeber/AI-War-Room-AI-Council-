...
import type { ReviewabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_REVIEWABILITY_TABLES } from './reviewability-rollout.helpers.js'

const WORKSPACE_REVIEWABILITY_DOMAINS: Array<{
  domain: ReviewabilityAdminDomain
  tableName: string
...
@Injectable()
export class ReviewabilityStatusService {
  ...
  async getReviewabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_REVIEWABILITY_TABLES,
    )

    return {
      existingReviewabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceReviewabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_REVIEWABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_REVIEWABILITY_DOMAINS.map(async (entry) => {
