...
import type { IsolatizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ISOLATIZABILITY_TABLES } from './isolatizability-rollout.helpers.js'

const WORKSPACE_ISOLATIZABILITY_DOMAINS: Array<{
  domain: IsolatizabilityAdminDomain
  tableName: string
...
@Injectable()
export class IsolatizabilityStatusService {
  ...
  async getIsolatizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ISOLATIZABILITY_TABLES,
    )

    return {
      existingIsolatizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceIsolatizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ISOLATIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ISOLATIZABILITY_DOMAINS.map(async (entry) => {
