...
import type { WitnessledgerizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_WITNESSLEDGERIZABILITY_TABLES } from './witnessledgerizability-rollout.helpers.js'

const WORKSPACE_WITNESSLEDGERIZABILITY_DOMAINS: Array<{
  domain: WitnessledgerizabilityAdminDomain
  tableName: string
...
@Injectable()
export class WitnessledgerizabilityStatusService {
  ...
  async getWitnessledgerizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_WITNESSLEDGERIZABILITY_TABLES,
    )

    return {
      existingWitnessledgerizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceWitnessledgerizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_WITNESSLEDGERIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_WITNESSLEDGERIZABILITY_DOMAINS.map(async (entry) => {
