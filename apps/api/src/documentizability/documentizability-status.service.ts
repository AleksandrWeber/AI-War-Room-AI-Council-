...
import type { DocumentizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_DOCUMENTIZABILITY_TABLES } from './documentizability-rollout.helpers.js'

const WORKSPACE_DOCUMENTIZABILITY_DOMAINS: Array<{
  domain: DocumentizabilityAdminDomain
  tableName: string
...
@Injectable()
export class DocumentizabilityStatusService {
  ...
  async getDocumentizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_DOCUMENTIZABILITY_TABLES,
    )

    return {
      existingDocumentizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceDocumentizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_DOCUMENTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_DOCUMENTIZABILITY_DOMAINS.map(async (entry) => {
