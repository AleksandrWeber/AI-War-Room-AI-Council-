...
import type { BroadcastizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_BROADCASTIZABILITY_TABLES } from './broadcastizability-rollout.helpers.js'

const WORKSPACE_BROADCASTIZABILITY_DOMAINS: Array<{
  domain: BroadcastizabilityAdminDomain
  tableName: string
...
@Injectable()
export class BroadcastizabilityStatusService {
  ...
  async getBroadcastizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_BROADCASTIZABILITY_TABLES,
    )

    return {
      existingBroadcastizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceBroadcastizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_BROADCASTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_BROADCASTIZABILITY_DOMAINS.map(async (entry) => {
