...
import type { CustomizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CUSTOMIZABILITY_TABLES } from './customizability-rollout.helpers.js'

const WORKSPACE_CUSTOMIZABILITY_DOMAINS: Array<{
  domain: CustomizabilityAdminDomain
  tableName: string
...
@Injectable()
export class CustomizabilityStatusService {
  ...
  async getCustomizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CUSTOMIZABILITY_TABLES,
    )

    return {
      existingCustomizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceCustomizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CUSTOMIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CUSTOMIZABILITY_DOMAINS.map(async (entry) => {
