...
import type { StylizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_STYLIZABILITY_TABLES } from './stylizability-rollout.helpers.js'

const WORKSPACE_STYLIZABILITY_DOMAINS: Array<{
  domain: StylizabilityAdminDomain
  tableName: string
...
@Injectable()
export class StylizabilityStatusService {
  ...
  async getStylizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_STYLIZABILITY_TABLES,
    )

    return {
      existingStylizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceStylizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_STYLIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_STYLIZABILITY_DOMAINS.map(async (entry) => {
