...
import type { RuleproofizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_RULEPROOFIZABILITY_TABLES } from './ruleproofizability-rollout.helpers.js'

const WORKSPACE_RULEPROOFIZABILITY_DOMAINS: Array<{
  domain: RuleproofizabilityAdminDomain
  tableName: string
...
@Injectable()
export class RuleproofizabilityStatusService {
  ...
  async getRuleproofizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_RULEPROOFIZABILITY_TABLES,
    )

    return {
      existingRuleproofizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceRuleproofizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_RULEPROOFIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_RULEPROOFIZABILITY_DOMAINS.map(async (entry) => {
