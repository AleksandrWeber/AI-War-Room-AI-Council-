...
import type { ComplianceizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_COMPLIANCEIZABILITY_TABLES } from './complianceizability-rollout.helpers.js'

const WORKSPACE_COMPLIANCEIZABILITY_DOMAINS: Array<{
  domain: ComplianceizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ComplianceizabilityStatusService {
  ...
  async getComplianceizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_COMPLIANCEIZABILITY_TABLES,
    )

    return {
      existingComplianceizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceComplianceizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_COMPLIANCEIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_COMPLIANCEIZABILITY_DOMAINS.map(async (entry) => {
