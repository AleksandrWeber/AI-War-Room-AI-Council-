...
import type { DiscoveryizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_DISCOVERYIZABILITY_TABLES } from './discoveryizability-rollout.helpers.js'

const WORKSPACE_DISCOVERYIZABILITY_DOMAINS: Array<{
  domain: DiscoveryizabilityAdminDomain
  tableName: string
...
@Injectable()
export class DiscoveryizabilityStatusService {
  ...
  async getDiscoveryizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_DISCOVERYIZABILITY_TABLES,
    )

    return {
      existingDiscoveryizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceDiscoveryizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_DISCOVERYIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_DISCOVERYIZABILITY_DOMAINS.map(async (entry) => {
