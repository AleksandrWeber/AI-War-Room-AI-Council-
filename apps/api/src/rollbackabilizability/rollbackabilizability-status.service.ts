...
import type { RollbackabilizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ROLLBACKABILIZABILITY_TABLES } from './rollbackabilizability-rollout.helpers.js'

const WORKSPACE_ROLLBACKABILIZABILITY_DOMAINS: Array<{
  domain: RollbackabilizabilityAdminDomain
  tableName: string
...
@Injectable()
export class RollbackabilizabilityStatusService {
  ...
  async getRollbackabilizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ROLLBACKABILIZABILITY_TABLES,
    )

    return {
      existingRollbackabilizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceRollbackabilizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ROLLBACKABILIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ROLLBACKABILIZABILITY_DOMAINS.map(async (entry) => {
