...
import type { DeadletterizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_DEADLETTERIZABILITY_TABLES } from './deadletterizability-rollout.helpers.js'

const WORKSPACE_DEADLETTERIZABILITY_DOMAINS: Array<{
  domain: DeadletterizabilityAdminDomain
  tableName: string
...
@Injectable()
export class DeadletterizabilityStatusService {
  ...
  async getDeadletterizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_DEADLETTERIZABILITY_TABLES,
    )

    return {
      existingDeadletterizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceDeadletterizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_DEADLETTERIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_DEADLETTERIZABILITY_DOMAINS.map(async (entry) => {
