...
import type { RegistrarproofizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_REGISTRARPROOFIZABILITY_TABLES } from './registrarproofizability-rollout.helpers.js'

const WORKSPACE_REGISTRARPROOFIZABILITY_DOMAINS: Array<{
  domain: RegistrarproofizabilityAdminDomain
  tableName: string
...
@Injectable()
export class RegistrarproofizabilityStatusService {
  ...
  async getRegistrarproofizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_REGISTRARPROOFIZABILITY_TABLES,
    )

    return {
      existingRegistrarproofizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceRegistrarproofizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_REGISTRARPROOFIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_REGISTRARPROOFIZABILITY_DOMAINS.map(async (entry) => {
