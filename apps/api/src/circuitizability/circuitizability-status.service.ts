...
import type { CircuitizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CIRCUITIZABILITY_TABLES } from './circuitizability-rollout.helpers.js'

const WORKSPACE_CIRCUITIZABILITY_DOMAINS: Array<{
  domain: CircuitizabilityAdminDomain
  tableName: string
...
@Injectable()
export class CircuitizabilityStatusService {
  ...
  async getCircuitizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CIRCUITIZABILITY_TABLES,
    )

    return {
      existingCircuitizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceCircuitizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CIRCUITIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CIRCUITIZABILITY_DOMAINS.map(async (entry) => {
