...
import type { OptimizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_OPTIMIZABILITY_TABLES } from './optimizability-rollout.helpers.js'

const WORKSPACE_OPTIMIZABILITY_DOMAINS: Array<{
  domain: OptimizabilityAdminDomain
  tableName: string
...
@Injectable()
export class OptimizabilityStatusService {
  ...
  async getOptimizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_OPTIMIZABILITY_TABLES,
    )

    return {
      existingOptimizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceOptimizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_OPTIMIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_OPTIMIZABILITY_DOMAINS.map(async (entry) => {
