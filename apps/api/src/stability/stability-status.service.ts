...
import type { StabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_STABILITY_TABLES } from './stability-rollout.helpers.js'

const WORKSPACE_STABILITY_DOMAINS: Array<{
  domain: StabilityAdminDomain
  tableName: string
...
@Injectable()
export class StabilityStatusService {
  ...
  async getStabilityTableCoverage() {
    const existingTables = await this.listExistingTables(CRITICAL_STABILITY_TABLES)

    return {
      existingStabilityTableCount: existingTables.size,
      existingTables,
      artifactsTableExists: existingTables.has('artifacts'),
    }
  }

  async getWorkspaceStabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_STABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_STABILITY_DOMAINS.map(async (entry) => {
