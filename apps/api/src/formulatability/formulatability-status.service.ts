...
import type { FormulatabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_FORMULATABILITY_TABLES } from './formulatability-rollout.helpers.js'

const WORKSPACE_FORMULATABILITY_DOMAINS: Array<{
  domain: FormulatabilityAdminDomain
  tableName: string
...
@Injectable()
export class FormulatabilityStatusService {
  ...
  async getFormulatabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_FORMULATABILITY_TABLES,
    )

    return {
      existingFormulatabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceFormulatabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_FORMULATABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_FORMULATABILITY_DOMAINS.map(async (entry) => {
