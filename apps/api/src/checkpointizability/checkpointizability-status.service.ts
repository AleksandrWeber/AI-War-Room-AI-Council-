...
import type { CheckpointizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CHECKPOINTIZABILITY_TABLES } from './checkpointizability-rollout.helpers.js'

const WORKSPACE_CHECKPOINTIZABILITY_DOMAINS: Array<{
  domain: CheckpointizabilityAdminDomain
  tableName: string
...
@Injectable()
export class CheckpointizabilityStatusService {
  ...
  async getCheckpointizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CHECKPOINTIZABILITY_TABLES,
    )

    return {
      existingCheckpointizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceCheckpointizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CHECKPOINTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CHECKPOINTIZABILITY_DOMAINS.map(async (entry) => {
