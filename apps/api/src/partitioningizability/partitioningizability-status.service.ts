...
import type { PartitioningizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_PARTITIONINGIZABILITY_TABLES } from './partitioningizability-rollout.helpers.js'

const WORKSPACE_PARTITIONINGIZABILITY_DOMAINS: Array<{
  domain: PartitioningizabilityAdminDomain
  tableName: string
...
@Injectable()
export class PartitioningizabilityStatusService {
  ...
  async getPartitioningizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_PARTITIONINGIZABILITY_TABLES,
    )

    return {
      existingPartitioningizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspacePartitioningizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_PARTITIONINGIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_PARTITIONINGIZABILITY_DOMAINS.map(async (entry) => {
