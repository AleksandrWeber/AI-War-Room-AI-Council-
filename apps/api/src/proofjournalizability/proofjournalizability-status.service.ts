...
import type { ProofjournalizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_PROOFJOURNALIZABILITY_TABLES } from './proofjournalizability-rollout.helpers.js'

const WORKSPACE_PROOFJOURNALIZABILITY_DOMAINS: Array<{
  domain: ProofjournalizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ProofjournalizabilityStatusService {
  ...
  async getProofjournalizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_PROOFJOURNALIZABILITY_TABLES,
    )

    return {
      existingProofjournalizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceProofjournalizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_PROOFJOURNALIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_PROOFJOURNALIZABILITY_DOMAINS.map(async (entry) => {
