...
import type { NavigabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_NAVIGABILITY_TABLES } from './navigability-rollout.helpers.js'

const WORKSPACE_NAVIGABILITY_DOMAINS: Array<{
  domain: NavigabilityAdminDomain
  tableName: string
...
@Injectable()
export class NavigabilityStatusService {
  ...
  async getNavigabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_NAVIGABILITY_TABLES,
    )

    return {
      existingNavigabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceNavigabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_NAVIGABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_NAVIGABILITY_DOMAINS.map(async (entry) => {
