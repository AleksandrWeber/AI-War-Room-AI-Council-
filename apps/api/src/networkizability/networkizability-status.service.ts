...
import type { NetworkizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_NETWORKIZABILITY_TABLES } from './networkizability-rollout.helpers.js'

const WORKSPACE_NETWORKIZABILITY_DOMAINS: Array<{
  domain: NetworkizabilityAdminDomain
  tableName: string
...
@Injectable()
export class NetworkizabilityStatusService {
  ...
  async getNetworkizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_NETWORKIZABILITY_TABLES,
    )

    return {
      existingNetworkizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceNetworkizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_NETWORKIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_NETWORKIZABILITY_DOMAINS.map(async (entry) => {
