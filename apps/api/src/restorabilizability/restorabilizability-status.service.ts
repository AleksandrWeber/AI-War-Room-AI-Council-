...
import type { RestorabilizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_RESTORABILIZABILITY_TABLES } from './restorabilizability-rollout.helpers.js'

const WORKSPACE_RESTORABILIZABILITY_DOMAINS: Array<{
  domain: RestorabilizabilityAdminDomain
  tableName: string
...
@Injectable()
export class RestorabilizabilityStatusService {
  ...
  async getRestorabilizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_RESTORABILIZABILITY_TABLES,
    )

    return {
      existingRestorabilizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceRestorabilizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_RESTORABILIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_RESTORABILIZABILITY_DOMAINS.map(async (entry) => {
