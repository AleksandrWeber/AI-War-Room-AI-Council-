...
import type { FlexibilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_FLEXIBILITYVAULTIZABILITY_TABLES } from './flexibilityvaultizability-rollout.helpers.js'

const WORKSPACE_FLEXIBILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: FlexibilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class FlexibilityvaultizabilityStatusService {
  ...
  async getFlexibilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_FLEXIBILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingFlexibilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceFlexibilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_FLEXIBILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_FLEXIBILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
