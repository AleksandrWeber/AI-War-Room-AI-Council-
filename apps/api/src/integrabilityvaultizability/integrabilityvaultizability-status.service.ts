...
import type { IntegrabilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_INTEGRABILITYVAULTIZABILITY_TABLES } from './integrabilityvaultizability-rollout.helpers.js'

const WORKSPACE_INTEGRABILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: IntegrabilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class IntegrabilityvaultizabilityStatusService {
  ...
  async getIntegrabilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_INTEGRABILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingIntegrabilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceIntegrabilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_INTEGRABILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_INTEGRABILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
