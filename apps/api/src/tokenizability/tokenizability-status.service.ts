...
import type { TokenizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_TOKENIZABILITY_TABLES } from './tokenizability-rollout.helpers.js'

const WORKSPACE_TOKENIZABILITY_DOMAINS: Array<{
  domain: TokenizabilityAdminDomain
  tableName: string
...
@Injectable()
export class TokenizabilityStatusService {
  ...
  async getTokenizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_TOKENIZABILITY_TABLES,
    )

    return {
      existingTokenizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceTokenizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_TOKENIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_TOKENIZABILITY_DOMAINS.map(async (entry) => {
