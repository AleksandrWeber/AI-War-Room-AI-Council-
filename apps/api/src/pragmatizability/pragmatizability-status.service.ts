...
import type { PragmatizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_PRAGMATIZABILITY_TABLES } from './pragmatizability-rollout.helpers.js'

const WORKSPACE_PRAGMATIZABILITY_DOMAINS: Array<{
  domain: PragmatizabilityAdminDomain
  tableName: string
...
@Injectable()
export class PragmatizabilityStatusService {
  ...
  async getPragmatizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_PRAGMATIZABILITY_TABLES,
    )

    return {
      existingPragmatizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspacePragmatizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_PRAGMATIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_PRAGMATIZABILITY_DOMAINS.map(async (entry) => {
