...
import type { IntegrityjournalizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_INTEGRITYJOURNALIZABILITY_TABLES } from './integrityjournalizability-rollout.helpers.js'

const WORKSPACE_INTEGRITYJOURNALIZABILITY_DOMAINS: Array<{
  domain: IntegrityjournalizabilityAdminDomain
  tableName: string
...
@Injectable()
export class IntegrityjournalizabilityStatusService {
  ...
  async getIntegrityjournalizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_INTEGRITYJOURNALIZABILITY_TABLES,
    )

    return {
      existingIntegrityjournalizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceIntegrityjournalizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_INTEGRITYJOURNALIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_INTEGRITYJOURNALIZABILITY_DOMAINS.map(async (entry) => {
