...
import type { ReviewabilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_REVIEWABILITYVAULTIZABILITY_TABLES } from './reviewabilityvaultizability-rollout.helpers.js'

const WORKSPACE_REVIEWABILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: ReviewabilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ReviewabilityvaultizabilityStatusService {
  ...
  async getReviewabilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_REVIEWABILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingReviewabilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceReviewabilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_REVIEWABILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_REVIEWABILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
