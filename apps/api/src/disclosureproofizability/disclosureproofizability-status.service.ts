...
import type { DisclosureproofizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_DISCLOSUREPROOFIZABILITY_TABLES } from './disclosureproofizability-rollout.helpers.js'

const WORKSPACE_DISCLOSUREPROOFIZABILITY_DOMAINS: Array<{
  domain: DisclosureproofizabilityAdminDomain
  tableName: string
...
@Injectable()
export class DisclosureproofizabilityStatusService {
  ...
  async getDisclosureproofizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_DISCLOSUREPROOFIZABILITY_TABLES,
    )

    return {
      existingDisclosureproofizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceDisclosureproofizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_DISCLOSUREPROOFIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_DISCLOSUREPROOFIZABILITY_DOMAINS.map(async (entry) => {
