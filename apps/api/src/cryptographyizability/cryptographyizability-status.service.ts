...
import type { CryptographyizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CRYPTOGRAPHYIZABILITY_TABLES } from './cryptographyizability-rollout.helpers.js'

const WORKSPACE_CRYPTOGRAPHYIZABILITY_DOMAINS: Array<{
  domain: CryptographyizabilityAdminDomain
  tableName: string
...
@Injectable()
export class CryptographyizabilityStatusService {
  ...
  async getCryptographyizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CRYPTOGRAPHYIZABILITY_TABLES,
    )

    return {
      existingCryptographyizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceCryptographyizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CRYPTOGRAPHYIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CRYPTOGRAPHYIZABILITY_DOMAINS.map(async (entry) => {
