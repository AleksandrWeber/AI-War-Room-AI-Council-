...
import type { AssessabilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ASSESSABILITYVAULTIZABILITY_TABLES } from './assessabilityvaultizability-rollout.helpers.js'

const WORKSPACE_ASSESSABILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: AssessabilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class AssessabilityvaultizabilityStatusService {
  ...
  async getAssessabilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ASSESSABILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingAssessabilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAssessabilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ASSESSABILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ASSESSABILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
