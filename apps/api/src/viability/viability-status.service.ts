...
import type { ViabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_VIABILITY_TABLES } from './viability-rollout.helpers.js'

const WORKSPACE_VIABILITY_DOMAINS: Array<{
  domain: ViabilityAdminDomain
  tableName: string
...
@Injectable()
export class ViabilityStatusService {
  ...
  async getViabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_VIABILITY_TABLES,
    )

    return {
      existingViabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceViabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_VIABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_VIABILITY_DOMAINS.map(async (entry) => {
