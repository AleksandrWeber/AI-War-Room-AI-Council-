...
import type { MetaphorizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_METAPHORIZABILITY_TABLES } from './metaphorizability-rollout.helpers.js'

const WORKSPACE_METAPHORIZABILITY_DOMAINS: Array<{
  domain: MetaphorizabilityAdminDomain
  tableName: string
...
@Injectable()
export class MetaphorizabilityStatusService {
  ...
  async getMetaphorizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_METAPHORIZABILITY_TABLES,
    )

    return {
      existingMetaphorizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceMetaphorizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_METAPHORIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_METAPHORIZABILITY_DOMAINS.map(async (entry) => {
