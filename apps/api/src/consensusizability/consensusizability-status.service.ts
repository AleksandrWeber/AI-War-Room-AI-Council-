...
import type { ConsensusizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CONSENSUSIZABILITY_TABLES } from './consensusizability-rollout.helpers.js'

const WORKSPACE_CONSENSUSIZABILITY_DOMAINS: Array<{
  domain: ConsensusizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ConsensusizabilityStatusService {
  ...
  async getConsensusizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CONSENSUSIZABILITY_TABLES,
    )

    return {
      existingConsensusizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceConsensusizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CONSENSUSIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CONSENSUSIZABILITY_DOMAINS.map(async (entry) => {
