...
import type { ControlizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CONTROLIZABILITY_TABLES } from './controlizability-rollout.helpers.js'

const WORKSPACE_CONTROLIZABILITY_DOMAINS: Array<{
  domain: ControlizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ControlizabilityStatusService {
  ...
  async getControlizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CONTROLIZABILITY_TABLES,
    )

    return {
      existingControlizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceControlizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CONTROLIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CONTROLIZABILITY_DOMAINS.map(async (entry) => {
