...
import type { PortabilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_PORTABILITYVAULTIZABILITY_TABLES } from './portabilityvaultizability-rollout.helpers.js'

const WORKSPACE_PORTABILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: PortabilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class PortabilityvaultizabilityStatusService {
  ...
  async getPortabilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_PORTABILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingPortabilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspacePortabilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_PORTABILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_PORTABILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
