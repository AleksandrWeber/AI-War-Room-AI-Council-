...
import type { IntelligibilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_INTELLIGIBILITY_TABLES } from './intelligibility-rollout.helpers.js'

const WORKSPACE_INTELLIGIBILITY_DOMAINS: Array<{
  domain: IntelligibilityAdminDomain
  tableName: string
...
@Injectable()
export class IntelligibilityStatusService {
  ...
  async getIntelligibilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_INTELLIGIBILITY_TABLES,
    )

    return {
      existingIntelligibilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceIntelligibilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_INTELLIGIBILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_INTELLIGIBILITY_DOMAINS.map(async (entry) => {
