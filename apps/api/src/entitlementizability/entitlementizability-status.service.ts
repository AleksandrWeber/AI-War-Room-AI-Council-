...
import type { EntitlementizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ENTITLEMENTIZABILITY_TABLES } from './entitlementizability-rollout.helpers.js'

const WORKSPACE_ENTITLEMENTIZABILITY_DOMAINS: Array<{
  domain: EntitlementizabilityAdminDomain
  tableName: string
...
@Injectable()
export class EntitlementizabilityStatusService {
  ...
  async getEntitlementizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ENTITLEMENTIZABILITY_TABLES,
    )

    return {
      existingEntitlementizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceEntitlementizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ENTITLEMENTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ENTITLEMENTIZABILITY_DOMAINS.map(async (entry) => {
