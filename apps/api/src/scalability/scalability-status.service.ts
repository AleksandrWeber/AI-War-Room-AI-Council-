...
import type { ScalabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_SCALABILITY_TABLES } from './scalability-rollout.helpers.js'

const WORKSPACE_SCALABILITY_DOMAINS: Array<{
  domain: ScalabilityAdminDomain
  tableName: string
...
@Injectable()
export class ScalabilityStatusService {
  ...
  async getScalabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_SCALABILITY_TABLES,
    )

    return {
      existingScalabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceScalabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_SCALABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_SCALABILITY_DOMAINS.map(async (entry) => {
