...
import type { ReplicationizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_REPLICATIONIZABILITY_TABLES } from './replicationizability-rollout.helpers.js'

const WORKSPACE_REPLICATIONIZABILITY_DOMAINS: Array<{
  domain: ReplicationizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ReplicationizabilityStatusService {
  ...
  async getReplicationizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_REPLICATIONIZABILITY_TABLES,
    )

    return {
      existingReplicationizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceReplicationizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_REPLICATIONIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_REPLICATIONIZABILITY_DOMAINS.map(async (entry) => {
