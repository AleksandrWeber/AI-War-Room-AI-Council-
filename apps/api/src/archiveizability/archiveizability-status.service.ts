...
import type { ArchiveizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ARCHIVEIZABILITY_TABLES } from './archiveizability-rollout.helpers.js'

const WORKSPACE_ARCHIVEIZABILITY_DOMAINS: Array<{
  domain: ArchiveizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ArchiveizabilityStatusService {
  ...
  async getArchiveizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ARCHIVEIZABILITY_TABLES,
    )

    return {
      existingArchiveizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceArchiveizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ARCHIVEIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ARCHIVEIZABILITY_DOMAINS.map(async (entry) => {
