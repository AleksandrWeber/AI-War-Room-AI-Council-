...
import type { DialectizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_DIALECTIZABILITY_TABLES } from './dialectizability-rollout.helpers.js'

const WORKSPACE_DIALECTIZABILITY_DOMAINS: Array<{
  domain: DialectizabilityAdminDomain
  tableName: string
...
@Injectable()
export class DialectizabilityStatusService {
  ...
  async getDialectizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_DIALECTIZABILITY_TABLES,
    )

    return {
      existingDialectizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceDialectizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_DIALECTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_DIALECTIZABILITY_DOMAINS.map(async (entry) => {
