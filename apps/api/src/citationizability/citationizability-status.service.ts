...
import type { CitationizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CITATIONIZABILITY_TABLES } from './citationizability-rollout.helpers.js'

const WORKSPACE_CITATIONIZABILITY_DOMAINS: Array<{
  domain: CitationizabilityAdminDomain
  tableName: string
...
@Injectable()
export class CitationizabilityStatusService {
  ...
  async getCitationizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CITATIONIZABILITY_TABLES,
    )

    return {
      existingCitationizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceCitationizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CITATIONIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CITATIONIZABILITY_DOMAINS.map(async (entry) => {
