...
import type { DistinguishabilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_DISTINGUISHABILITYVAULTIZABILITY_TABLES } from './distinguishabilityvaultizability-rollout.helpers.js'

const WORKSPACE_DISTINGUISHABILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: DistinguishabilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class DistinguishabilityvaultizabilityStatusService {
  ...
  async getDistinguishabilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_DISTINGUISHABILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingDistinguishabilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceDistinguishabilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_DISTINGUISHABILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_DISTINGUISHABILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
