...
import type { PolicyproofizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_POLICYPROOFIZABILITY_TABLES } from './policyproofizability-rollout.helpers.js'

const WORKSPACE_POLICYPROOFIZABILITY_DOMAINS: Array<{
  domain: PolicyproofizabilityAdminDomain
  tableName: string
...
@Injectable()
export class PolicyproofizabilityStatusService {
  ...
  async getPolicyproofizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_POLICYPROOFIZABILITY_TABLES,
    )

    return {
      existingPolicyproofizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspacePolicyproofizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_POLICYPROOFIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_POLICYPROOFIZABILITY_DOMAINS.map(async (entry) => {
