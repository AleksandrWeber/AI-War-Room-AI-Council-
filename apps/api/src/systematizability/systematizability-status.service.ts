...
import type { SystematizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_SYSTEMATIZABILITY_TABLES } from './systematizability-rollout.helpers.js'

const WORKSPACE_SYSTEMATIZABILITY_DOMAINS: Array<{
  domain: SystematizabilityAdminDomain
  tableName: string
...
@Injectable()
export class SystematizabilityStatusService {
  ...
  async getSystematizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_SYSTEMATIZABILITY_TABLES,
    )

    return {
      existingSystematizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceSystematizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_SYSTEMATIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_SYSTEMATIZABILITY_DOMAINS.map(async (entry) => {
