...
import type { TunabilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_TUNABILITYVAULTIZABILITY_TABLES } from './tunabilityvaultizability-rollout.helpers.js'

const WORKSPACE_TUNABILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: TunabilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class TunabilityvaultizabilityStatusService {
  ...
  async getTunabilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_TUNABILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingTunabilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceTunabilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_TUNABILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_TUNABILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
