...
import type { AttestjournalizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ATTESTJOURNALIZABILITY_TABLES } from './attestjournalizability-rollout.helpers.js'

const WORKSPACE_ATTESTJOURNALIZABILITY_DOMAINS: Array<{
  domain: AttestjournalizabilityAdminDomain
  tableName: string
...
@Injectable()
export class AttestjournalizabilityStatusService {
  ...
  async getAttestjournalizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ATTESTJOURNALIZABILITY_TABLES,
    )

    return {
      existingAttestjournalizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAttestjournalizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ATTESTJOURNALIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ATTESTJOURNALIZABILITY_DOMAINS.map(async (entry) => {
