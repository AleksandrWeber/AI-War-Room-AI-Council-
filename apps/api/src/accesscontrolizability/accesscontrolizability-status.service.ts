...
import type { AccesscontrolizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ACCESSCONTROLIZABILITY_TABLES } from './accesscontrolizability-rollout.helpers.js'

const WORKSPACE_ACCESSCONTROLIZABILITY_DOMAINS: Array<{
  domain: AccesscontrolizabilityAdminDomain
  tableName: string
...
@Injectable()
export class AccesscontrolizabilityStatusService {
  ...
  async getAccesscontrolizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ACCESSCONTROLIZABILITY_TABLES,
    )

    return {
      existingAccesscontrolizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAccesscontrolizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ACCESSCONTROLIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ACCESSCONTROLIZABILITY_DOMAINS.map(async (entry) => {
