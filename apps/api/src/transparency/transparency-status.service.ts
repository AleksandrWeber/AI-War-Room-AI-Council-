...
import type { TransparencyAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_TRANSPARENCY_TABLES } from './transparency-rollout.helpers.js'

const WORKSPACE_TRANSPARENCY_DOMAINS: Array<{
  domain: TransparencyAdminDomain
  tableName: string
...
@Injectable()
export class TransparencyStatusService {
  ...
  async getTransparencyTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_TRANSPARENCY_TABLES,
    )

    return {
      existingTransparencyTableCount: existingTables.size,
      existingTables,
      runWorkflowsTableExists: existingTables.has('run_workflows'),
      billingNotificationsTableExists: existingTables.has(
        'billing_notifications',
      ),
      billingMeterUsageReportsTableExists: existingTables.has(
        'billing_meter_usage_reports',
      ),
    }
  }

  async getWorkspaceTransparencyInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_TRANSPARENCY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_TRANSPARENCY_DOMAINS.map(async (entry) => {
