...
import type { DependableizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_DEPENDABLEIZABILITY_TABLES } from './dependableizability-rollout.helpers.js'

const WORKSPACE_DEPENDABLEIZABILITY_DOMAINS: Array<{
  domain: DependableizabilityAdminDomain
  tableName: string
...
@Injectable()
export class DependableizabilityStatusService {
  ...
  async getDependableizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_DEPENDABLEIZABILITY_TABLES,
    )

    return {
      existingDependableizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceDependableizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_DEPENDABLEIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_DEPENDABLEIZABILITY_DOMAINS.map(async (entry) => {
