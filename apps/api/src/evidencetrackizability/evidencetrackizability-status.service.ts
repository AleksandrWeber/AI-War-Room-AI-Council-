...
import type { EvidencetrackizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_EVIDENCETRACKIZABILITY_TABLES } from './evidencetrackizability-rollout.helpers.js'

const WORKSPACE_EVIDENCETRACKIZABILITY_DOMAINS: Array<{
  domain: EvidencetrackizabilityAdminDomain
  tableName: string
...
@Injectable()
export class EvidencetrackizabilityStatusService {
  ...
  async getEvidencetrackizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_EVIDENCETRACKIZABILITY_TABLES,
    )

    return {
      existingEvidencetrackizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceEvidencetrackizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_EVIDENCETRACKIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_EVIDENCETRACKIZABILITY_DOMAINS.map(async (entry) => {
