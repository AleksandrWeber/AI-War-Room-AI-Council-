...
import type { BalancingizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_BALANCINGIZABILITY_TABLES } from './balancingizability-rollout.helpers.js'

const WORKSPACE_BALANCINGIZABILITY_DOMAINS: Array<{
  domain: BalancingizabilityAdminDomain
  tableName: string
...
@Injectable()
export class BalancingizabilityStatusService {
  ...
  async getBalancingizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_BALANCINGIZABILITY_TABLES,
    )

    return {
      existingBalancingizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceBalancingizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_BALANCINGIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_BALANCINGIZABILITY_DOMAINS.map(async (entry) => {
