...
import type { SimulatizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_SIMULATIZABILITY_TABLES } from './simulatizability-rollout.helpers.js'

const WORKSPACE_SIMULATIZABILITY_DOMAINS: Array<{
  domain: SimulatizabilityAdminDomain
  tableName: string
...
@Injectable()
export class SimulatizabilityStatusService {
  ...
  async getSimulatizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_SIMULATIZABILITY_TABLES,
    )

    return {
      existingSimulatizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceSimulatizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_SIMULATIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_SIMULATIZABILITY_DOMAINS.map(async (entry) => {
