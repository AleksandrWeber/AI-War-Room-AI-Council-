...
import type { ArticulabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ARTICULABILITY_TABLES } from './articulability-rollout.helpers.js'

const WORKSPACE_ARTICULABILITY_DOMAINS: Array<{
  domain: ArticulabilityAdminDomain
  tableName: string
...
@Injectable()
export class ArticulabilityStatusService {
  ...
  async getArticulabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ARTICULABILITY_TABLES,
    )

    return {
      existingArticulabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceArticulabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ARTICULABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ARTICULABILITY_DOMAINS.map(async (entry) => {
