...
import type { TracejournalizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_TRACEJOURNALIZABILITY_TABLES } from './tracejournalizability-rollout.helpers.js'

const WORKSPACE_TRACEJOURNALIZABILITY_DOMAINS: Array<{
  domain: TracejournalizabilityAdminDomain
  tableName: string
...
@Injectable()
export class TracejournalizabilityStatusService {
  ...
  async getTracejournalizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_TRACEJOURNALIZABILITY_TABLES,
    )

    return {
      existingTracejournalizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceTracejournalizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_TRACEJOURNALIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_TRACEJOURNALIZABILITY_DOMAINS.map(async (entry) => {
