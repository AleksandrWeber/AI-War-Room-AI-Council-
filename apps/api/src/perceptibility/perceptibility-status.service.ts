...
import type { PerceptibilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_PERCEPTIBILITY_TABLES } from './perceptibility-rollout.helpers.js'

const WORKSPACE_PERCEPTIBILITY_DOMAINS: Array<{
  domain: PerceptibilityAdminDomain
  tableName: string
...
@Injectable()
export class PerceptibilityStatusService {
  ...
  async getPerceptibilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_PERCEPTIBILITY_TABLES,
    )

    return {
      existingPerceptibilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspacePerceptibilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_PERCEPTIBILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_PERCEPTIBILITY_DOMAINS.map(async (entry) => {
