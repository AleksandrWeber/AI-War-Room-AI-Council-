...
import type { StandardizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_STANDARDIZABILITY_TABLES } from './standardizability-rollout.helpers.js'

const WORKSPACE_STANDARDIZABILITY_DOMAINS: Array<{
  domain: StandardizabilityAdminDomain
  tableName: string
...
@Injectable()
export class StandardizabilityStatusService {
  ...
  async getStandardizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_STANDARDIZABILITY_TABLES,
    )

    return {
      existingStandardizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceStandardizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_STANDARDIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_STANDARDIZABILITY_DOMAINS.map(async (entry) => {
