...
import type { DeliverabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_DELIVERABILITY_TABLES } from './deliverability-rollout.helpers.js'

const WORKSPACE_DELIVERABILITY_DOMAINS: Array<{
  domain: DeliverabilityAdminDomain
  tableName: string
...
@Injectable()
export class DeliverabilityStatusService {
  ...
  async getDeliverabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_DELIVERABILITY_TABLES,
    )

    return {
      existingDeliverabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceDeliverabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_DELIVERABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_DELIVERABILITY_DOMAINS.map(async (entry) => {
