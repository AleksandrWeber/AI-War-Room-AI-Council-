...
import type { CertifiabilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CERTIFIABILITYVAULTIZABILITY_TABLES } from './certifiabilityvaultizability-rollout.helpers.js'

const WORKSPACE_CERTIFIABILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: CertifiabilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class CertifiabilityvaultizabilityStatusService {
  ...
  async getCertifiabilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CERTIFIABILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingCertifiabilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceCertifiabilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CERTIFIABILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CERTIFIABILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
