...
import type { BackupizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_BACKUPIZABILITY_TABLES } from './backupizability-rollout.helpers.js'

const WORKSPACE_BACKUPIZABILITY_DOMAINS: Array<{
  domain: BackupizabilityAdminDomain
  tableName: string
...
@Injectable()
export class BackupizabilityStatusService {
  ...
  async getBackupizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_BACKUPIZABILITY_TABLES,
    )

    return {
      existingBackupizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceBackupizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_BACKUPIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_BACKUPIZABILITY_DOMAINS.map(async (entry) => {
