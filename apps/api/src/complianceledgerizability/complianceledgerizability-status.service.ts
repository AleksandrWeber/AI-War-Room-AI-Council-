...
import type { ComplianceledgerizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_COMPLIANCELEDGERIZABILITY_TABLES } from './complianceledgerizability-rollout.helpers.js'

const WORKSPACE_COMPLIANCELEDGERIZABILITY_DOMAINS: Array<{
  domain: ComplianceledgerizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ComplianceledgerizabilityStatusService {
  ...
  async getComplianceledgerizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_COMPLIANCELEDGERIZABILITY_TABLES,
    )

    return {
      existingComplianceledgerizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceComplianceledgerizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_COMPLIANCELEDGERIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_COMPLIANCELEDGERIZABILITY_DOMAINS.map(async (entry) => {
