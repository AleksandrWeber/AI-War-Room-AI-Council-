...
import type { WarrantabilityvaultizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_WARRANTABILITYVAULTIZABILITY_TABLES } from './warrantabilityvaultizability-rollout.helpers.js'

const WORKSPACE_WARRANTABILITYVAULTIZABILITY_DOMAINS: Array<{
  domain: WarrantabilityvaultizabilityAdminDomain
  tableName: string
...
@Injectable()
export class WarrantabilityvaultizabilityStatusService {
  ...
  async getWarrantabilityvaultizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_WARRANTABILITYVAULTIZABILITY_TABLES,
    )

    return {
      existingWarrantabilityvaultizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceWarrantabilityvaultizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_WARRANTABILITYVAULTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_WARRANTABILITYVAULTIZABILITY_DOMAINS.map(async (entry) => {
