...
import type { ThesaurusizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_THESAURUSIZABILITY_TABLES } from './thesaurusizability-rollout.helpers.js'

const WORKSPACE_THESAURUSIZABILITY_DOMAINS: Array<{
  domain: ThesaurusizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ThesaurusizabilityStatusService {
  ...
  async getThesaurusizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_THESAURUSIZABILITY_TABLES,
    )

    return {
      existingThesaurusizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceThesaurusizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_THESAURUSIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_THESAURUSIZABILITY_DOMAINS.map(async (entry) => {
