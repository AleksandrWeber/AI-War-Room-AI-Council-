...
import type { DistinguishabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_DISTINGUISHABILITY_TABLES } from './distinguishability-rollout.helpers.js'

const WORKSPACE_DISTINGUISHABILITY_DOMAINS: Array<{
  domain: DistinguishabilityAdminDomain
  tableName: string
...
@Injectable()
export class DistinguishabilityStatusService {
  ...
  async getDistinguishabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_DISTINGUISHABILITY_TABLES,
    )

    return {
      existingDistinguishabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceDistinguishabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_DISTINGUISHABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_DISTINGUISHABILITY_DOMAINS.map(async (entry) => {
