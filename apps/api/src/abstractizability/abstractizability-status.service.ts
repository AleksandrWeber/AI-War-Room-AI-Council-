...
import type { AbstractizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ABSTRACTIZABILITY_TABLES } from './abstractizability-rollout.helpers.js'

const WORKSPACE_ABSTRACTIZABILITY_DOMAINS: Array<{
  domain: AbstractizabilityAdminDomain
  tableName: string
...
@Injectable()
export class AbstractizabilityStatusService {
  ...
  async getAbstractizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ABSTRACTIZABILITY_TABLES,
    )

    return {
      existingAbstractizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAbstractizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ABSTRACTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ABSTRACTIZABILITY_DOMAINS.map(async (entry) => {
