...
import type { FailoverizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_FAILOVERIZABILITY_TABLES } from './failoverizability-rollout.helpers.js'

const WORKSPACE_FAILOVERIZABILITY_DOMAINS: Array<{
  domain: FailoverizabilityAdminDomain
  tableName: string
...
@Injectable()
export class FailoverizabilityStatusService {
  ...
  async getFailoverizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_FAILOVERIZABILITY_TABLES,
    )

    return {
      existingFailoverizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceFailoverizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_FAILOVERIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_FAILOVERIZABILITY_DOMAINS.map(async (entry) => {
