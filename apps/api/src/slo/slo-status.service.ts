...
import type { SloAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_SLO_TABLES } from './slo-rollout.helpers.js'

const WORKSPACE_SLO_DOMAINS: Array<{
  domain: SloAdminDomain
  tableName: string
...
export class SloStatusService {
  ...
  async getSloTableCoverage() {
    const existingTables = await this.listExistingTables(CRITICAL_SLO_TABLES)

    return {
      existingSloTableCount: existingTables.size,
      existingTables,
      modelHealthEventTableExists: existingTables.has('model_health_events'),
    }
  }

  async getWorkspaceSloInventory(
    workspaceId: string,
    observabilityErrorCount: number,
  ) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_SLO_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_SLO_DOMAINS.map(async (entry) => {
        if (entry.domain === 'observability_errors') {
          return {
            domain: entry.domain,
            tableName: entry.tableName,
            recordCount: observabilityErrorCount,
            tableExists: true,
          }
        }
