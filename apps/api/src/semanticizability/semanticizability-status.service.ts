...
import type { SemanticizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_SEMANTICIZABILITY_TABLES } from './semanticizability-rollout.helpers.js'

const WORKSPACE_SEMANTICIZABILITY_DOMAINS: Array<{
  domain: SemanticizabilityAdminDomain
  tableName: string
...
@Injectable()
export class SemanticizabilityStatusService {
  ...
  async getSemanticizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_SEMANTICIZABILITY_TABLES,
    )

    return {
      existingSemanticizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceSemanticizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_SEMANTICIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_SEMANTICIZABILITY_DOMAINS.map(async (entry) => {
