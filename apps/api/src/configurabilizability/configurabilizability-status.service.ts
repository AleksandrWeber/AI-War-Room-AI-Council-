...
import type { ConfigurabilizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CONFIGURABILIZABILITY_TABLES } from './configurabilizability-rollout.helpers.js'

const WORKSPACE_CONFIGURABILIZABILITY_DOMAINS: Array<{
  domain: ConfigurabilizabilityAdminDomain
  tableName: string
...
@Injectable()
export class ConfigurabilizabilityStatusService {
  ...
  async getConfigurabilizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CONFIGURABILIZABILITY_TABLES,
    )

    return {
      existingConfigurabilizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceConfigurabilizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CONFIGURABILIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CONFIGURABILIZABILITY_DOMAINS.map(async (entry) => {
