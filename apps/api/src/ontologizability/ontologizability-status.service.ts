...
import type { OntologizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ONTOLOGIZABILITY_TABLES } from './ontologizability-rollout.helpers.js'

const WORKSPACE_ONTOLOGIZABILITY_DOMAINS: Array<{
  domain: OntologizabilityAdminDomain
  tableName: string
...
@Injectable()
export class OntologizabilityStatusService {
  ...
  async getOntologizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ONTOLOGIZABILITY_TABLES,
    )

    return {
      existingOntologizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceOntologizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ONTOLOGIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ONTOLOGIZABILITY_DOMAINS.map(async (entry) => {
