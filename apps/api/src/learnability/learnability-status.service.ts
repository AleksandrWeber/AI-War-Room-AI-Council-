...
import type { LearnabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_LEARNABILITY_TABLES } from './learnability-rollout.helpers.js'

const WORKSPACE_LEARNABILITY_DOMAINS: Array<{
  domain: LearnabilityAdminDomain
  tableName: string
...
@Injectable()
export class LearnabilityStatusService {
  ...
  async getLearnabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_LEARNABILITY_TABLES,
    )

    return {
      existingLearnabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceLearnabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_LEARNABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_LEARNABILITY_DOMAINS.map(async (entry) => {
