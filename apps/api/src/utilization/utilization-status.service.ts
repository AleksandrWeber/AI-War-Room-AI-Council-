...
import type { UtilizationAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_UTILIZATION_TABLES } from './utilization-rollout.helpers.js'

const WORKSPACE_UTILIZATION_DOMAINS: Array<{
  domain: UtilizationAdminDomain
  tableName: string
...
export class UtilizationStatusService {
  ...
  async getUtilizationTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_UTILIZATION_TABLES,
    )

    return {
      existingUtilizationTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceUtilizationInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_UTILIZATION_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_UTILIZATION_DOMAINS.map(async (entry) => {
