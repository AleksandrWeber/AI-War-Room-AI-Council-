...
import type { PersistizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_PERSISTIZABILITY_TABLES } from './persistizability-rollout.helpers.js'

const WORKSPACE_PERSISTIZABILITY_DOMAINS: Array<{
  domain: PersistizabilityAdminDomain
  tableName: string
...
@Injectable()
export class PersistizabilityStatusService {
  ...
  async getPersistizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_PERSISTIZABILITY_TABLES,
    )

    return {
      existingPersistizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspacePersistizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_PERSISTIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_PERSISTIZABILITY_DOMAINS.map(async (entry) => {
