...
import type { BatchizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_BATCHIZABILITY_TABLES } from './batchizability-rollout.helpers.js'

const WORKSPACE_BATCHIZABILITY_DOMAINS: Array<{
  domain: BatchizabilityAdminDomain
  tableName: string
...
@Injectable()
export class BatchizabilityStatusService {
  ...
  async getBatchizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_BATCHIZABILITY_TABLES,
    )

    return {
      existingBatchizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceBatchizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_BATCHIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_BATCHIZABILITY_DOMAINS.map(async (entry) => {
