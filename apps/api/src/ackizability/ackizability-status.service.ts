...
import type { AckizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_ACKIZABILITY_TABLES } from './ackizability-rollout.helpers.js'

const WORKSPACE_ACKIZABILITY_DOMAINS: Array<{
  domain: AckizabilityAdminDomain
  tableName: string
...
@Injectable()
export class AckizabilityStatusService {
  ...
  async getAckizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_ACKIZABILITY_TABLES,
    )

    return {
      existingAckizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceAckizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_ACKIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_ACKIZABILITY_DOMAINS.map(async (entry) => {
