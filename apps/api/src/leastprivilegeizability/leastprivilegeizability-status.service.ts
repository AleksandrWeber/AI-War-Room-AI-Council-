...
import type { LeastprivilegeizabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_LEASTPRIVILEGEIZABILITY_TABLES } from './leastprivilegeizability-rollout.helpers.js'

const WORKSPACE_LEASTPRIVILEGEIZABILITY_DOMAINS: Array<{
  domain: LeastprivilegeizabilityAdminDomain
  tableName: string
...
@Injectable()
export class LeastprivilegeizabilityStatusService {
  ...
  async getLeastprivilegeizabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_LEASTPRIVILEGEIZABILITY_TABLES,
    )

    return {
      existingLeastprivilegeizabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceLeastprivilegeizabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_LEASTPRIVILEGEIZABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_LEASTPRIVILEGEIZABILITY_DOMAINS.map(async (entry) => {
