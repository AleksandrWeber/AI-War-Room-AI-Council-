...
import type { ConfirmabilityAdminDomain } from '@ai-war-room/schemas'
import { PostgresService } from '../persistence/postgres.service.js'
import { CRITICAL_CONFIRMABILITY_TABLES } from './confirmability-rollout.helpers.js'

const WORKSPACE_CONFIRMABILITY_DOMAINS: Array<{
  domain: ConfirmabilityAdminDomain
  tableName: string
...
@Injectable()
export class ConfirmabilityStatusService {
  ...
  async getConfirmabilityTableCoverage() {
    const existingTables = await this.listExistingTables(
      CRITICAL_CONFIRMABILITY_TABLES,
    )

    return {
      existingConfirmabilityTableCount: existingTables.size,
      existingTables,
  ...
  async getWorkspaceConfirmabilityInventory(workspaceId: string) {
    const postgresTableNames = [
      ...new Set(
        WORKSPACE_CONFIRMABILITY_DOMAINS.flatMap((entry) => entry.requiredTables),
      ),
    ]
    const existingTables = await this.listExistingTables(postgresTableNames)

    return Promise.all(
      WORKSPACE_CONFIRMABILITY_DOMAINS.map(async (entry) => {
