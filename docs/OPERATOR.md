...
5. **Shield blocks / override disputes** — use Shield override with reason (audit trail); review workspace Shield admin false-positive summary. Production FP budget is **≤ 5%** on the review set (`docs/PRODUCT_POLICIES.md`). Secrets/PII finding quotes are redacted in PostgreSQL. Business-tier optional full-scan retain (`SHIELD_FULL_SCAN_RETAIN_*`) keeps unredacted quotes briefly for dispute/debug; purge via Shield review admin. Layer 2 LLM escalation is on by default (`SHIELD_LLM_ESCALATION_ENABLED`); disable if you want deterministic-only scans.
